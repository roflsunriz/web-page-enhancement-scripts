...
`.trim();class q{styleElement;constructor(){this.styleElement=this.findOrCreateStyle(I);}apply(e){this.styleElement.textContent=this.createCss(e);}findOrCreateStyle(e){const t=document.getElementById(e);if(t instanceof HTMLStyleElement)return t;const i=document.createElement("style");return i.id=e,(document.head||document.documentElement).appendChild(i),i}createCss(e){if(!e.globalEnabled)return "";const t=[];for(const[i,r]of Object.entries(_))!e[i]||r.length===0||t.push(`${r.join(`,
`)} {
  display: none !important;
}`);return e.blurVideoThumbnails&&t.push(`${N.join(`,
`)} {
  filter: blur(20px) !important;
}`),e.shrinkVideoThumbnails&&t.push(V),e.hideEntireWatchSidebar&&t.push(D),e.hideLeftNavigation&&t.push(U),e.disableLogoLink&&t.push(P),e.reverseChannelVideos&&t.push(O),e.searchEngineMode&&t.push(Y),e.grayscaleMode&&t.push(`ytd-app, ytm-app {
  filter: grayscale(100%) !important;
}`),`/* YouTube UI Modifier */
${t.join(`

`)}`}}const x=[{boxId:"youtube-ui-modifier-homepage-reveal",containerSelector:"ytd-page-manager",enabledSetting:"showHomepageRevealBox",revealSetting:"showHomepageRevealBox",targetSetting:"hideHomepageSuggestions",shouldShow:d=>d.hideHomepageSuggestions},{boxId:"youtube-ui-modifier-sidebar-reveal",containerSelector:"#secondary-inner",enabledSetting:"showSidebarRevealBox",revealSetting:"showSidebarRevealBox",targetSetting:"hideWatchSidebar",shouldShow:d=>d.hideWatchSidebar},{boxId:"youtube-ui-modifier-endscreen-reveal",containerSelector:"#movie_player",enabledSetting:"showEndScreenRevealBox",revealSetting:"showEndScreenRevealBox",targetSetting:"hideEndScreen",shouldShow:d=>d.hideEndScreen&&document.querySelector("#movie_player.ended-mode")!==null}];class F{closedBoxIds=new Set;apply(e,t){if(!e.globalEnabled){this.removeAll();return}x.forEach(i=>{if(!e[i.enabledSetting]||!i.shouldShow(e)||this.closedBoxIds.has(i.boxId)){document.getElementById(i.boxId)?.remove();return}this.ensureBox(i,t);});}ensureBox(e,t){if(document.getElementById(e.boxId))return;const i=document.querySelector(e.containerSelector);if(!i)return;const r=document.createElement("div");r.id=e.boxId,r.className="youtube-ui-modifier-reveal-box";const o=document.createElement("div");o.className="youtube-ui-modifier-reveal-actions";const a=document.createElement("button");a.type="button",a.textContent="今後表示しない",a.addEventListener("click",()=>{t(e.revealSetting,false),r.remove();}),o.appendChild(a);const n=document.createElement("button");n.type="button",n.textContent="x",n.addEventListener("click",()=>{this.closedBoxIds.add(e.boxId),r.remove();}),o.appendChild(n);const c=document.createElement("button");c.type="button",c.className="youtube-ui-modifier-reveal-primary",c.textContent="おすすめを表示する",c.addEventListener("click",()=>{t(e.targetSetting,false),r.remove();}),r.appendChild(o),r.appendChild(c),i.appendChild(r);}removeAll(){x.forEach(e=>document.getElementById(e.boxId)?.remove());}}class W{storage=new R;styleManager=new q;domMarker=new M;revealBoxManager=new F;settingsUi;settings={...l};observer=null;applyTimer=null;actionInterval=null;constructor(){this.settingsUi=new B({categories:H,getSettings:()=>this.settings,onSettingChange:(e,t)=>this.updateSetting(e,t),onReset:()=>this.resetSettings()});}async initialize(){this.settings=await this.storage.load(),this.applySettings(),this.registerMenuCommands(),this.startObserver(),this.startActionInterval(),this.patchHistoryNavigation(),document.addEventListener("keydown",this.handleKeydown);}registerMenuCommands(){p("YouTube UI Modifier 設定を開く",()=>this.openSettingsUi()),p("YouTube UI Modifier 有効/無効",()=>{this.updateSetting("globalEnabled",!this.settings.globalEnabled);});}updateSetting(e,t){this.settings={...this.settings,[e]:t},this.applySettingEffects(e,t),this.storage.save(this.settings),this.applySettings();}applySettingEffects(e,t){t&&(e==="showOnlyFirstHomepageRow"&&(this.settings.hideHomepageSuggestions=false,this.settings.hideHomepageExtraRows=true,this.settings.hideHomepageInfiniteScroll=true),e==="onlyShowPlaylists"&&(this.settings.hideLeftNavigation=false,this.settings.hideSubscriptionsSection=true,this.settings.hideExploreSection=true,this.settings.hideMoreFromYoutubeSection=true,this.settings.hideSettingsSection=true,this.settings.hideFooterSection=true,this.settings.hideHomeLink=true,this.settings.hideExploreLink=true,this.settings.hideShortsLink=true,this.settings.hideSubscriptionsLink=true),e==="redirectHomeToSubscriptions"?(this.settings.redirectHomeToWatchLater=false,this.settings.redirectHomeToLibrary=false):e==="redirectHomeToWatchLater"?(this.settings.redirectHomeToSubscriptions=false,this.settings.redirectHomeToLibrary=false):e==="redirectHomeToLibrary"&&(this.settings.redirectHomeToSubscriptions=false,this.settings.redirectHomeToWatchLater=false));}resetSettings(){this.settings={...l},this.storage.save(this.settings),this.applySettings();}applySettings(){const e=this.getEffectiveSettings();this.styleManager.apply(e),this.domMarker.apply(e),this.revealBoxManager.apply(e,(t,i)=>this.updateSetting(t,i)),this.settingsUi.refresh();}startObserver(){if(!document.body){window.setTimeout(()=>this.startObserver(),100);return}this.observer=new MutationObserver(()=>this.scheduleDomMarker()),this.observer.observe(document.body,{childList:true,subtree:true});}scheduleDomMarker(){this.applyTimer&&clearTimeout(this.applyTimer),this.applyTimer=setTimeout(()=>{this.applyTimer=null,this.domMarker.apply(this.getEffectiveSettings());},L);}startActionInterval(){this.actionInterval=setInterval(()=>{const e=this.getEffectiveSettings();this.domMarker.apply(e),this.revealBoxManager.apply(e,(t,i)=>this.updateSetting(t,i));},1e3);}openSettingsUi(){if(!(this.settings.lockSettingsWithCode&&window.prompt("YouTube UI Modifierの設定コードを入力してください")!=="youtube-ui-modifier")){if(this.settings.lockSettingsWithTimer){window.setTimeout(()=>this.settingsUi.show(),1e4);return}this.settingsUi.show();}}getEffectiveSettings(){return !this.settings.scheduleEnabled||this.isInsideSchedule()?this.settings:{...this.settings,globalEnabled:false}}isInsideSchedule(){const e=new Date,t=e.getDay(),i=e.getHours();return t>=1&&t<=5&&i>=9&&i<17}patchHistoryNavigation(){const e=()=>{window.setTimeout(()=>this.applySettings(),50),window.setTimeout(()=>this.applySettings(),500);},t=history.pushState.bind(history);history.pushState=(...r)=>{t(...r),e();};const i=history.replaceState.bind(history);history.replaceState=(...r)=>{i(...r),e();},window.addEventListener("popstate",e);}handleKeydown=e=>{e.key==="Escape"&&this.settingsUi.hide();}}window.top===window.self&&new W().initialize().catch(d=>{console.error("[YouTube UI Modifier] initialization failed",d);});
