...
`)}</textarea>
  ...
  `;const r=e.querySelector("#ctl-save-btn"),i=e.querySelector("#ctl-reset-btn"),o=e.querySelector("#ctl-cancel-btn");return r?.addEventListener("click",()=>{be(e),t.remove();}),i?.addEventListener("click",()=>{confirm("すべての設定をリセットしますか？")&&(ee(),B(),q(),X(),t.remove(),v.info("設定をリセットしました"),alert("設定をリセットして適用しました。"));}),o?.addEventListener("click",()=>{t.remove();}),t.addEventListener("click",l=>{l.target===t&&t.remove();}),t.appendChild(e),t}function X(){const t=document.querySelectorAll(m.article);t.forEach(e=>{delete e.dataset.ctlProcessed,K(e);}),v.info(`${t.length}件のツイートを再処理しました`);}function be(t){const e=i=>t.querySelector(`#${i}`)?.checked??false,r=i=>t.querySelector(`#${i}`)?.value??"";n.showPlaceholder=e("ctl-show-placeholder"),n.debugMode=e("ctl-debug-mode"),n.mediaFilter.enabled=e("ctl-media-enabled"),n.mediaFilter.enableOnTimeline=e("ctl-media-timeline"),n.mediaFilter.enableOnLists=e("ctl-media-lists"),n.mediaFilter.enableOnProfile=e("ctl-media-profile"),n.mediaFilter.enableOnSearch=e("ctl-media-search"),n.mediaFilter.enableOnTweetDetail=e("ctl-media-detail"),n.muteFilter.enabled=e("ctl-mute-enabled"),n.muteFilter.stringKeywords=r("ctl-mute-strings").split(`
`).map(i=>i.trim()).filter(i=>i.length>0),n.muteFilter.regexKeywords=r("ctl-mute-regexes").split(`
`).map(i=>i.trim()).filter(i=>i.length>0),n.retweetFilter.enabled=e("ctl-retweet-enabled"),C(),B(),q(),X(),v.info("設定を保存しました"),alert("設定を保存して適用しました。");}const E=f("twitter-clean-timeline");function ye(){E.info("Twitter Clean Timeline を初期化中..."),le(),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{R();}):R(),GM_registerMenuCommand("タイムラインフィルタ設定",ge),E.info("初期化完了",{mediaFilter:n.mediaFilter.enabled,muteFilter:n.muteFilter.enabled,retweetFilter:n.retweetFilter.enabled,showPlaceholder:n.showPlaceholder});}ye();
