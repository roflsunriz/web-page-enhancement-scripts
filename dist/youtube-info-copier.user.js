...
(function () {
  ...
  const C={debug:"debug",info:"info",warn:"warn",error:"error"},T={debug:10,info:20,warn:30,error:40};let _=n=>T[n]>=T.warn;const P=n=>{const e=`[${n}]`,t={};return Object.keys(C).forEach(i=>{const a=C[i];t[i]=(...o)=>{if(!_(i))return;(console[a]??console.log)(e,...o);};}),t},M=new WeakMap;function z(n){const{trustedTypes:e}=n;return !e||typeof e.createPolicy!="function"?null:e}function Y(n,e){let t=M.get(n);t||(t=new Map,M.set(n,t));const i=t.get(e);if(i)return i;const a=n.createPolicy(e,{createHTML:o=>o});return a?(t.set(e,a),a):null}function O(n,e,t=window){const i=z(t);if(!i)return n;const a=Y(i,e);return a?a.createHTML(n):n}function H(n,e,t){const a=(n.ownerDocument??document).defaultView??window,o=O(e,t,a);if(typeof o=="string"){n.innerHTML=o;return}n.innerHTML=o;}var U=typeof GM_getValue<"u"?GM_getValue:void 0,G=typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand:void 0,N=typeof GM_setClipboard<"u"?GM_setClipboard:void 0,j=typeof GM_setValue<"u"?GM_setValue:void 0;function v(n,e){G(n,e);}function X(n,e){j(n,e);}function Z(n,e){return U(n,e)}function Q(n,e){N(n,e);}const u={descriptionCandidates:["#description","ytd-expander#description","#meta-contents #description","#meta-contents"],inlineExpander:"ytd-text-inline-expander",inlineExpanderById:"ytd-text-inline-expander#description-inline-expander",interactiveElements:"tp-yt-paper-button, button, a",titleCandidates:["h1.ytd-watch-metadata yt-formatted-string","#title h1 yt-formatted-string","h1.title"],channelCandidates:["#owner #channel-name a","ytd-channel-name a",".ytd-video-owner-renderer a","#upload-info #channel-name a","#owner-text a"],descriptionRoot:"#description-inline-expander",descriptionExpandedContent:["#description-inline-expander #expanded yt-attributed-string","#description-inline-expander #expanded yt-formatted-string","#description-inline-expander #snippet yt-attributed-string"]},h=P("youtube-info-copier:dom-utils");async function $(n=4e3){const e=u.descriptionCandidates.map(o=>document.querySelector(o)).find(Boolean);if(!e)return;const t=()=>{const o=e.querySelector(u.inlineExpander)||document.querySelector(u.inlineExpanderById);if(o?.hasAttribute("is-expanded"))return h.debug("Already has is-expanded attribute"),true;const s=document.querySelector("tp-yt-paper-button#expand");if(s&&s.textContent?.includes("もっと見る"))try{return h.debug("Clicking tp-yt-paper-button#expand directly"),s.click(),h.info("概要欄の「もっと見る」ボタンをクリックしました。"),!0}catch(r){h.warn("Direct click failed:",r);}const c=/(もっと見る|もっと表示|もっと読む|Show more|SHOW MORE|More|一部を表示|…|...もっと見る)/i,l=[];o&&l.push(...Array.from(o.querySelectorAll(u.interactiveElements))),l.push(...Array.from(document.querySelectorAll(u.interactiveElements)));for(const r of l)try{const d=(r.textContent||"").trim();if((r.getAttribute("aria-expanded")==="false"||c.test(d)||r.id&&/expand|more|collapse/i.test(r.id))&&typeof r.click=="function")return h.debug(`Clicking element: ${r.tagName}#${r.id} "${d}"`),r.click(),h.info("概要欄の「もっと見る」ボタンをクリックしました（フォールバック）。"),!0}catch{}return h.warn("No expand button found"),false},i=()=>{const o=e.querySelector(u.inlineExpander)||document.querySelector(u.inlineExpanderById),s=(o||e).querySelector("#expanded");if(s){const c=(s.textContent||s.innerText||"").trim();if(c&&c.length>200)return h.debug(`Description is expanded (found ${c.length} chars in #expanded)`),true}if(o?.hasAttribute("is-expanded")&&s){const c=(s.textContent||s.innerText||"").trim();return c&&c.length>200?true:(h.debug("is-expanded attr found but content not yet loaded"),false)}return  false};if(!(i()||!t()))return new Promise(o=>{const s=Date.now();let c=0,l=null,r=null;const d=()=>{l&&(l.disconnect(),l=null),r&&(clearInterval(r),r=null);},p=I=>{c++;const S=e.querySelector("#expanded"),k=S?(S.textContent||"").trim().length:0,L=i(),m=Date.now()-s;return h.debug(`Check ${c} (${I}): expanded=${L}, #expanded length=${k}, elapsed=${m}ms`),L?(d(),h.info(`Description expanded successfully (#expanded: ${k} chars) after ${m}ms`),setTimeout(()=>o(),200),true):m>n?(d(),h.warn(`Description expansion timed out after ${m}ms (${c} checks)`),o(),true):false},b=e.querySelector("#expanded");b&&(l=new MutationObserver(()=>{p("mutation");}),l.observe(b,{childList:true,subtree:true,characterData:true}),h.debug("MutationObserver started on #expanded")),r=window.setInterval(()=>{p("poll");},200),p("initial");})}var W="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z",K="M19,21H8V7H19M19,5H8A2,2 0 0,0 6,7V21A2,2 0 0,0 8,23H19A2,2 0 0,0 21,21V7A2,2 0 0,0 19,5M16,1H4A2,2 0 0,0 2,3V17H4V3H16V1Z",J="M7,2V13H10V22L17,10H13L17,2H7Z",ee="M21,16H3V4H21M21,2H3C1.89,2 1,2.89 1,4V16A2,2 0 0,0 3,18H10V20H8V22H16V20H14V18H21A2,2 0 0,0 23,16V4C23,2.89 22.1,2 21,2Z";function x(n,e=24){const t=String(e),i=String(e);return `<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${i}" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true" focusable="false"><path d="${n}"></path></svg>`}const te=x(ee),y=x(K),q=x(J),ne=x(W),ie="youtube-info-copier-template";function oe(){return `
    <style>
