...
(function () {
  ...
  const m={debug:"debug",info:"info",warn:"warn",error:"error"},E=o=>{const e=`[${o}]`,t={};return Object.keys(m).forEach(n=>{const a=m[n];t[n]=(...i)=>{(console[a]??console.log)(e,...i);};}),t},y=new WeakMap;function k(o){const{trustedTypes:e}=o;return !e||typeof e.createPolicy!="function"?null:e}function L(o,e){let t=y.get(o);t||(t=new Map,y.set(o,t));const n=t.get(e);if(n)return n;const a=o.createPolicy(e,{createHTML:i=>i});return a?(t.set(e,a),a):null}function T(o,e,t=window){const n=k(t);if(!n)return o;const a=L(n,e);return a?a.createHTML(o):o}function C(o,e,t){const a=(o.ownerDocument??document).defaultView??window,i=T(e,t,a);if(typeof i=="string"){o.innerHTML=i;return}o.innerHTML=i;}const u={descriptionCandidates:["#description","ytd-expander#description","#meta-contents #description","#meta-contents"],inlineExpander:"ytd-text-inline-expander",inlineExpanderById:"ytd-text-inline-expander#description-inline-expander",interactiveElements:"tp-yt-paper-button, button, a",titleCandidates:["h1.ytd-watch-metadata yt-formatted-string","#title h1 yt-formatted-string","h1.title"],channelCandidates:["#owner #channel-name a","ytd-channel-name a",".ytd-video-owner-renderer a","#upload-info #channel-name a","#owner-text a"],descriptionRoot:"#description-inline-expander",descriptionExpandedContent:["#description-inline-expander #expanded yt-attributed-string","#description-inline-expander #expanded yt-formatted-string","#description-inline-expander #snippet yt-attributed-string"]},h=E("youtube-info-copier:dom-utils");async function S(o=4e3){const e=u.descriptionCandidates.map(i=>document.querySelector(i)).find(Boolean);if(!e)return;const t=()=>{const i=e.querySelector(u.inlineExpander)||document.querySelector(u.inlineExpanderById);if(i?.hasAttribute("is-expanded"))return  true;const r=/(もっと見る|もっと表示|もっと読む|Show more|SHOW MORE|More|一部を表示|…|...もっと見る)/i,c=[];i&&c.push(...Array.from(i.querySelectorAll(u.interactiveElements))),c.push(...Array.from(document.querySelectorAll(u.interactiveElements)));for(const s of c)try{const l=(s.textContent||"").trim();if((s.getAttribute("aria-expanded")==="false"||r.test(l)||s.id&&/expand|more|collapse/i.test(s.id))&&typeof s.click=="function")return s.click(),h.info("概要欄の「もっと見る」ボタンをクリックしました。"),!0}catch{}return  false},n=()=>{const i=e.querySelector(u.inlineExpander)||document.querySelector(u.inlineExpanderById),r=(i||e).querySelector("#expanded");if(r){const c=(r.textContent||r.innerText||"").trim();if(c&&c.length>200)return h.debug(`Description is expanded (found ${c.length} chars in #expanded)`),true}if(i?.hasAttribute("is-expanded")&&r){const c=(r.textContent||r.innerText||"").trim();return c&&c.length>200?true:(h.debug("is-expanded attr found but content not yet loaded"),false)}return  false};if(!(n()||!t()))return new Promise(i=>{let r=0;const c=Math.ceil(o/150),s=setInterval(()=>{r++;const l=e.querySelector("#expanded"),d=l?(l.textContent||"").trim().length:0,p=n(),x=r>=c;h.debug(`Check ${r}: expanded=${p}, #expanded length=${d}`),(p||x)&&(clearInterval(s),p?(h.info(`Description expanded successfully (#expanded: ${d} chars)`),setTimeout(()=>i(),200)):(h.warn(`Description expansion timed out after ${r} checks`),i()));},150);})}var M="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z",H="M19,21H8V7H19M19,5H8A2,2 0 0,0 6,7V21A2,2 0 0,0 8,23H19A2,2 0 0,0 21,21V7A2,2 0 0,0 19,5M16,1H4A2,2 0 0,0 2,3V17H4V3H16V1Z",F="M7,2V13H10V22L17,10H13L17,2H7Z",I="M21,16H3V4H21M21,2H3C1.89,2 1,2.89 1,4V16A2,2 0 0,0 3,18H10V20H8V22H16V20H14V18H21A2,2 0 0,0 23,16V4C23,2.89 22.1,2 21,2Z";function g(o,e=24){const t=String(e),n=String(e);return `<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${n}" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true" focusable="false"><path d="${o}"></path></svg>`}const R=g(I),$=g(H),q=g(F),D=g(M),V="youtube-info-copier-template";function P(){return `
    <style>
