...
(function () {
  ...
  const R={debug:"debug",info:"info",warn:"warn",error:"error"},F={debug:10,info:20,warn:30,error:40};let B=n=>F[n]>=F.warn;const H=n=>{const e=`[${n}]`,t={};return Object.keys(R).forEach(o=>{const r=R[o];t[o]=(...a)=>{if(!B(o))return;(console[r]??console.log)(e,...a);};}),t},A=new WeakMap;function _(n){const{trustedTypes:e}=n;return !e||typeof e.createPolicy!="function"?null:e}function O(n,e){let t=A.get(n);t||(t=new Map,A.set(n,t));const o=t.get(e);if(o)return o;const r=n.createPolicy(e,{createHTML:a=>a});return r?(t.set(e,r),r):null}function G(n,e,t=window){const o=_(t);if(!o)return n;const r=O(o,e);return r?r.createHTML(n):n}function V(n,e,t){const r=(n.ownerDocument??document).defaultView??window,a=G(e,t,r);if(typeof a=="string"){n.innerHTML=a;return}n.innerHTML=a;}var N=typeof GM_getValue<"u"?GM_getValue:void 0,j=typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand:void 0,X=typeof GM_setClipboard<"u"?GM_setClipboard:void 0,Z=typeof GM_setValue<"u"?GM_setValue:void 0;function S(n,e){j(n,e);}function K(n,e){Z(n,e);}function Q(n,e){return N(n,e)}function W(n,e){X(n,e);}const b={descriptionCandidates:["#description","ytd-expander#description","#meta-contents #description","#meta-contents"],inlineExpander:"ytd-text-inline-expander",inlineExpanderById:"ytd-text-inline-expander#description-inline-expander",interactiveElements:"tp-yt-paper-button, button, a",titleCandidates:["h1.ytd-watch-metadata yt-formatted-string","#title h1 yt-formatted-string","h1.title"],channelCandidates:["#owner #channel-name a","ytd-channel-name a",".ytd-video-owner-renderer a","#upload-info #channel-name a","#owner-text a"],descriptionRoot:"#description-inline-expander",descriptionExpandedContent:["#description-inline-expander #expanded yt-attributed-string","#description-inline-expander #expanded yt-formatted-string","#description-inline-expander #snippet yt-attributed-string"]},h=H("youtube-info-copier:dom-utils");async function $(n=4e3){const e=b.descriptionCandidates.map(a=>document.querySelector(a)).find(Boolean);if(!e)return;const t=()=>{const a=e.querySelector(b.inlineExpander)||document.querySelector(b.inlineExpanderById);if(a?.hasAttribute("is-expanded"))return h.debug("Already has is-expanded attribute"),true;const c=document.querySelector("tp-yt-paper-button#expand");if(c&&c.textContent?.includes("もっと見る"))try{return h.debug("Clicking tp-yt-paper-button#expand directly"),c.click(),h.info("概要欄の「もっと見る」ボタンをクリックしました。"),!0}catch(i){h.warn("Direct click failed:",i);}const p=/(もっと見る|もっと表示|もっと読む|Show more|SHOW MORE|More|一部を表示|…|...もっと見る)/i,d=[];a&&d.push(...Array.from(a.querySelectorAll(b.interactiveElements))),d.push(...Array.from(document.querySelectorAll(b.interactiveElements)));for(const i of d)try{const s=(i.textContent||"").trim();if((i.getAttribute("aria-expanded")==="false"||p.test(s)||i.id&&/expand|more|collapse/i.test(i.id))&&typeof i.click=="function")return h.debug(`Clicking element: ${i.tagName}#${i.id} "${s}"`),i.click(),h.info("概要欄の「もっと見る」ボタンをクリックしました（フォールバック）。"),!0}catch{}return h.warn("No expand button found"),false},o=()=>{const a=e.querySelector(b.inlineExpander)||document.querySelector(b.inlineExpanderById),c=(a||e).querySelector("#expanded");if(c){const p=(c.textContent||c.innerText||"").trim();if(p&&p.length>200)return h.debug(`Description is expanded (found ${p.length} chars in #expanded)`),true}if(a?.hasAttribute("is-expanded")&&c){const p=(c.textContent||c.innerText||"").trim();return p&&p.length>200?true:(h.debug("is-expanded attr found but content not yet loaded"),false)}return  false};if(!(o()||!t()))return new Promise(a=>{const c=Date.now();let p=0,d=null,i=null;const s=()=>{d&&(d.disconnect(),d=null),i&&(clearInterval(i),i=null);},l=v=>{p++;const g=e.querySelector("#expanded"),C=g?(g.textContent||"").trim().length:0,M=o(),y=Date.now()-c;return h.debug(`Check ${p} (${v}): expanded=${M}, #expanded length=${C}, elapsed=${y}ms`),M?(s(),h.info(`Description expanded successfully (#expanded: ${C} chars) after ${y}ms`),setTimeout(()=>a(),200),true):y>n?(s(),h.warn(`Description expansion timed out after ${y}ms (${p} checks)`),a(),true):false},f=e.querySelector("#expanded");f&&(d=new MutationObserver(()=>{l("mutation");}),d.observe(f,{childList:true,subtree:true,characterData:true}),h.debug("MutationObserver started on #expanded")),i=window.setInterval(()=>{l("poll");},200),l("initial");})}var J="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z",ee="M19,21H8V7H19M19,5H8A2,2 0 0,0 6,7V21A2,2 0 0,0 8,23H19A2,2 0 0,0 21,21V7A2,2 0 0,0 19,5M16,1H4A2,2 0 0,0 2,3V17H4V3H16V1Z",te="M7,2V13H10V22L17,10H13L17,2H7Z",ne="M21,16H3V4H21M21,2H3C1.89,2 1,2.89 1,4V16A2,2 0 0,0 3,18H10V20H8V22H16V20H14V18H21A2,2 0 0,0 23,16V4C23,2.89 22.1,2 21,2Z";function E(n,e=24){const t=String(e),o=String(e);return `<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${o}" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true" focusable="false"><path d="${n}"></path></svg>`}const oe=E(ne),T=E(ee),D=E(te),ie=E(J),ae=new Set(["ar","ur"]);function re(n){return ae.has(n)?"rtl":"ltr"}function se(n,e){return n.replace(/\{([a-zA-Z0-9_]+)\}/g,(t,o)=>{const r=e[o];return r===void 0?t:String(r)})}function le(n){const e=Object.keys(n.translations);let t=n.defaultLocale;const o=i=>{const s=i.toLowerCase(),l=n.aliases?.[s];if(l)return l;const f=e.find(g=>g.toLowerCase()===s);if(f)return f;const v=s.split("-")[0];return e.find(g=>g.toLowerCase().split("-")[0]===v)??null},r=()=>{const i=navigator.languages.length>0?navigator.languages:[navigator.language];for(const s of i){const l=o(s);if(l)return l}return n.fallbackLocale},a=i=>{const s=n.translations[t]?.[i];if(s)return s;const l=n.translations[n.fallbackLocale]?.[i];return l||(n.translations[n.defaultLocale]?.[i]??i)};return {locales:e,getLocale:()=>t,setLocale:i=>{t=i;},detectBrowserLocale:r,t:a,format:(i,s)=>se(a(i),s),getTranslations:(i=t)=>n.translations[i]??n.translations[n.fallbackLocale],getDirection:(i=t)=>re(i),getMissingTranslationKeys:i=>{const s=n.translations[n.fallbackLocale],l=n.translations[i];return Object.keys(s).filter(f=>!l[f])}}}const ce={ja:{controlLabel:"YouTube動画情報コピー",controlTitle:"YouTube動画情報",copyVideoInfo:"動画情報をコピー",copyTitleAndUrl:"タイトル+URLのみ",close:"閉じる",copiedSummary:"コピーした概要",unknownTitle:"タイトル不明",unknownAuthor:"投稿者不明",descriptionFailed:"概要取得に失敗しました",fullCopyText:`タイトル：{title}
投稿者名：{author}
URL：{url}
...
ناشر: {author}
URL: {url}
تفصیل: {description}`}},x=le({translations:ce,defaultLocale:"ja",fallbackLocale:"en",aliases:{zh:"zh-Hans","zh-cn":"zh-Hans","zh-sg":"zh-Hans"}});x.setLocale(x.detectBrowserLocale());const u=x.t,de=x.format,pe=x.getDirection,ue="youtube-info-copier-template";function he(){return `
    <style>
