...
(function () {
    ...
    `;const a=localStorage.getItem("translationProvider");a==="local"||a==="google"||a==="openai"?i.value=a:i.value="local",i.addEventListener("change",h=>{const g=h.target.value;localStorage.setItem("translationProvider",g),d.log(`Translation provider set to ${g}`);});const l=document.createElement("div");l.appendChild(s),l.appendChild(i),t.appendChild(l);const u=document.createElement("button");u.className="settings-button",u.type="button",u.title="設定",u.innerHTML=A.SETTINGS,u.addEventListener("click",()=>this.showSettingsModal()),t.appendChild(u),this.appendChild(t),this.controlPanel=t,this.configureHoverVisibility(),d.log("Control panel added to shadow DOM");}addMainButton(t){if(this.querySelector(".copy-thread-button"))return;const e=document.createElement("button");e.className="copy-thread-button",e.id="twitter-thread-copier-button",e.title="スレッドをコピー",e.addEventListener("click",async()=>{if(this.ignoreNextClick){this.ignoreNextClick=false;return}c.isSecondStage?await t("clipboard"):await t("copy");}),this.appendChild(e),this.mainButton=e,this.configureHoverVisibility(),this.updateMainButtonText(),this.initializeDrag(e),d.log("Copy button added to shadow DOM");}updateMainButtonText(){const t=this.querySelector("#twitter-thread-copier-button");if(!t)return;if(c.isCollecting){t.classList.add("loading"),t.classList.remove("ready"),t.innerHTML=`<span class="text">収集中...</span>${A.LOADING}`;return}if(c.translationInProgress){t.classList.add("loading"),t.classList.remove("ready"),t.innerHTML=`<span class="text">翻訳中...</span>${A.LOADING}`;return}if(t.classList.remove("loading"),t.classList.remove("ready"),c.isSecondStage){t.classList.add("ready"),t.innerHTML=`<span class="text">クリックしてコピー</span>${A.CLIPBOARD}`;return}const e=c.selectedTweetIds.length;if(e>0){t.innerHTML=`<span class="text">選択ツイート(${e})をコピー</span>${A.COPY}`;return}if(c.startFromTweetId){const o=c.startFromTweetText.length>20?c.startFromTweetText.substring(0,20)+"...":c.startFromTweetText;t.innerHTML=`<span class="text">${o}からコピー</span>${A.COPY}`;return}t.innerHTML=`<span class="text">スレッドをコピー</span>${A.COPY}`;}showToast(t,e){let o=this.querySelector(".copy-toast");o||(o=document.createElement("div"),o.className="copy-toast",this.appendChild(o)),o.innerHTML=`
...
`;for(let r=0;r<n.length;r++){const s=n[r],i=R(s),a=r===0?i:o+i;if(e.length+a.length>t){const l=t-e.length;l>o.length&&(e+=a.substring(0,l-3)+"...");break}e+=a;}return e}const Se=/https?:\/\/[^\s]+/g;class Ae{hasThreadUrlIncluded=false;remainingMediaSlots=5;consumeThreadUrl(t){return !t||this.hasThreadUrlIncluded?null:(this.hasThreadUrlIncluded=true,t)}consumeMediaUrls(t){if(this.remainingMediaSlots<=0)return [];const o=t.filter(r=>r.trim().length>0).slice(0,this.remainingMediaSlots);return this.remainingMediaSlots-=o.length,o}}function et(n){if(!n)return "";const e=n.replace(Se,"").replace(/[ \t]+\n/g,`
`).split(`
