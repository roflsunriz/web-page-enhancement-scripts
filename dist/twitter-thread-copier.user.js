...
(function () {
    ...
    `,this.shadowRoot.appendChild(t);}querySelector(t){return this.shadowRoot?this.shadowRoot.querySelector(t):null}appendChild(t){this.ensureFloatingContainer().appendChild(t);}extractTweetId(t){const e=t.querySelector('a[href*="/status/"]');return e&&(e.href.split("/").pop()?.split("?")[0]??"")||null}destroy(){this.container&&this.container.remove(),this.shadowRoot=null,this.container=null,this.floatingContainer=null,this.dragState=null,this.customPosition=null,this.ignoreNextClick=false,this.mainButton=null,this.controlPanel=null,this.hoverInteractionConfigured=false,this.hoverPointerCount=0,this.clearHoverHideTimeout(),window.removeEventListener("resize",this.handleResize),l.selectedTweetIds=[],u.log("Shadow DOM destroyed");}addSelectionButtons(){document.querySelectorAll(g.article).forEach(t=>{const e=this.extractTweetId(t);if(!e)return;let o=Array.from(t.children).find(r=>r.classList.contains("select-tweet-button"));o?o.dataset.tweetId||(o.dataset.tweetId=e):(o=document.createElement("button"),o.type="button",o.className="select-tweet-button",o.textContent="+",o.title="このツイートを選択",o.dataset.tweetId=e,o.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),this.toggleTweetSelection(t,e);}),t.appendChild(o));}),this.refreshSelectionIndicators();}refreshSelectionIndicators(){const t=new Map;l.selectedTweetIds.forEach((e,o)=>{t.set(e,o+1);}),document.querySelectorAll(g.article).forEach(e=>{const o=this.extractTweetId(e);if(!o)return;const r=e.querySelector(".select-tweet-button");if(r)if(t.has(o)){const s=t.get(o)??0;e.classList.add("tweet-selected"),r.classList.add("active"),r.textContent=s>0?s.toString():"✓",r.title=`選択中 (${s})`;}else e.classList.remove("tweet-selected"),r.classList.remove("active"),r.textContent="+",r.title="このツイートを選択";}),this.updateSelectionResetButton();}toggleTweetSelection(t,e){const o=l.selectedTweetIds.includes(e);o?l.selectedTweetIds=l.selectedTweetIds.filter(i=>i!==e):l.selectedTweetIds=[...l.selectedTweetIds,e],l.isSecondStage&&(l.isSecondStage=false),l.collectedThreadData=null;const r=l.selectedTweetIds.length,s=r>0?`${r}件選択中`:"選択をすべて解除しました";this.refreshSelectionIndicators(),this.updateMainButtonText(),o?this.showToast("選択解除",s):this.showToast("選択追加",s),u.log(`Selected tweet ids: ${l.selectedTweetIds.join(",")}`);}updateSelectionResetButton(){let t=this.querySelector(".reset-selection");l.selectedTweetIds.length>0?(t||(t=document.createElement("button"),t.className="reset-selection",t.textContent="選択をリセット",t.addEventListener("click",()=>this.resetSelection()),this.appendChild(t)),t.classList.add("visible")):t?.classList.remove("visible");}resetSelection(){l.selectedTweetIds.length!==0&&(l.selectedTweetIds=[],l.isSecondStage&&(l.isSecondStage=false),l.collectedThreadData=null,this.refreshSelectionIndicators(),this.updateMainButtonText(),this.showToast("選択リセット","選択したツイートをすべて解除しました"),u.log("Selections reset"));}addControlPanel(){if(this.querySelector(".control-panel-container"))return;const t=document.createElement("div");t.className="control-panel-container";const e=document.createElement("select");e.id="copy-mode-select",e.innerHTML=`
      <option value="all">全て</option>
      <option value="first">最初</option>
      <option value="shitaraba">4K</option>
      <option value="5ch">2K</option>
    `,e.value=l.copyMode,e.addEventListener("change",s=>{l.copyMode=s.target.value,u.log(`Copy mode changed to: ${l.copyMode}`);}),t.appendChild(e);const o=document.createElement("label"),r=document.createElement("input");r.type="checkbox",r.id="translate-checkbox",r.checked=l.translateEnabled,r.addEventListener("change",s=>{l.translateEnabled=s.target.checked,u.log(`Translation ${l.translateEnabled?"enabled":"disabled"}`);}),o.appendChild(r),o.appendChild(document.createTextNode("翻訳")),t.appendChild(o),this.appendChild(t),this.controlPanel=t,this.configureHoverVisibility(),u.log("Control panel added to shadow DOM");}addMainButton(t){if(this.querySelector(".copy-thread-button"))return;const e=document.createElement("button");e.className="copy-thread-button",e.id="twitter-thread-copier-button",e.title="スレッドをコピー",e.addEventListener("click",async()=>{if(this.ignoreNextClick){this.ignoreNextClick=false;return}l.isSecondStage?await t("clipboard"):await t("copy");}),this.appendChild(e),this.mainButton=e,this.configureHoverVisibility(),this.updateMainButtonText(),this.initializeDrag(e),u.log("Copy button added to shadow DOM");}updateMainButtonText(){const t=this.querySelector("#twitter-thread-copier-button");if(!t)return;if(l.isCollecting){t.classList.add("loading"),t.classList.remove("ready"),t.innerHTML=`<span class="text">収集中...</span>${v.LOADING}`;return}if(l.translationInProgress){t.classList.add("loading"),t.classList.remove("ready"),t.innerHTML=`<span class="text">翻訳中...</span>${v.LOADING}`;return}if(t.classList.remove("loading"),t.classList.remove("ready"),l.isSecondStage){t.classList.add("ready"),t.innerHTML=`<span class="text">クリックしてコピー</span>${v.CLIPBOARD}`;return}const e=l.selectedTweetIds.length;if(e>0){t.innerHTML=`<span class="text">選択ツイート(${e})をコピー</span>${v.COPY}`;return}if(l.startFromTweetId){const o=l.startFromTweetText.length>20?l.startFromTweetText.substring(0,20)+"...":l.startFromTweetText;t.innerHTML=`<span class="text">${o}からコピー</span>${v.COPY}`;return}t.innerHTML=`<span class="text">スレッドをコピー</span>${v.COPY}`;}showToast(t,e){let o=this.querySelector(".copy-toast");o||(o=document.createElement("div"),o.className="copy-toast",this.appendChild(o)),o.innerHTML=`
      <div class="toast-title">${t}</div>
      <div class="toast-content">${e.substring(0,100)}</div>
    `,o.classList.remove("visible"),setTimeout(()=>{o?.classList.add("visible"),setTimeout(()=>{o?.classList.remove("visible"),setTimeout(()=>o?.remove(),500);},3e3);},10);}updateAllUI(){this.addControlPanel(),this.addSelectionButtons(),this.addStartPointButtons(),this.updateResetButton();}addStartPointButtons(){document.querySelectorAll(g.article).forEach(t=>{if(Array.from(t.children).find(s=>s.classList.contains("start-point-button")))return;const o=this.extractTweetId(t);if(!o)return;const r=document.createElement("button");r.className="start-point-button",r.textContent="★",r.title="この位置からコピー開始",r.dataset.tweetId=o,l.startFromTweetId===o&&(r.classList.add("active"),r.textContent="✓",t.classList.add("start-point-set")),r.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation(),this.setStartPoint(t,o);}),t.appendChild(r);});}setStartPoint(t,e){l.startFromTweetId&&document.querySelectorAll(".start-point-set").forEach(i=>{i.classList.remove("start-point-set");const a=i.querySelector(".start-point-button");a&&(a.classList.remove("active"),a.textContent="★");});const o=t.querySelector(g.userName)?.innerText??"",r=t.querySelector(g.tweetText)?.innerText??"";l.startFromTweetId=e,l.startFromTweetAuthor=o,l.startFromTweetText=r,t.classList.add("start-point-set");const s=t.querySelector(".start-point-button");s&&(s.classList.add("active"),s.textContent="✓"),this.updateResetButton(),this.updateMainButtonText(),this.showToast("起点設定完了",`${o}のツイートを起点に設定しました`),u.log(`Start point set: ${e} by ${o}`);}updateResetButton(){let t=this.querySelector(".reset-start-point");l.startFromTweetId?(t||(t=document.createElement("button"),t.className="reset-start-point",t.textContent="起点をリセット",t.addEventListener("click",()=>this.resetStartPoint()),this.appendChild(t)),t.classList.add("visible")):t?.classList.remove("visible");}resetStartPoint(){l.startFromTweetId=null,l.startFromTweetAuthor="",l.startFromTweetText="",document.querySelectorAll(".start-point-set").forEach(t=>{t.classList.remove("start-point-set");const e=t.querySelector(".start-point-button");e&&(e.classList.remove("active"),e.textContent="★");}),this.updateResetButton(),this.updateMainButtonText(),this.showToast("起点リセット","コピー起点をリセットしました"),u.log("Start point reset");}configureHoverVisibility(){if(this.hoverInteractionConfigured||!this.mainButton||!this.controlPanel||!this.supportsHover())return;this.hoverInteractionConfigured=true;const t=this.ensureFloatingContainer(),e=this.controlPanel;e.classList.add("hover-hidden");const o=()=>{this.clearHoverHideTimeout(),t.classList.add("show-hover-controls");},r=()=>{this.clearHoverHideTimeout(),this.hoverHideTimeout=window.setTimeout(()=>{this.hoverPointerCount===0&&!this.hasFocusWithin(t)&&t.classList.remove("show-hover-controls");},150);},s=()=>{this.hoverPointerCount+=1,o();},i=a=>{this.hoverPointerCount=Math.max(0,this.hoverPointerCount-1);const c=a.relatedTarget;c&&t.contains(c)||r();};this.mainButton.addEventListener("pointerenter",s),this.mainButton.addEventListener("pointerleave",i),e.addEventListener("pointerenter",s),e.addEventListener("pointerleave",i),t.addEventListener("focusin",o),t.addEventListener("focusout",a=>{const c=a.relatedTarget;c&&t.contains(c)||r();});}clearHoverHideTimeout(){this.hoverHideTimeout!==null&&(window.clearTimeout(this.hoverHideTimeout),this.hoverHideTimeout=null);}hasFocusWithin(t){const e=document.activeElement;return !!e&&t.contains(e)}supportsHover(){try{return window.matchMedia("(hover: hover)").matches}catch(t){return u.warn("hover media query check failed",t),false}}ensureFloatingContainer(){if(!this.shadowRoot)throw new Error("Shadow root is not initialized");if(!this.floatingContainer){const t=document.createElement("div");t.className="floating-ui-container",this.shadowRoot.appendChild(t);const e=this.loadPosition();e&&(this.customPosition=e,this.applyPosition(e.top,e.left,t)),this.floatingContainer=t;}return this.floatingContainer}initializeDrag(t){if(t.dataset.dragInitialized==="true")return;t.dataset.dragInitialized="true",t.addEventListener("pointerdown",o=>{if(!o.isPrimary)return;const r=this.ensureFloatingContainer(),s=r.getBoundingClientRect();this.dragState={pointerId:o.pointerId,startX:o.clientX,startY:o.clientY,initialTop:this.customPosition?.top??s.top,initialLeft:this.customPosition?.left??s.left,moved:false},r.classList.add("dragging"),t.classList.add("dragging");try{t.setPointerCapture(o.pointerId);}catch(i){u.warn("pointer capture failed",i);}}),t.addEventListener("pointermove",o=>{if(!this.dragState||o.pointerId!==this.dragState.pointerId)return;const r=this.floatingContainer;if(!r)return;const s=o.clientX-this.dragState.startX,i=o.clientY-this.dragState.startY;if(!this.dragState.moved){if(Math.abs(s)<4&&Math.abs(i)<4)return;this.dragState.moved=true,r.classList.add("has-custom-position"),r.style.bottom="auto",r.style.right="auto";}const a=this.dragState.initialTop+i,c=this.dragState.initialLeft+s,{top:h,left:p}=this.clampPosition(a,c,r);this.applyPosition(h,p,r);});const e=o=>{!this.dragState||o.pointerId!==this.dragState.pointerId||(t.hasPointerCapture(o.pointerId)&&t.releasePointerCapture(o.pointerId),t.classList.remove("dragging"),this.floatingContainer?.classList.remove("dragging"),this.dragState.moved&&(this.ignoreNextClick=true,this.handleResize(),this.savePosition()),this.dragState=null);};t.addEventListener("pointerup",e),t.addEventListener("pointercancel",e);}clampPosition(t,e,o){const s=o.offsetHeight||0,i=o.offsetWidth||0,a=Math.max(16,window.innerHeight-s-16),c=Math.max(16,window.innerWidth-i-16);return {top:Math.min(Math.max(16,t),a),left:Math.min(Math.max(16,e),c)}}applyPosition(t,e,o){const r=o??this.ensureFloatingContainer();r.style.top=`${t}px`,r.style.left=`${e}px`,r.style.bottom="auto",r.style.right="auto",r.classList.add("has-custom-position"),this.customPosition={top:t,left:e},this.savePosition();}loadPosition(){try{const t=window.localStorage.getItem(this.storageKey);if(!t)return null;const e=JSON.parse(t);return typeof e.top=="number"&&typeof e.left=="number"&&Number.isFinite(e.top)&&Number.isFinite(e.left)?{top:e.top,left:e.left}:(u.warn("stored position is invalid",e),null)}catch(t){return u.warn("failed to load UI position",t),null}}savePosition(){if(this.customPosition)try{const t=JSON.stringify(this.customPosition);window.localStorage.setItem(this.storageKey,t);}catch(t){u.warn("failed to save UI position",t);}}}const y=new at;var lt=typeof GM_notification<"u"?GM_notification:void 0,ct=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0;const dt=ct;function ut(n,t,e,o){lt({text:n,title:t,image:e,onclick:o});}const ht="https://twitter.com",pt="https://t.co",ft="https://video.twimg.com",_=n=>`${ht}/i/status/${n}`,gt=n=>`${ft}/tweet_video/${n}.mp4`,mt=/https?:\/\/(twitter\.com|x\.com)\/[^/]+\/status\/[0-9]+/,wt=`${pt}/`,xt="https://translate.googleapis.com/translate_a/single";async function Tt(){try{const n=[],t=new Set;let e="",o="";const r=window.location.href,s=l.copyMode==="first";let i=!1;const a=window.scrollY;async function c(){if(i)return;const p=Array.from(document.querySelectorAll(g.article));p.length>0&&!e&&(e=H(p[0]),o=N(p[0]));for(const f of p){const x=f.querySelector(g.statusLink);if(!x)continue;const m=x.href.split("/"),d=m.indexOf("status");if(d===-1||d+1>=m.length)continue;const w=m[d+1].split("?")[0];if(t.has(w))continue;const T=H(f);if(e&&T!==e)continue;let b=N(f);T===e&&!b&&o&&(b=o);const k=f.querySelector(g.tweetText);let R="";k&&(R=await j(k));const U=f.querySelector("time"),q=U?U.getAttribute("datetime"):"",Z=q?It(new Date(q)):"",tt=bt(f),et=await yt(f),nt=t.size===0?r:x.href?x.href.split("?")[0]:"";if(n.push({id:w,author:T,handle:b,text:R,time:Z,url:nt,mediaUrls:tt,quotedTweet:et}),t.add(w),s){i=!0;break}}}async function h(){try{if(await D(),await c(),i)return;let x=n.length,m=0;for(let d=0;d<30;d++)try{if(window.scrollBy(0,window.innerHeight*.7),await new Promise(w=>setTimeout(w,700)),await D(),await c(),i)return;if(n.length===x){if(m++,m>=3)break}else x=n.length,m=0;}catch(w){u.error(`スクロール処理エラー (試行 ${d+1}): ${w.message}`);continue}try{window.scrollTo(0,a);}catch(d){u.error(`スクロール位置復元エラー: ${d.message}`);}}catch(x){throw u.error(`スクロール収集処理エラー: ${x.message}`),x}}return await h(),n.sort((p,f)=>{const x=new Date(p.time.replace(/年|月|日/g,"-").replace(/:/g,"-").split(" ")[0]),m=new Date(f.time.replace(/年|月|日/g,"-").replace(/:/g,"-").split(" ")[0]);return x.getTime()-m.getTime()}),n}catch(n){return u.error(`ツイート収集中にエラーが発生: ${n.message}`),u.error(`エラースタック: ${n.stack}`),[]}}function N(n){try{const e=Array.from(n.querySelectorAll("span")).find(s=>{try{const i=s.textContent?s.textContent.trim():"";return i.startsWith("@")&&!i.includes(" ")&&i.length>1}catch{return !1}});if(e)return e.textContent.trim();const o=Array.from(n.querySelectorAll(g.userLink));for(const s of o)try{const i=s.getAttribute("href");if(i&&!i.includes("/status/")&&i.length>1&&!i.includes("/i/"))return "@"+i.replace(/^\//,"")}catch{continue}const r=n.querySelector(g.userName);if(r){const s=r.querySelectorAll("span");for(const i of Array.from(s))try{const a=i.textContent?i.textContent.trim():"";if(a.startsWith("@")&&!a.includes(" "))return a}catch{continue}}return ""}catch(t){return u.error(`ユーザーハンドル取得エラー: ${t.message}`),""}}function H(n){try{const t=n.querySelector(g.userNameLinkSpan);return t&&t.textContent?t.textContent.trim():""}catch(t){return u.error(`ユーザー名取得エラー: ${t.message}`),""}}async function yt(n){const t=n.querySelector(g.quotedLink);let e=null;if(t)try{const o=t.closest(g.roleLink);o&&(e=await B(o));}catch(o){u.error(`引用ツイート取得エラー: ${o.message}`);}else try{const o=n.innerText||"",r=o.includes("引用")||o.includes("Quote")||o.includes("quote"),s=n.querySelectorAll(g.roleLink);if(s.length>0&&r)for(let i=0;i<s.length;i++){const a=s[i],c=a.innerText||"";if(c.includes("@")&&(c.includes("年")||c.includes("時間")||c.includes("分")||c.includes("日")))try{const h=await B(a);if(h&&h.author&&h.text){e=h;break}}catch(h){u.error(`代替引用ツイート抽出エラー: ${h.message}`);}}}catch(o){u.error(`代替引用ツイート検索エラー: ${o.message}`);}return e}async function B(n){const t=n.querySelector(g.quotedAuthor),e=t?t.textContent.trim():"",o=n.querySelector(g.quotedHandle),r=o?o.textContent.trim():"";let s="";const i=n.querySelector(g.tweetText);if(i)s=await j(i);else {const m=(n.innerText||"").split(`
`).map(T=>T.trim()).filter(T=>T);let d=-1;for(let T=0;T<m.length;T++)if(m[T].includes(r)){d=T;break}const w=d+1;d>=0&&w<m.length&&(s=m.slice(w+1).join(`
`));}const a=[];n.querySelectorAll(g.tweetPhoto).forEach(x=>{const m=x.querySelector(g.tweetMediaImage);if(m){const d=S(m.src);d&&!a.includes(d)&&a.push(d);}}),a.length===0&&(n.querySelectorAll(g.roleGroup).forEach(d=>{d.querySelectorAll(g.tweetMediaImage).forEach(T=>{const b=S(T.src);b&&!a.includes(b)&&a.push(b);});}),n.querySelectorAll(g.tweetMediaImage).forEach(d=>{const w=S(d.src);w&&!a.includes(w)&&a.push(w);}));let h="",p="";const f=n.querySelectorAll(g.statusLink);for(const x of Array.from(f)){const m=x.href;if(m&&m.includes("/status/")){const d=m.split("/"),w=d.indexOf("status");if(w!==-1&&w+1<d.length){h=d[w+1].split("?")[0],p=m;break}}}return e&&s?{author:e,handle:r,text:s,id:h,url:p,mediaUrls:a}:null}function bt(n){const t=[];return n.querySelectorAll(g.tweetPhoto).forEach(r=>{const s=r.querySelector(g.tweetMediaImage);if(s){const i=S(s.src);i&&!t.includes(i)&&t.push(i);}}),n.querySelectorAll("video").forEach(r=>{if(r.src&&r.src.startsWith("blob:"))try{if(r.poster&&r.poster.includes("pbs.twimg.com/tweet_video_thumb")){const a=F(r.poster);if(a&&!t.includes(a)){t.push(a);return}}const i=n.querySelector(g.statusLink);if(i){const a=i.href.split("/"),c=a.indexOf("status");if(c!==-1&&c+1<a.length){const h=a[c+1].split("?")[0],p=_(h);t.includes(p)||t.push(`[動画] ${p}`);}}}catch(i){u.error("Error processing blob URL: "+i);}else {if(r.poster&&r.poster.includes("pbs.twimg.com/tweet_video_thumb")){const i=F(r.poster);i&&!t.includes(i)&&t.push(i);}r.src&&r.src.includes("video.twimg.com")&&(t.includes(r.src)||t.push(r.src));}r.querySelectorAll("source").forEach(i=>{if(i.src&&i.src.includes("video.twimg.com"))t.includes(i.src)||t.push(i.src);else if(i.src&&i.src.startsWith("blob:")){const a=n.querySelector(g.statusLink);if(a){const c=a.href.split("/"),h=c.indexOf("status");if(h!==-1&&h+1<c.length){const p=c[h+1].split("?")[0],f=_(p);t.includes(f)||t.push(`[動画] ${f}`);}}}});}),t.length===0&&n.querySelectorAll(g.roleGroup).forEach(s=>{s.querySelectorAll(g.tweetMediaImage).forEach(a=>{const c=S(a.src);c&&!t.includes(c)&&t.push(c);});}),t.length===0&&n.querySelectorAll(g.tweetMediaImage).forEach(s=>{const i=S(s.src);i&&!t.includes(i)&&t.push(i);}),t}function S(n){if(!n||typeof n!="string"||!n.includes("pbs.twimg.com/media"))return null;try{const t=n.match(/format=([^&]+)/),e=t?t[1]:"jpg",o=n.split("?")[0];if(!o||o.length===0)return u.error(`無効なベースURL: ${n}`),null;const r=o+"."+e;try{return new URL(r),r}catch(s){return u.error(`無効なURL形式: ${r}, エラー内容: ${s.message}`),null}}catch(t){return u.error(`メディアURL処理エラー: ${t.message}`),null}}function F(n){if(!n||!n.includes("tweet_video_thumb"))return null;try{const t=n.match(/tweet_video_thumb\/([^.]+)/);if(!t||!t[1])return null;const e=t[1];return gt(e)}catch(t){return u.error(`動画URL生成エラー: ${t.message}`),null}}async function j(n){try{const t=n.cloneNode(!0);return await vt(t)}catch(t){return u.error(`ツイートテキスト取得エラー: ${t.message}`),n.innerText??""}}async function vt(n){const t=Array.from(n.querySelectorAll("a[href]")),e=[];for(const o of t)if(Ct(o,o.textContent??"")){const r=o.getAttribute("href");r&&r.startsWith(wt)&&e.push(St(r).then(s=>({anchor:o,resolvedUrl:s})));}if(e.length>0){const o=await Promise.all(e);for(const{anchor:r,resolvedUrl:s}of o)if(s){const i=document.createTextNode(s);r.replaceWith(i);}}return n.innerText}function St(n){return new Promise(t=>{dt({method:"HEAD",url:n,timeout:1e4,onload:e=>{t(e.finalUrl||n);},onerror:()=>{t(n);},ontimeout:()=>{t(n);}});})}function Ct(n,t){const e=t.trim();return /^https?:\/\//i.test(e)||n.hasAttribute("data-expanded-url")?true:/^https?:\/\//i.test(n.getAttribute("href")??"")?!(e.startsWith("@")||e.startsWith("#")):false}async function D(){try{const n=document.querySelectorAll(g.tweetButtonsWithinArticle);let t=0;for(const e of Array.from(n))try{const o=e.textContent?e.textContent.trim():"";(o==="さらに表示"||o==="Show more"||o.match(/もっと見る/i)||o.match(/show more/i))&&!e.hasAttribute("href")&&!e.querySelector("a")&&e.closest(g.tweetRoot)&&e.click&&(e.click(),t++,await new Promise(s=>setTimeout(s,100)));}catch(o){u.error(`個別ツイート展開エラー: ${o.message}`);continue}return t}catch(n){return u.error(`ツイート展開処理エラー: ${n.message}`),0}}function It(n){const t=n.getFullYear(),e=String(n.getMonth()+1).padStart(2,"0"),o=String(n.getDate()).padStart(2,"0"),r=String(n.getHours()).padStart(2,"0"),s=String(n.getMinutes()).padStart(2,"0");return `${t}年${e}月${o}日 ${r}:${s}`}const O=150,Lt=.9144,At=.453592,Et=3.78541,A="[0-9０-９]",$t=`${A}{1,3}(?:[,，]${A}{3})*`,Pt=`${A}+`,Mt=`(?:[\\.．]${A}+)?`,V=`(?:${$t}|${Pt})${Mt}`,G="(?:兆|億|万)",I=`${V}(?:${G}${V})*${G}?`,kt=new RegExp(`(?<prefix>約|およそ|ほぼ)?\\s*(?<amount>${I})\\s*(?<currencyPrefix>米|US|ＵＳ)?ドル`,"gu"),Rt=new RegExp(`(?<symbol>(?:US\\$|ＵＳ\\$|\\$))\\s*(?<amount>${I})(?!\\s*(?:米|US|ＵＳ)?ドル)`,"gu"),Ut=new RegExp(`(?<amount>${I})\\s*ヤード`,"gu"),qt=new RegExp(`(?<amount>${I})\\s*ポンド`,"gu"),_t=new RegExp(`(?<amount>${I})\\s*ガロン`,"gu");function Nt(n){if(!n||n.length===0)return n;let t=n;return t=C(t,kt,["円","JPY"],e=>z(e*O)),t=C(t,Rt,["円","JPY"],e=>z(e*O)),t=C(t,Ut,["メートル","m","ｍ"],e=>`${M(e*Lt)}メートル`),t=C(t,qt,["キログラム","kg","㎏","キロ"],e=>`${M(e*At)}キログラム`),t=C(t,_t,["リットル","l","L","ℓ"],e=>`${M(e*Et)}リットル`),t}function C(n,t,e,o){const r=[...n.matchAll(t)];if(r.length===0)return n;let s="",i=0;for(const a of r){const c=a.index??0,h=c+a[0].length,p=n.slice(i,c);s+=p;const f=a.groups??{},x=f.amount??"",m=Dt(x),d=Ht(n,h);if(m===null){s+=a[0],d?(s+=d.whitespace+d.openChar+d.inside+d.closeChar,i=d.endIndex):i=h;continue}if(d&&e.some(T=>d.inside.includes(T))){s+=a[0],s+=d.whitespace+d.openChar+d.inside+d.closeChar,i=d.endIndex;continue}const w=o(m,f);if(!w){s+=a[0],d?(s+=d.whitespace+d.openChar+d.inside+d.closeChar,i=d.endIndex):i=h;continue}if(d){const T=d.inside.trim(),b=T.length>0?`${w}、${T}`:w;s+=a[0]+d.whitespace+d.openChar+b+d.closeChar,i=d.endIndex;}else s+=`${a[0]}（${w}）`,i=h;}return s+=n.slice(i),s}function Ht(n,t){let e=t,o="";for(;e<n.length;){const c=n[e];if(Ft(c)){o+=c,e+=1;continue}break}if(e>=n.length)return null;const r=n[e];if(r!=="("&&r!=="（")return null;const s=r==="("?")":"）",i=Bt(n,e,r,s);if(i===-1)return null;const a=n.slice(e+1,i);return {whitespace:o,openChar:r,closeChar:s,inside:a,endIndex:i+1}}function Bt(n,t,e,o){let r=0;for(let s=t;s<n.length;s+=1){const i=n[s];if(i===e)r+=1;else if(i===o&&(r-=1,r===0))return s}return  -1}function Ft(n){return /\s/u.test(n)}function Dt(n){if(!n)return null;const t=n.replace(/[０-９]/gu,a=>String.fromCharCode(a.charCodeAt(0)-65248)).replace(/[，]/gu,",").replace(/[．]/gu,".").replace(/,/gu,"").replace(/\s+/gu,"");if(t.length===0)return null;const e=/(\d+(?:\.\d+)?)(兆|億|万)?/gu;let o=0,r=false,s=null,i;for(;(i=e.exec(t))!==null;){if(i[0].length===0)continue;const a=Number.parseFloat(i[1]);if(Number.isNaN(a))return null;r=true;const c=i[2];c?o+=a*Ot(c):s=a;}return r?(s!==null&&(o+=s),o):null}function Ot(n){switch(n){case "万":return 1e4;case "億":return 1e8;case "兆":return 1e12;default:return 1}}function z(n){const t=Math.abs(n),e=[{threshold:1e12,divisor:1e12,suffix:"兆円"},{threshold:1e8,divisor:1e8,suffix:"億円"},{threshold:1e4,divisor:1e4,suffix:"万円"}];for(const o of e)if(t>=o.threshold){const r=n/o.divisor;return `${E(r)}${o.suffix}`}return `${E(n)}円`}function M(n){return Math.abs(n)<1?E(n,3):E(n,2)}function E(n,t){const e=Math.abs(n);let o=t;o===void 0&&(e>=100?o=0:e>=10?o=1:e>=1?o=2:o=3);const r=Number.isFinite(n)?Number.parseFloat(n.toFixed(o)):0;return new Intl.NumberFormat("ja-JP",{maximumFractionDigits:o,minimumFractionDigits:0}).format(r)}const Vt="http://localhost:3002/v1/chat/completions",Gt=xt,zt="You are a highly skilled translation engine with expertise in the technology sector. Your function is to translate texts accurately into the target Japanese, maintaining the original format, technical terms, and abbreviations. Do not add any explanations or annotations to the translated text. Additionally, perform the following unit conversions within the text: 1. Convert US dollars to Japanese Yen (e.g., $100 -> 100ドル(15,000円)). Use the rate 1 USD = 150 JPY. 2. Convert imperial units to metric units: 1 yard = 0.9144 meters, 1 pound = 0.453592 kilograms, 1 US gallon = 3.78541 liters.",Wt=/(?:\u200B|\u200C|\u200D|\u2060|\uFEFF)/g;async function Yt(n){const t=n.map(te),e=t.map(i=>({tweet:i,textSegments:J(i.text),quotedSegments:i.quotedTweet?J(i.quotedTweet.text):null})),o=[];let r=false;for(const i of e){for(const a of i.textSegments)W(a,o);if(i.quotedSegments)for(const a of i.quotedSegments)W(a,o);}if(o.length===0)return {tweets:t,hasTranslation:false};let s=false;for(let i=0;i<o.length;i++){const a=o[i];try{const{text:h,provider:p}=await Qt(a.original),f=p==="none"?h:Nt(h);a.translated=f,p==="local"&&(r=!0),!s&&f!==a.original&&(s=!0);}catch(h){u.error(`セグメント翻訳に失敗: ${h.message}`),a.translated=a.original;}i<o.length-1&&await K(1e3+Math.random()*500);}for(const i of e)i.tweet.text=Y(i.textSegments),i.quotedSegments&&i.tweet.quotedTweet&&(i.tweet.quotedTweet.text=Y(i.quotedSegments));return r&&ut("ローカルAIでの翻訳が完了しました。","Twitter Thread Copier"),{tweets:t,hasTranslation:s}}function W(n,t){if(n.kind==="text"){if(n.original.trim().length===0){n.translated=n.original;return}t.push(n);}}function Y(n){if(n.length===0)return "";let t="";for(const e of n)if(e.kind==="fixed"){const o=e.value;Q(o)&&(t=jt(t)),t+=o;}else t+=e.translated??e.original;return t}function J(n){if(!n)return [];const t=/(https?:\/\/[^\s]+|@[A-Za-z0-9_]{1,15})/g,e=[];let o=0,r;for(;(r=t.exec(n))!==null;)r.index>o&&e.push({kind:"text",original:n.slice(o,r.index)}),e.push({kind:"fixed",value:Jt(r[0])}),o=r.index+r[0].length;return o<n.length&&e.push({kind:"text",original:n.slice(o)}),e.length===0&&e.push({kind:"text",original:n}),e}function Jt(n){if(Q(n)){const t=Xt(n);return t.length>0?t:n}return n}function Q(n){return /^https?:\/\//i.test(n.trim())}function Xt(n){return n.replace(Wt,"").replace(/\s+/g,"").replace(/\u2026+$/gu,"")}function jt(n){return n.length===0?n:n.replace(/：\s*$/u,": ").replace(/:\s*$/u,": ")}async function Qt(n){if(n.trim().length===0)return {text:n,provider:"none"};const t=await Kt(n);if(t)return {text:t,provider:"local"};try{return {text:await Zt(n),provider:"google"}}catch(e){return u.error(`Google翻訳にも失敗しました: ${e.message}`),{text:n,provider:"none"}}}async function Kt(n){try{const t=`<|plamo:op|>dataset
translation
<|plamo:op|>input lang=auto
${n}
<|plamo:op|>output lang=ja`,e=await new Promise((s,i)=>{GM_xmlhttpRequest({method:"POST",url:Vt,headers:{"Content-Type":"application/json"},data:JSON.stringify({model:"plamo-13b-instruct",messages:[{role:"system",content:zt},{role:"user",content:t}],temperature:0,max_tokens:4096,stream:!1}),timeout:12e4,onload:a=>a.status>=200&&a.status<300?s(a):i(new Error(`API error: ${a.status}`)),onerror:a=>i(a),ontimeout:()=>i(new Error("Timeout"))});}),r=JSON.parse(e.responseText)?.choices?.[0]?.message?.content;if(r&&r.trim().length>0)return u.log("ローカルAIでの翻訳に成功しました。"),r;throw new Error("ローカルAIからの翻訳結果が空です。")}catch(t){return u.error(`ローカルAIでの翻訳に失敗: ${t.message}`),null}}async function Zt(n){const t="auto";let o=0;const r=3;for(;o<r;)try{const s=`${Gt}?client=gtx&sl=${t}&tl=ja&dt=t&dt=bd&dj=1&q=${encodeURIComponent(n)}`,i=await new Promise((h,p)=>{GM_xmlhttpRequest({method:"GET",url:s,timeout:15e3,onload:f=>f.status>=200&&f.status<300?h(f):p(new Error(`API error: ${f.status}`)),onerror:f=>p(f),ontimeout:()=>p(new Error("Timeout"))});}),a=JSON.parse(i.responseText);if(!a?.sentences?.length)throw new Error("Invalid translation response format.");const c=a.sentences.map(h=>h?.trans??"").join("");if(!c.trim())throw new Error("Translation result is empty");return c}catch(s){if(o++,u.error(`Google翻訳試行 ${o}/${r} 失敗: ${s.message}`),o>=r)throw s;await K(1e3*Math.pow(2,o));}throw new Error("Google翻訳に失敗しました。")}function te(n){return {...n,mediaUrls:[...n.mediaUrls],quotedTweet:n.quotedTweet?{...n.quotedTweet,mediaUrls:[...n.quotedTweet.mediaUrls]}:null}}async function K(n){await new Promise(t=>setTimeout(t,n));}function $(n){let t=`${n.author} ${n.handle}
${n.text}
${n.time}
`;if(n.url&&(t+=`${n.url}
...
> `)}
`),t+=`> ${e.url}
`;}return t}function ee(n,t){if(!n||n.length===0)return "";let e="";const o=`

---

`;for(let r=0;r<n.length;r++){const s=n[r],i=$(s),a=r===0?i:o+i;if(e.length+a.length>t){const c=t-e.length;c>o.length&&(e+=a.substring(0,c-3)+"...");break}e+=a;}return e}const ne=/https?:\/\/[^\s]+/g;class oe{hasThreadUrlIncluded=false;remainingMediaSlots=5;consumeThreadUrl(t){return !t||this.hasThreadUrlIncluded?null:(this.hasThreadUrlIncluded=true,t)}consumeMediaUrls(t){if(this.remainingMediaSlots<=0)return [];const o=t.filter(r=>r.trim().length>0).slice(0,this.remainingMediaSlots);return this.remainingMediaSlots-=o.length,o}}function X(n){if(!n)return "";const e=n.replace(ne,"").replace(/[ \t]+\n/g,`
`).split(`
`).map(o=>o.replace(/[ \t]{2,}/g," ").trim());for(let o=e.length-1;o>=0&&e[o]==="";o--)e.pop();return e.join(`
`)}function re(n,t){const e=[];e.push(`${n.author} ${n.handle}`);const o=X(n.text);o.length>0?e.push(o):e.push(""),e.push(n.time);const r=t.consumeThreadUrl(n.url);r&&e.push(r);const s=t.consumeMediaUrls(n.mediaUrls);if(s.length>0&&e.push(...s),n.quotedTweet){const{quotedTweet:i}=n;if(i){const a=[];a.push(`> 引用元: ${i.author} ${i.handle}`);const c=X(i.text);c.length>0&&a.push(...c.split(`
`).map(p=>`> ${p}`));const h=t.consumeMediaUrls(i.mediaUrls);h.length>0&&a.push(...h.map(p=>`> ${p}`)),a.length>0&&(e.push(""),e.push(...a));}}return `${e.join(`
`)}
`}function ie(n,t){if(!n||n.length===0)return "";const e=new oe;let o="";const r=`

---

`;for(let s=0;s<n.length;s++){const i=n[s],a=re(i,e),c=s===0?a:r+a;if(o.length+c.length>t){const h=t-o.length;h>r.length&&(o+=c.substring(0,h-3)+"...");break}o+=c;}return o}function se(n){if(!n||n.length===0)return "";let t=$(n[0]);for(let e=1;e<n.length;e++)t+=`

---

`+$(n[e]);return t}function ae(n,t,e,o=null){let r="";if(o&&(r+=`${o}のツイートから`),n.length>0){const s=n[0].author;r+=`${s}のスレッド`;}return r+=`(${n.length}件)をコピーしました。`,r+=`文字数: ${le(t.length)}`,(e==="shitaraba"||e==="5ch")&&(r+=`/${e==="shitaraba"?4096:2048}`),r}function le(n){return n<1e3?n.toString():n<1e4?(n/1e3).toFixed(1)+"k":Math.round(n/1e3)+"k"}async function ce(n){if(!n||!n.formattedText){const o=n?"コピーするテキストがありません (formattedText is null)":"コピーするデータがありません (threadData is null)";return u.error(`クリップボードコピー失敗: ${o}`),y.showToast("エラー",o),false}if(n.formattedText.trim().length===0)return u.error("クリップボードコピー失敗: formattedTextが空です"),y.showToast("エラー","コピーするテキストが空です"),false;let t=false,e=null;if(navigator.clipboard&&window.ClipboardItem)try{const o=new Blob([n.formattedText],{type:"text/plain"}),r=new window.ClipboardItem({"text/plain":o});await navigator.clipboard.write([r]),t=!0;}catch(o){e=o,u.error(`ClipboardItem API失敗: ${e.message}`);}if(!t&&navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(n.formattedText),t=!0;}catch(o){e=o,u.error(`Navigator clipboard API失敗: ${e.message}`);}if(!t)try{const o=document.createElement("textarea");if(o.value=n.formattedText,o.style.position="fixed",o.style.left="-9999px",document.body.appendChild(o),o.select(),t=document.execCommand("copy"),document.body.removeChild(o),!t)throw new Error("execCommand returned false")}catch(o){e=o,u.error(`execCommand fallback失敗: ${e.message}`);}if(t)y.showToast("コピーしました",n.summary);else {const o=e?e.message:"不明なエラー";u.error(`クリップボードコピー失敗: ${o}`),y.showToast("エラー","クリップボードへのコピーに失敗しました。");}return t}class de{constructor(){this.init();}init(){try{this.observeUrlChanges(),this.updateButtonVisibility(),this.observeUrlChanges(),u.log("Application initialized.");}catch(t){u.error(`初期化中にエラーが発生: ${t.message}`);}}async handleButtonClick(t){try{if(t==="clipboard"){l.collectedThreadData&&(await ce(l.collectedThreadData),l.isSecondStage=!1,l.collectedThreadData=null,y.updateMainButtonText());return}if(l.isCollecting)return;l.isCollecting=!0,y.updateMainButtonText();try{const e=await Tt();let o=e;const r=l.selectedTweetIds??[];if(r.length>0){const h=new Map(e.map(f=>[f.id,f])),p=[];for(const f of r){const x=h.get(f);x&&p.push(x);}if(o=p,o.length===0){u.warn("選択済みツイートを取得できませんでした。"),l.collectedThreadData=null,l.isSecondStage=!1,y.showToast("選択エラー","選択したツイートが見つかりませんでした。再度読み込みしてください。");return}}else if(l.startFromTweetId){const h=e.findIndex(p=>p.id===l.startFromTweetId);h!==-1&&(o=e.slice(h));}let s="",i=o,a=!1;if(o.length>0){if(l.translateEnabled)try{l.translationInProgress=!0,y.updateMainButtonText(),y.showToast("翻訳中","翻訳処理を実行しています...");const h=await Yt(o);i=h.tweets,a=h.hasTranslation;}catch(h){u.error(`Translation error: ${h}`),y.showToast("翻訳エラー","翻訳中にエラーが発生しましたが、原文をコピーできます"),i=o,a=!1;}finally{l.translationInProgress=!1;}switch(l.copyMode){case "first":s=$(i[0]);break;case "shitaraba":s=ie(i,4096);break;case "5ch":s=ee(i,2048);break;default:s=se(i);break}}let c=ae(o,s,l.copyMode,r.length===0?l.startFromTweetAuthor:null);l.translateEnabled&&a&&s.trim().length>0&&(c+=" (翻訳済み)"),l.collectedThreadData={formattedText:s,summary:c},l.isSecondStage=!0,y.showToast("準備完了",`${c} クリックしてコピーしてください`);}catch(e){u.error(`Error in copy process: ${e}`),y.showToast("エラー","スレッドのコピーに失敗しました");}finally{l.isCollecting=!1,l.translationInProgress=!1,y.updateMainButtonText();}}catch(e){u.error(`Button click handler error: ${e}`),y.showToast("内部エラー","処理中に予期せぬエラーが発生しました。");}}updateButtonVisibility(){this.isTwitterStatusPage()?(y.init(),y.addMainButton(this.handleButtonClick.bind(this)),y.updateAllUI()):y.destroy();}isTwitterStatusPage(){return mt.test(location.href)}observeUrlChanges(){let t=location.href;const e=i=>{location.href!==t&&(t=location.href,setTimeout(()=>this.updateButtonVisibility(),300),console.log(`URL changed (${i}): ${t}`));},o=history.pushState;history.pushState=function(...i){o.apply(this,i),e("pushState");};const r=history.replaceState;history.replaceState=function(...i){r.apply(this,i),e("replaceState");},window.addEventListener("popstate",()=>e("popstate")),new MutationObserver(()=>e("mutation")).observe(document.body,{childList:true,subtree:true});}}new de;
