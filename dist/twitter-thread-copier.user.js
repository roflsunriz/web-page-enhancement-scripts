...
`));}const a=[];r.querySelectorAll(h.tweetPhoto).forEach(w=>{const m=w.querySelector(h.tweetMediaImage);if(m){const g=S(m.src);g&&!a.includes(g)&&a.push(g);}}),a.length===0&&(r.querySelectorAll(h.roleGroup).forEach(g=>{g.querySelectorAll(h.tweetMediaImage).forEach(b=>{const T=S(b.src);T&&!a.includes(T)&&a.push(T);});}),r.querySelectorAll(h.tweetMediaImage).forEach(g=>{const y=S(g.src);y&&!a.includes(y)&&a.push(y);}));let u="",f="";const p=r.querySelectorAll(h.statusLink);for(const w of Array.from(p)){const m=w.href;if(m&&m.includes("/status/")){const g=m.split("/"),y=g.indexOf("status");if(y!==-1&&y+1<g.length){u=g[y+1].split("?")[0],f=m;break}}}return e&&s?{author:e,handle:n,text:s,id:u,url:f,mediaUrls:a}:null}function nt(r){const t=[];return r.querySelectorAll(h.tweetPhoto).forEach(n=>{const s=n.querySelector(h.tweetMediaImage);if(s){const i=S(s.src);i&&!t.includes(i)&&t.push(i);}}),r.querySelectorAll("video").forEach(n=>{if(n.src&&n.src.startsWith("blob:"))try{if(n.poster&&n.poster.includes("pbs.twimg.com/tweet_video_thumb")){const a=H(n.poster);if(a&&!t.includes(a)){t.push(a);return}}const i=r.querySelector(h.statusLink);if(i){const a=i.href.split("/"),d=a.indexOf("status");if(d!==-1&&d+1<a.length){const f=`https://twitter.com/i/status/${a[d+1].split("?")[0]}`;t.includes(f)||t.push(`[動画] ${f}`);}}}catch(i){c.error("Error processing blob URL: "+i);}else {if(n.poster&&n.poster.includes("pbs.twimg.com/tweet_video_thumb")){const i=H(n.poster);i&&!t.includes(i)&&t.push(i);}n.src&&n.src.includes("video.twimg.com")&&(t.includes(n.src)||t.push(n.src));}n.querySelectorAll("source").forEach(i=>{if(i.src&&i.src.includes("video.twimg.com"))t.includes(i.src)||t.push(i.src);else if(i.src&&i.src.startsWith("blob:")){const a=r.querySelector(h.statusLink);if(a){const d=a.href.split("/"),u=d.indexOf("status");if(u!==-1&&u+1<d.length){const p=`https://twitter.com/i/status/${d[u+1].split("?")[0]}`;t.includes(p)||t.push(`[動画] ${p}`);}}}});}),t.length===0&&r.querySelectorAll(h.roleGroup).forEach(s=>{s.querySelectorAll(h.tweetMediaImage).forEach(a=>{const d=S(a.src);d&&!t.includes(d)&&t.push(d);});}),t.length===0&&r.querySelectorAll(h.tweetMediaImage).forEach(s=>{const i=S(s.src);i&&!t.includes(i)&&t.push(i);}),t}function S(r){if(!r||typeof r!="string"||!r.includes("pbs.twimg.com/media"))return null;try{const t=r.match(/format=([^&]+)/),e=t?t[1]:"jpg",o=r.split("?")[0];if(!o||o.length===0)return c.error(`無効なベースURL: ${r}`),null;const n=o+"."+e;try{return new URL(n),n}catch(s){return c.error(`無効なURL形式: ${n}, エラー内容: ${s.message}`),null}}catch(t){return c.error(`メディアURL処理エラー: ${t.message}`),null}}function H(r){if(!r||!r.includes("tweet_video_thumb"))return null;try{const t=r.match(/tweet_video_thumb\/([^.]+)/);return !t||!t[1]?null:`https://video.twimg.com/tweet_video/${t[1]}.mp4`}catch(t){return c.error(`動画URL生成エラー: ${t.message}`),null}}async function D(r){try{const t=r.cloneNode(!0);return await it(t)}catch(t){return c.error(`ツイートテキスト取得エラー: ${t.message}`),r.innerText??""}}async function it(r){const t=Array.from(r.querySelectorAll("a[href]")),e=[];for(const o of t)if(at(o,o.textContent??"")){const n=o.getAttribute("href");n&&n.startsWith("https://t.co/")&&e.push(st(n).then(s=>({anchor:o,resolvedUrl:s})));}if(e.length>0){const o=await Promise.all(e);for(const{anchor:n,resolvedUrl:s}of o)if(s){const i=document.createTextNode(s);n.replaceWith(i);}}return r.innerText}function st(r){return new Promise(t=>{tt({method:"HEAD",url:r,timeout:1e4,onload:e=>{t(e.finalUrl||r);},onerror:()=>{t(r);},ontimeout:()=>{t(r);}});})}function at(r,t){const e=t.trim();return /^https?:\/\//i.test(e)||r.hasAttribute("data-expanded-url")?true:/^https?:\/\//i.test(r.getAttribute("href")??"")?!(e.startsWith("@")||e.startsWith("#")):false}async function B(){try{const r=document.querySelectorAll(h.tweetButtonsWithinArticle);let t=0;for(const e of Array.from(r))try{const o=e.textContent?e.textContent.trim():"";(o==="さらに表示"||o==="Show more"||o.match(/もっと見る/i)||o.match(/show more/i))&&!e.hasAttribute("href")&&!e.querySelector("a")&&e.closest(h.tweetRoot)&&e.click&&(e.click(),t++,await new Promise(s=>setTimeout(s,100)));}catch(o){c.error(`個別ツイート展開エラー: ${o.message}`);continue}return t}catch(r){return c.error(`ツイート展開処理エラー: ${r.message}`),0}}function lt(r){const t=r.getFullYear(),e=String(r.getMonth()+1).padStart(2,"0"),o=String(r.getDate()).padStart(2,"0"),n=String(r.getHours()).padStart(2,"0"),s=String(r.getMinutes()).padStart(2,"0");return `${t}年${e}月${o}日 ${n}:${s}`}const ct="http://localhost:3002/v1/chat/completions",dt="https://translate.googleapis.com/translate_a/single",ut="You are a highly skilled translation engine with expertise in the technology sector. Your function is to translate texts accurately into the target Japanese, maintaining the original format, technical terms, and abbreviations. Do not add any explanations or annotations to the translated text.",ht=/(?:\u200B|\u200C|\u200D|\u2060|\uFEFF)/g;async function pt(r){const t=r.map(bt),e=t.map(i=>({tweet:i,textSegments:F(i.text),quotedSegments:i.quotedTweet?F(i.quotedTweet.text):null})),o=[];let n=false;for(const i of e){for(const a of i.textSegments)U(a,o);if(i.quotedSegments)for(const a of i.quotedSegments)U(a,o);}if(o.length===0)return {tweets:t,hasTranslation:false};let s=false;for(let i=0;i<o.length;i++){const a=o[i];try{const{text:u,provider:f}=await wt(a.original);a.translated=u,f==="local"&&(n=!0),!s&&u!==a.original&&(s=!0);}catch(u){c.error(`セグメント翻訳に失敗: ${u.message}`),a.translated=a.original;}i<o.length-1&&await O(1e3+Math.random()*500);}for(const i of e)i.tweet.text=R(i.textSegments),i.quotedSegments&&i.tweet.quotedTweet&&(i.tweet.quotedTweet.text=R(i.quotedSegments));return n&&et("ローカルAIでの翻訳が完了しました。","Twitter Thread Copier"),{tweets:t,hasTranslation:s}}function U(r,t){if(r.kind==="text"){if(r.original.trim().length===0){r.translated=r.original;return}t.push(r);}}function R(r){if(r.length===0)return "";let t="";for(const e of r)if(e.kind==="fixed"){const o=e.value;_(o)&&(t=mt(t)),t+=o;}else t+=e.translated??e.original;return t}function F(r){if(!r)return [];const t=/(https?:\/\/[^\s]+|@[A-Za-z0-9_]{1,15})/g,e=[];let o=0,n;for(;(n=t.exec(r))!==null;)n.index>o&&e.push({kind:"text",original:r.slice(o,n.index)}),e.push({kind:"fixed",value:ft(n[0])}),o=n.index+n[0].length;return o<r.length&&e.push({kind:"text",original:r.slice(o)}),e.length===0&&e.push({kind:"text",original:r}),e}function ft(r){if(_(r)){const t=gt(r);return t.length>0?t:r}return r}function _(r){return /^https?:\/\//i.test(r.trim())}function gt(r){return r.replace(ht,"").replace(/\s+/g,"").replace(/\u2026+$/gu,"")}function mt(r){return r.length===0?r:r.replace(/：\s*$/u,": ").replace(/:\s*$/u,": ")}async function wt(r){if(r.trim().length===0)return {text:r,provider:"none"};const t=await yt(r);if(t)return {text:t,provider:"local"};try{return {text:await xt(r),provider:"google"}}catch(e){return c.error(`Google翻訳にも失敗しました: ${e.message}`),{text:r,provider:"none"}}}async function yt(r){try{const t=`<|plamo:op|>dataset
translation
<|plamo:op|>input lang=auto
${r}
<|plamo:op|>output lang=ja`,e=await new Promise((s,i)=>{GM_xmlhttpRequest({method:"POST",url:ct,headers:{"Content-Type":"application/json"},data:JSON.stringify({model:"plamo-13b-instruct",messages:[{role:"system",content:ut},{role:"user",content:t}],temperature:0,max_tokens:4096,stream:!1}),timeout:12e4,onload:a=>a.status>=200&&a.status<300?s(a):i(new Error(`API error: ${a.status}`)),onerror:a=>i(a),ontimeout:()=>i(new Error("Timeout"))});}),n=JSON.parse(e.responseText)?.choices?.[0]?.message?.content;if(n&&n.trim().length>0)return c.log("ローカルAIでの翻訳に成功しました。"),n;throw new Error("ローカルAIからの翻訳結果が空です。")}catch(t){return c.error(`ローカルAIでの翻訳に失敗: ${t.message}`),null}}async function xt(r){const t="auto";let o=0;const n=3;for(;o<n;)try{const s=`${dt}?client=gtx&sl=${t}&tl=ja&dt=t&dt=bd&dj=1&q=${encodeURIComponent(r)}`,i=await new Promise((u,f)=>{GM_xmlhttpRequest({method:"GET",url:s,timeout:15e3,onload:p=>p.status>=200&&p.status<300?u(p):f(new Error(`API error: ${p.status}`)),onerror:p=>f(p),ontimeout:()=>f(new Error("Timeout"))});}),a=JSON.parse(i.responseText);if(!a?.sentences?.length)throw new Error("Invalid translation response format.");const d=a.sentences.map(u=>u?.trans??"").join("");if(!d.trim())throw new Error("Translation result is empty");return d}catch(s){if(o++,c.error(`Google翻訳試行 ${o}/${n} 失敗: ${s.message}`),o>=n)throw s;await O(1e3*Math.pow(2,o));}throw new Error("Google翻訳に失敗しました。")}function bt(r){return {...r,mediaUrls:[...r.mediaUrls],quotedTweet:r.quotedTweet?{...r.quotedTweet,mediaUrls:[...r.quotedTweet.mediaUrls]}:null}}async function O(r){await new Promise(t=>setTimeout(t,r));}function L(r){let t=`${r.author} ${r.handle}
${r.text}
${r.time}
`;if(r.url&&(t+=`${r.url}
...
> `)}
`),t+=`> ${e.url}
`;}return t}function N(r,t){if(!r||r.length===0)return "";let e="";const o=`

---

`;for(let n=0;n<r.length;n++){const s=r[n],i=L(s),a=n===0?i:o+i;if(e.length+a.length>t){const d=t-e.length;d>o.length&&(e+=a.substring(0,d-3)+"...");break}e+=a;}return e}function Tt(r){if(!r||r.length===0)return "";let t=L(r[0]);for(let e=1;e<r.length;e++)t+=`

---

`+L(r[e]);return t}function vt(r,t,e,o=null){let n="";if(o&&(n+=`${o}のツイートから`),r.length>0){const s=r[0].author;n+=`${s}のスレッド`;}return n+=`(${r.length}件)をコピーしました。`,n+=`文字数: ${St(t.length)}`,(e==="shitaraba"||e==="5ch")&&(n+=`/${e==="shitaraba"?4096:2048}`),n}function St(r){return r<1e3?r.toString():r<1e4?(r/1e3).toFixed(1)+"k":Math.round(r/1e3)+"k"}async function Ct(r){if(!r||!r.formattedText){const o=r?"コピーするテキストがありません (formattedText is null)":"コピーするデータがありません (threadData is null)";return c.error(`クリップボードコピー失敗: ${o}`),x.showToast("エラー",o),false}if(r.formattedText.trim().length===0)return c.error("クリップボードコピー失敗: formattedTextが空です"),x.showToast("エラー","コピーするテキストが空です"),false;let t=false,e=null;if(navigator.clipboard&&window.ClipboardItem)try{const o=new Blob([r.formattedText],{type:"text/plain"}),n=new window.ClipboardItem({"text/plain":o});await navigator.clipboard.write([n]),t=!0;}catch(o){e=o,c.error(`ClipboardItem API失敗: ${e.message}`);}if(!t&&navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(r.formattedText),t=!0;}catch(o){e=o,c.error(`Navigator clipboard API失敗: ${e.message}`);}if(!t)try{const o=document.createElement("textarea");if(o.value=r.formattedText,o.style.position="fixed",o.style.left="-9999px",document.body.appendChild(o),o.select(),t=document.execCommand("copy"),document.body.removeChild(o),!t)throw new Error("execCommand returned false")}catch(o){e=o,c.error(`execCommand fallback失敗: ${e.message}`);}if(t)x.showToast("コピーしました",r.summary);else {const o=e?e.message:"不明なエラー";c.error(`クリップボードコピー失敗: ${o}`),x.showToast("エラー","クリップボードへのコピーに失敗しました。");}return t}class Lt{constructor(){this.init();}init(){try{this.observeUrlChanges(),this.updateButtonVisibility(),this.observeUrlChanges(),c.log("Application initialized.");}catch(t){c.error(`初期化中にエラーが発生: ${t.message}`);}}async handleButtonClick(t){try{if(t==="clipboard"){l.collectedThreadData&&(await Ct(l.collectedThreadData),l.isSecondStage=!1,l.collectedThreadData=null,x.updateMainButtonText());return}if(l.isCollecting)return;l.isCollecting=!0,x.updateMainButtonText();try{const e=await ot();let o=e;const n=l.selectedTweetIds??[];if(n.length>0){const u=new Map(e.map(p=>[p.id,p])),f=[];for(const p of n){const w=u.get(p);w&&f.push(w);}if(o=f,o.length===0){c.warn("選択済みツイートを取得できませんでした。"),l.collectedThreadData=null,l.isSecondStage=!1,x.showToast("選択エラー","選択したツイートが見つかりませんでした。再度読み込みしてください。");return}}else if(l.startFromTweetId){const u=e.findIndex(f=>f.id===l.startFromTweetId);u!==-1&&(o=e.slice(u));}let s="",i=o,a=!1;if(o.length>0){if(l.translateEnabled)try{l.translationInProgress=!0,x.updateMainButtonText(),x.showToast("翻訳中","翻訳処理を実行しています...");const u=await pt(o);i=u.tweets,a=u.hasTranslation;}catch(u){c.error(`Translation error: ${u}`),x.showToast("翻訳エラー","翻訳中にエラーが発生しましたが、原文をコピーできます"),i=o,a=!1;}finally{l.translationInProgress=!1;}switch(l.copyMode){case "first":s=L(i[0]);break;case "shitaraba":s=N(i,4096);break;case "5ch":s=N(i,2048);break;default:s=Tt(i);break}}let d=vt(o,s,l.copyMode,n.length===0?l.startFromTweetAuthor:null);l.translateEnabled&&a&&s.trim().length>0&&(d+=" (翻訳済み)"),l.collectedThreadData={formattedText:s,summary:d},l.isSecondStage=!0,x.showToast("準備完了",`${d} クリックしてコピーしてください`);}catch(e){c.error(`Error in copy process: ${e}`),x.showToast("エラー","スレッドのコピーに失敗しました");}finally{l.isCollecting=!1,l.translationInProgress=!1,x.updateMainButtonText();}}catch(e){c.error(`Button click handler error: ${e}`),x.showToast("内部エラー","処理中に予期せぬエラーが発生しました。");}}updateButtonVisibility(){this.isTwitterStatusPage()?(x.init(),x.addMainButton(this.handleButtonClick.bind(this)),x.updateAllUI()):x.destroy();}isTwitterStatusPage(){return /https?:\/\/(twitter\.com|x\.com)\/[^/]+\/status\/[0-9]+/.test(location.href)}observeUrlChanges(){let t=location.href;const e=i=>{location.href!==t&&(t=location.href,setTimeout(()=>this.updateButtonVisibility(),300),console.log(`URL changed (${i}): ${t}`));},o=history.pushState;history.pushState=function(...i){o.apply(this,i),e("pushState");};const n=history.replaceState;history.replaceState=function(...i){n.apply(this,i),e("replaceState");},window.addEventListener("popstate",()=>e("popstate")),new MutationObserver(()=>e("mutation")).observe(document.body,{childList:true,subtree:true});}}new Lt;
