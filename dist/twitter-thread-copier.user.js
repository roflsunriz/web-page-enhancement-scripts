...
(function () {
    ...
    `,t.appendChild(e),!this.shadowRoot)return;this.shadowRoot.appendChild(t),this.settingsModal=t;const r=e.querySelector(".btn-save"),s=e.querySelector(".btn-cancel"),i=e.querySelector(".btn-reset");r&&r.addEventListener("click",()=>{const a={localAiEndpoint:e.querySelector("#local-ai-endpoint")?.value??o.localAiEndpoint,localAiSystemPrompt:e.querySelector("#local-ai-system-prompt")?.value??o.localAiSystemPrompt,openaiEndpoint:e.querySelector("#openai-endpoint")?.value??o.openaiEndpoint,openaiModel:e.querySelector("#openai-model")?.value??o.openaiModel,openaiSystemPrompt:e.querySelector("#openai-system-prompt")?.value??o.openaiSystemPrompt,openaiApiKey:e.querySelector("#openai-api-key")?.value??o.openaiApiKey};it(a),this.hideSettingsModal(),this.showToast("設定保存","設定を保存しました");}),s&&s.addEventListener("click",()=>{this.hideSettingsModal();}),i&&i.addEventListener("click",()=>{confirm("設定をデフォルトに戻しますか？")&&(vt(),this.hideSettingsModal(),this.showToast("設定リセット","設定をデフォルトに戻しました"));}),t.addEventListener("click",a=>{a.target===t&&this.hideSettingsModal();}),requestAnimationFrame(()=>{t.classList.add("visible");}),d.log("Settings modal opened");}hideSettingsModal(){this.settingsModal&&(this.settingsModal.classList.remove("visible"),setTimeout(()=>{this.settingsModal?.remove(),this.settingsModal=null;},300),d.log("Settings modal closed"));}escapeHtml(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}}const x=new St;var Ct=typeof GM_notification<"u"?GM_notification:void 0,At=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0;const Et=At;function Lt(n,t,e,o){Ct({text:n,title:t,image:e,onclick:o});}const It="https://twitter.com",$t="https://t.co",Mt="https://video.twimg.com",D=n=>`${It}/i/status/${n}`,Pt=n=>`${Mt}/tweet_video/${n}.mp4`,kt=/https?:\/\/(twitter\.com|x\.com)\/[^/]+\/status\/[0-9]+/,Rt=`${$t}/`,Ut="https://translate.googleapis.com/translate_a/single";async function qt(){try{const n=[],t=new Set;let e="",o="";const r=window.location.href,s=c.copyMode==="first";let i=!1;const a=window.scrollY;async function l(){if(i)return;const h=Array.from(document.querySelectorAll(f.article));h.length>0&&!e&&(e=z(h[0]),o=G(h[0]));for(const m of h){const w=m.querySelector(f.statusLink);if(!w)continue;const g=w.href.split("/"),p=g.indexOf("status");if(p===-1||p+1>=g.length)continue;const b=g[p+1].split("?")[0];if(t.has(b))continue;const y=z(m);if(e&&y!==e)continue;let T=G(m);y===e&&!T&&o&&(T=o);const L=m.querySelector(f.tweetText);let H="";L&&(H=await st(L));const O=m.querySelector("time"),F=O?O.getAttribute("datetime"):"",q=F?new Date(F):null,B=q?.getTime()??0,lt=q&&Number.isFinite(B)?Bt(q):"",ct=Nt(m),dt=await _t(m),ut=t.size===0?r:w.href?w.href.split("?")[0]:"";if(n.push({id:b,author:y,handle:T,text:H,time:lt,timestampMs:B,url:ut,mediaUrls:ct,quotedTweet:dt}),t.add(b),s){i=!0;break}}}async function u(){try{if(await Y(),await l(),i)return;let w=n.length,g=0;for(let p=0;p<30;p++)try{if(window.scrollBy(0,window.innerHeight*.7),await new Promise(b=>setTimeout(b,700)),await Y(),await l(),i)return;if(n.length===w){if(g++,g>=3)break}else w=n.length,g=0;}catch(b){d.error(`スクロール処理エラー (試行 ${p+1}): ${b.message}`);continue}try{window.scrollTo(0,a);}catch(p){d.error(`スクロール位置復元エラー: ${p.message}`);}}catch(w){throw d.error(`スクロール収集処理エラー: ${w.message}`),w}}return await u(),n.sort((h,m)=>h.timestampMs-m.timestampMs),n}catch(n){return d.error(`ツイート収集中にエラーが発生: ${n.message}`),d.error(`エラースタック: ${n.stack}`),[]}}function G(n){try{const e=Array.from(n.querySelectorAll("span")).find(s=>{try{const i=s.textContent?s.textContent.trim():"";return i.startsWith("@")&&!i.includes(" ")&&i.length>1}catch{return !1}});if(e)return e.textContent.trim();const o=Array.from(n.querySelectorAll(f.userLink));for(const s of o)try{const i=s.getAttribute("href");if(i&&!i.includes("/status/")&&i.length>1&&!i.includes("/i/"))return "@"+i.replace(/^\//,"")}catch{continue}const r=n.querySelector(f.userName);if(r){const s=r.querySelectorAll("span");for(const i of Array.from(s))try{const a=i.textContent?i.textContent.trim():"";if(a.startsWith("@")&&!a.includes(" "))return a}catch{continue}}return ""}catch(t){return d.error(`ユーザーハンドル取得エラー: ${t.message}`),""}}function z(n){try{const t=n.querySelector(f.userNameLinkSpan);return t&&t.textContent?t.textContent.trim():""}catch(t){return d.error(`ユーザー名取得エラー: ${t.message}`),""}}async function _t(n){const t=n.querySelector(f.quotedLink);let e=null;if(t)try{const o=t.closest(f.roleLink);o&&(e=await V(o));}catch(o){d.error(`引用ツイート取得エラー: ${o.message}`);}else try{const o=n.innerText||"",r=o.includes("引用")||o.includes("Quote")||o.includes("quote"),s=n.querySelectorAll(f.roleLink);if(s.length>0&&r)for(let i=0;i<s.length;i++){const a=s[i],l=a.innerText||"";if(l.includes("@")&&(l.includes("年")||l.includes("時間")||l.includes("分")||l.includes("日")))try{const u=await V(a);if(u&&u.author&&u.text){e=u;break}}catch(u){d.error(`代替引用ツイート抽出エラー: ${u.message}`);}}}catch(o){d.error(`代替引用ツイート検索エラー: ${o.message}`);}return e}async function V(n){const t=n.querySelector(f.quotedAuthor),e=t?t.textContent.trim():"",o=n.querySelector(f.quotedHandle),r=o?o.textContent.trim():"";let s="";const i=n.querySelector(f.tweetText);if(i)s=await st(i);else {const g=(n.innerText||"").split(`
`).map(y=>y.trim()).filter(y=>y);let p=-1;for(let y=0;y<g.length;y++)if(g[y].includes(r)){p=y;break}const b=p+1;p>=0&&b<g.length&&(s=g.slice(b+1).join(`
`));}const a=[];n.querySelectorAll(f.tweetPhoto).forEach(w=>{const g=w.querySelector(f.tweetMediaImage);if(g){const p=A(g.src);p&&!a.includes(p)&&a.push(p);}}),a.length===0&&(n.querySelectorAll(f.roleGroup).forEach(p=>{p.querySelectorAll(f.tweetMediaImage).forEach(y=>{const T=A(y.src);T&&!a.includes(T)&&a.push(T);});}),n.querySelectorAll(f.tweetMediaImage).forEach(p=>{const b=A(p.src);b&&!a.includes(b)&&a.push(b);}));let u="",h="";const m=n.querySelectorAll(f.statusLink);for(const w of Array.from(m)){const g=w.href;if(g&&g.includes("/status/")){const p=g.split("/"),b=p.indexOf("status");if(b!==-1&&b+1<p.length){u=p[b+1].split("?")[0],h=g;break}}}return e&&s?{author:e,handle:r,text:s,id:u,url:h,mediaUrls:a}:null}function Nt(n){const t=[];return n.querySelectorAll(f.tweetPhoto).forEach(r=>{const s=r.querySelector(f.tweetMediaImage);if(s){const i=A(s.src);i&&!t.includes(i)&&t.push(i);}}),n.querySelectorAll("video").forEach(r=>{if(r.src&&r.src.startsWith("blob:"))try{if(r.poster&&r.poster.includes("pbs.twimg.com/tweet_video_thumb")){const a=W(r.poster);if(a&&!t.includes(a)){t.push(a);return}}const i=n.querySelector(f.statusLink);if(i){const a=i.href.split("/"),l=a.indexOf("status");if(l!==-1&&l+1<a.length){const u=a[l+1].split("?")[0],h=D(u);t.includes(h)||t.push(`[動画] ${h}`);}}}catch(i){d.error("Error processing blob URL: "+i);}else {if(r.poster&&r.poster.includes("pbs.twimg.com/tweet_video_thumb")){const i=W(r.poster);i&&!t.includes(i)&&t.push(i);}r.src&&r.src.includes("video.twimg.com")&&(t.includes(r.src)||t.push(r.src));}r.querySelectorAll("source").forEach(i=>{if(i.src&&i.src.includes("video.twimg.com"))t.includes(i.src)||t.push(i.src);else if(i.src&&i.src.startsWith("blob:")){const a=n.querySelector(f.statusLink);if(a){const l=a.href.split("/"),u=l.indexOf("status");if(u!==-1&&u+1<l.length){const h=l[u+1].split("?")[0],m=D(h);t.includes(m)||t.push(`[動画] ${m}`);}}}});}),t.length===0&&n.querySelectorAll(f.roleGroup).forEach(s=>{s.querySelectorAll(f.tweetMediaImage).forEach(a=>{const l=A(a.src);l&&!t.includes(l)&&t.push(l);});}),t.length===0&&n.querySelectorAll(f.tweetMediaImage).forEach(s=>{const i=A(s.src);i&&!t.includes(i)&&t.push(i);}),t}function A(n){if(!n||typeof n!="string"||!n.includes("pbs.twimg.com/media"))return null;try{const t=n.match(/format=([^&]+)/),e=t?t[1]:"jpg",o=n.split("?")[0];if(!o||o.length===0)return d.error(`無効なベースURL: ${n}`),null;const r=o+"."+e;try{return new URL(r),r}catch(s){return d.error(`無効なURL形式: ${r}, エラー内容: ${s.message}`),null}}catch(t){return d.error(`メディアURL処理エラー: ${t.message}`),null}}function W(n){if(!n||!n.includes("tweet_video_thumb"))return null;try{const t=n.match(/tweet_video_thumb\/([^.]+)/);if(!t||!t[1])return null;const e=t[1];return Pt(e)}catch(t){return d.error(`動画URL生成エラー: ${t.message}`),null}}async function st(n){try{const t=n.cloneNode(!0);return await Ht(t)}catch(t){return d.error(`ツイートテキスト取得エラー: ${t.message}`),n.innerText??""}}async function Ht(n){const t=Array.from(n.querySelectorAll("a[href]")),e=[];for(const o of t){const r=o.textContent??"";if(Ft(o,r)){const s=r.trim();if(/^https?:\/\//i.test(s)){e.push(Promise.resolve({anchor:o,resolvedUrl:s}));continue}const i=o.href;i&&i.startsWith(Rt)&&e.push(Ot(i).then(a=>({anchor:o,resolvedUrl:a})));}}if(e.length>0){const o=await Promise.all(e);for(const{anchor:r,resolvedUrl:s}of o)if(s){const i=document.createTextNode(s);r.replaceWith(i);}}return n.innerText}function Ot(n){return new Promise(t=>{Et({method:"HEAD",url:n,timeout:1e4,onload:e=>{t(e.finalUrl||n);},onerror:()=>{t(n);},ontimeout:()=>{t(n);}});})}function Ft(n,t){const e=t.trim();return /^https?:\/\//i.test(e)?true:/^https?:\/\//i.test(n.getAttribute("href")??"")?!(e.startsWith("@")||e.startsWith("#")):false}async function Y(){try{const n=document.querySelectorAll(f.tweetButtonsWithinArticle);let t=0;for(const e of Array.from(n))try{const o=e.textContent?e.textContent.trim():"";(o==="さらに表示"||o==="Show more"||o.match(/もっと見る/i)||o.match(/show more/i))&&!e.hasAttribute("href")&&!e.querySelector("a")&&e.closest(f.tweetRoot)&&e.click&&(e.click(),t++,await new Promise(s=>setTimeout(s,100)));}catch(o){d.error(`個別ツイート展開エラー: ${o.message}`);continue}return t}catch(n){return d.error(`ツイート展開処理エラー: ${n.message}`),0}}function Bt(n,t=new Date){const e=n.getFullYear(),o=String(n.getMonth()+1).padStart(2,"0"),r=String(n.getDate()).padStart(2,"0"),s=String(n.getHours()).padStart(2,"0"),i=String(n.getMinutes()).padStart(2,"0");return `${e}年${o}月${r}日 ${s}:${i} (${Dt(n,t)})`}function Dt(n,t){const e=t.getTime()-n.getTime(),o=e>=0?"前":"後",r=Math.floor(Math.abs(e)/6e4);if(r<1)return e>=0?"たった今":"1分未満後";const s=r%60,i=Math.floor(r/60),a=i%24,l=Math.floor(i/24),u=l%30,h=Math.floor(l/30),m=h%12,w=Math.floor(h/12);return w>0?m>0?`${w}年${m}か月${o}`:`${w}年${o}`:h>0?u>0?`${h}か月${u}日${o}`:`${h}か月${o}`:l>0?a>0?`${l}日${a}時間${o}`:`${l}日${o}`:i>0?s>0?`${i}時間${s}分${o}`:`${i}時間${o}`:`${r}分${o}`}const K=150,Gt=.9144,zt=.453592,Vt=3.78541,k="[0-9０-９]",Wt=`${k}{1,3}(?:[,，]${k}{3})*`,Yt=`${k}+`,Kt=`(?:[\\.．]${k}+)?`,X=`(?:${Wt}|${Yt})${Kt}`,J="(?:兆|億|万)",S=`${X}(?:${J}${X})*${J}?`,Xt=new RegExp(`(?<prefix>約|およそ|ほぼ)?\\s*(?<amount>${S})\\s*(?<currencyPrefix>米|US|ＵＳ)?ドル`,"gu"),Jt=new RegExp(`(?<symbol>(?:US\\$|ＵＳ\\$|\\$))\\s*(?<amount>${S})(?!\\s*(?:米|US|ＵＳ)?ドル)`,"gu"),jt=new RegExp(`(?<amount>${S})\\s*ヤード`,"gu"),Qt=new RegExp(`(?<amount>${S})\\s*ポンド`,"gu"),Zt=new RegExp(`(?<amount>${S})\\s*ガロン`,"gu"),te=new RegExp(`(?<amount>${S})\\s*(?<unit>(?:[kKｋＫmMｍＭgGｇＧtTｔＴ]?[wWｗＷ]|(?:キロ|メガ|ギガ|テラ)ワット))(?![A-Za-zａ-ｚＡ-Ｚ])`,"gu"),ee=new RegExp(`(?<prefix>約|およそ|ほぼ)?\\s*(?<amount>${S})\\s*(?<unit>[kKｋＫmMｍＭgGｇＧtTｔＴbBｂＢ])(?!(?:[A-Za-zａ-ｚＡ-Ｚ]))`,"gu"),ne=new RegExp(`(?<prefix>約|およそ|ほぼ)?\\s*(?<amount>${S})\\s*(?<unit>ミリオン|ビリオン|トリリオン|キロ|メガ|ギガ|テラ|million|Million|billion|Billion|trillion|Trillion|kilo|Kilo|mega|Mega|giga|Giga|tera|Tera)(?!(?:[A-Za-zａ-ｚＡ-Ｚ]))`,"gu");function oe(n){if(!n||n.length===0)return n;let t=n;return t=v(t,Xt,["円","JPY"],e=>Q(e*K)),t=v(t,Jt,["円","JPY"],e=>Q(e*K)),t=v(t,jt,["メートル","m","ｍ"],e=>`${_(e*Gt)}メートル`),t=v(t,Qt,["キログラム","kg","㎏","キロ"],e=>`${_(e*zt)}キログラム`),t=v(t,Zt,["リットル","l","L","ℓ"],e=>`${_(e*Vt)}リットル`),t=v(t,te,["ワット","W","w","Ｗ","ｗ"],(e,o,r)=>{const s=o.unit??"";if(de(r))return null;const i=ce(s);if(i===null)return null;const a=e*i;return P(a,"ワット")}),t=v(t,ee,["万","億","兆"],(e,o,r)=>{const s=o.unit??"",i=Z(s);return i===null||j(s,r)?null:P(e*i)}),t=v(t,ne,["万","億","兆"],(e,o,r)=>{const s=o.unit??"",i=Z(s);return i===null||j(s,r)?null:P(e*i)}),t}function v(n,t,e,o){const r=[...n.matchAll(t)];if(r.length===0)return n;let s="",i=0;for(const a of r){const l=a.index??0,u=l+a[0].length,h=n.slice(i,l);s+=h;const m=a.groups??{},w=m.amount??"",g=ae(w),p=re(n,u);if(g===null){s+=a[0],p?(s+=p.whitespace+p.openChar+p.inside+p.closeChar,i=p.endIndex):i=u;continue}if(p&&e.some(T=>p.inside.includes(T))){s+=a[0],s+=p.whitespace+p.openChar+p.inside+p.closeChar,i=p.endIndex;continue}const y=o(g,m,{text:n,match:a,startIndex:l,endIndex:u,followingParentheses:p});if(y===null){s+=a[0],p?(s+=p.whitespace+p.openChar+p.inside+p.closeChar,i=p.endIndex):i=u;continue}if(p){const T=p.inside.trim(),L=T.length>0?`${y}、${T}`:y;s+=a[0]+p.whitespace+p.openChar+L+p.closeChar,i=p.endIndex;}else s+=`${a[0]}（${y}）`,i=u;}return s+=n.slice(i),s}function re(n,t){let e=t,o="";for(;e<n.length;){const l=n[e];if(se(l)){o+=l,e+=1;continue}break}if(e>=n.length)return null;const r=n[e];if(r!=="("&&r!=="（")return null;const s=r==="("?")":"）",i=ie(n,e,r,s);if(i===-1)return null;const a=n.slice(e+1,i);return {whitespace:o,openChar:r,closeChar:s,inside:a,endIndex:i+1}}function ie(n,t,e,o){let r=0;for(let s=t;s<n.length;s+=1){const i=n[s];if(i===e)r+=1;else if(i===o&&(r-=1,r===0))return s}return  -1}function se(n){return /\s/u.test(n)}function j(n,t){if(!ue(n))return  false;const e=t.text.slice(t.endIndex,t.endIndex+6);return /^(?:バイト|ビット|ヘルツ|メートル|リットル|グラム|ワット|ジュール|パスカル|ボルト|アンペア)/u.test(e)}function ae(n){if(!n)return null;const t=n.replace(/[０-９]/gu,a=>String.fromCharCode(a.charCodeAt(0)-65248)).replace(/[，]/gu,",").replace(/[．]/gu,".").replace(/,/gu,"").replace(/\s+/gu,"");if(t.length===0)return null;const e=/(\d+(?:\.\d+)?)(兆|億|万)?/gu;let o=0,r=false,s=null,i;for(;(i=e.exec(t))!==null;){if(i[0].length===0)continue;const a=Number.parseFloat(i[1]);if(Number.isNaN(a))return null;r=true;const l=i[2];l?o+=a*le(l):s=a;}return r?(s!==null&&(o+=s),o):null}function le(n){switch(n){case "万":return 1e4;case "億":return 1e8;case "兆":return 1e12;default:return 1}}function P(n,t=""){const e=Math.abs(n),o=[{threshold:1e12,divisor:1e12,suffix:`兆${t}`},{threshold:1e8,divisor:1e8,suffix:`億${t}`},{threshold:1e4,divisor:1e4,suffix:`万${t}`}];for(const r of o)if(e>=r.threshold){const s=n/r.divisor;return `${E(s)}${r.suffix}`}return t.length>0?`${E(n)}${t}`:E(n)}function Q(n){return P(n,"円")}function _(n){return Math.abs(n)<1?E(n,3):E(n,2)}function E(n,t){const e=Math.abs(n);let o=t;o===void 0&&(e>=100?o=0:e>=10?o=1:e>=1?o=2:o=3);const r=Number.isFinite(n)?Number.parseFloat(n.toFixed(o)):0;return new Intl.NumberFormat("ja-JP",{maximumFractionDigits:o,minimumFractionDigits:0}).format(r)}function ce(n){if(!n)return null;const t=n.replace(/[Ａ-Ｚａ-ｚ]/gu,r=>String.fromCharCode(r.charCodeAt(0)-65248)).replace(/\s+/gu,"");if(t.length===0)return null;const e=t.toUpperCase(),o=t[0];if(e==="KW"&&(o==="K"||o==="k"))return 1e3;if(e==="MW"&&o==="M")return 1e6;if(e==="GW"&&o==="G")return 1e9;if(e==="TW"&&o==="T")return 1e12;if(e==="W"&&(o==="W"||o==="w"))return 1;switch(t){case "ワット":return 1;case "キロワット":return 1e3;case "メガワット":return 1e6;case "ギガワット":return 1e9;case "テラワット":return 1e12;default:return null}}function de(n){return n.text.slice(n.endIndex,n.endIndex+1)==="時"}function Z(n){switch(n.replace(/[Ａ-Ｚａ-ｚ]/gu,e=>String.fromCharCode(e.charCodeAt(0)-65248)).trim().toLowerCase()){case "k":case "kilo":case "キロ":return 1e3;case "m":case "mega":case "million":case "ミリオン":case "メガ":return 1e6;case "b":case "billion":case "g":case "giga":case "ビリオン":case "ギガ":return 1e9;case "t":case "tera":case "trillion":case "テラ":case "トリリオン":return 1e12;default:return null}}function ue(n){const t=n.replace(/[Ａ-Ｚａ-ｚ]/gu,e=>String.fromCharCode(e.charCodeAt(0)-65248)).trim().toLowerCase();return t==="k"||t==="kilo"||t==="キロ"||t==="m"||t==="mega"||t==="メガ"||t==="g"||t==="giga"||t==="ギガ"||t==="t"||t==="tera"||t==="テラ"}const pe=Ut,he=/(?:\u200B|\u200C|\u200D|\u2060|\uFEFF)/g;function me(){const n=localStorage.getItem("translationProvider");return n==="local"||n==="google"||n==="openai"?n:"local"}async function fe(n){const t=n.map(Ce),e=t.map(i=>({tweet:i,textSegments:nt(i.text),quotedSegments:i.quotedTweet?nt(i.quotedTweet.text):null})),o=[];let r=false;for(const i of e){for(const a of i.textSegments)tt(a,o);if(i.quotedSegments)for(const a of i.quotedSegments)tt(a,o);}if(o.length===0)return {tweets:t,hasTranslation:false};let s=false;for(let i=0;i<o.length;i++){const a=o[i];try{const{text:u,provider:h}=await ye(a.original),m=h==="none"?u:oe(u);a.translated=m,h==="local"&&(r=!0),!s&&m!==a.original&&(s=!0);}catch(u){d.error(`セグメント翻訳に失敗: ${u.message}`),a.translated=a.original;}i<o.length-1&&await N(1e3+Math.random()*500);}for(const i of e)i.tweet.text=et(i.textSegments),i.quotedSegments&&i.tweet.quotedTweet&&(i.tweet.quotedTweet.text=et(i.quotedSegments));return r&&Lt("ローカルAIでの翻訳が完了しました。","Twitter Thread Copier"),{tweets:t,hasTranslation:s}}function tt(n,t){if(n.kind==="text"){if(n.original.trim().length===0){n.translated=n.original;return}t.push(n);}}function et(n){if(n.length===0)return "";let t="";for(const e of n)if(e.kind==="fixed"){const o=e.value;at(o)&&(t=be(t)),t+=o;}else t+=e.translated??e.original;return t}function nt(n){if(!n)return [];const t=/(https?:\/\/[^\s]+|@[A-Za-z0-9_]{1,15})/g,e=[];let o=0,r;for(;(r=t.exec(n))!==null;)r.index>o&&e.push({kind:"text",original:n.slice(o,r.index)}),e.push({kind:"fixed",value:ge(r[0])}),o=r.index+r[0].length;return o<n.length&&e.push({kind:"text",original:n.slice(o)}),e.length===0&&e.push({kind:"text",original:n}),e}function ge(n){if(at(n)){const t=we(n);return t.length>0?t:n}return n}function at(n){return /^https?:\/\//i.test(n.trim())}function we(n){return n.replace(he,"").replace(/\s+/g,"").replace(/\u2026+$/gu,"")}function be(n){return n.length===0?n:n.replace(/：\s*$/u,": ").replace(/:\s*$/u,": ")}async function ye(n){if(n.trim().length===0)return {text:n,provider:"none"};const t=me();if(t==="local"){const e=await xe(n);return e?{text:e,provider:"local"}:{text:n,provider:"none"}}if(t==="google")try{return {text:await Te(n),provider:"google"}}catch(e){return d.error(`Google翻訳にも失敗しました: ${e.message}`),{text:n,provider:"none"}}if(t==="openai"){ve();const e=await Se(n);return e?{text:e,provider:"openai"}:{text:n,provider:"none"}}return {text:n,provider:"none"}}async function xe(n){try{const t=U(),e=`<|plamo:op|>dataset
translation
<|plamo:op|>input lang=auto
${n}
<|plamo:op|>output lang=ja`,o=await new Promise((i,a)=>{GM_xmlhttpRequest({method:"POST",url:t.localAiEndpoint,headers:{"Content-Type":"application/json"},data:JSON.stringify({model:"plamo-13b-instruct",messages:[{role:"system",content:t.localAiSystemPrompt},{role:"user",content:e}],temperature:0,max_tokens:8192,stream:!1}),timeout:12e4,onload:l=>l.status>=200&&l.status<300?i(l):a(new Error(`API error: ${l.status}`)),onerror:l=>a(l),ontimeout:()=>a(new Error("Timeout"))});}),s=JSON.parse(o.responseText)?.choices?.[0]?.message?.content;if(s&&s.trim().length>0)return d.log("ローカルAIでの翻訳に成功しました。"),s;throw new Error("ローカルAIからの翻訳結果が空です。")}catch(t){return d.error(`ローカルAIでの翻訳に失敗: ${t.message}`),null}}async function Te(n){const t="auto";let o=0;const r=3;for(;o<r;)try{const s=`${pe}?client=gtx&sl=${t}&tl=ja&dt=t&dt=bd&dj=1&q=${encodeURIComponent(n)}`,i=await new Promise((u,h)=>{GM_xmlhttpRequest({method:"GET",url:s,timeout:15e3,onload:m=>m.status>=200&&m.status<300?u(m):h(new Error(`API error: ${m.status}`)),onerror:m=>h(m),ontimeout:()=>h(new Error("Timeout"))});}),a=JSON.parse(i.responseText);if(!a?.sentences?.length)throw new Error("Invalid translation response format.");const l=a.sentences.map(u=>u?.trans??"").join("");if(!l.trim())throw new Error("Translation result is empty");return l}catch(s){if(o++,d.error(`Google翻訳試行 ${o}/${r} 失敗: ${s.message}`),o>=r)throw s;await N(1e3*Math.pow(2,o));}throw new Error("Google翻訳に失敗しました。")}function ve(){U().openaiApiKey||d.warn("OpenAI互換 APIキーが設定されていません。設定画面から設定してください。");}async function Se(n){const t=U();if(!t.openaiEndpoint)return d.error("OpenAI互換 APIエンドポイントが設定されていません。"),null;let e=0;const o=3,r=2e3;for(;e<o;)try{const s={"Content-Type":"application/json"};t.openaiApiKey&&(s.Authorization=`Bearer ${t.openaiApiKey}`),t.openaiEndpoint.includes("openrouter.ai")&&(s["HTTP-Referer"]=window.location.href,s["X-Title"]="Twitter Thread Copier");const a=`以下の英文を日本語に翻訳してください。翻訳結果のみを出力し、説明や注釈は不要です。

${n}`,l=await new Promise((m,w)=>{GM_xmlhttpRequest({method:"POST",url:t.openaiEndpoint,headers:s,data:JSON.stringify({model:t.openaiModel,messages:[{role:"system",content:t.openaiSystemPrompt},{role:"user",content:a}],temperature:0,max_tokens:8192}),timeout:6e4,onload:g=>g.status>=200&&g.status<300?m(g):w(new Error(`API error: ${g.status} - ${g.responseText}`)),onerror:g=>w(g),ontimeout:()=>w(new Error("Timeout"))});}),h=JSON.parse(l.responseText)?.choices?.[0]?.message?.content;if(h&&h.trim().length>0)return d.log("OpenAI互換での翻訳に成功しました。"),h;throw new Error("OpenAI Compatible translation result is empty")}catch(s){const i=s.message,a=i.includes("429");if(e++,d.error(`OpenAI互換翻訳試行 ${e}/${o} 失敗: ${i}`),e>=o)return d.error("OpenAI互換翻訳に失敗（最大リトライ回数到達）"),null;const l=a?r*Math.pow(2,e):r*e;d.log(`${l}ms後にリトライします...`),await N(l);}return null}function Ce(n){return {...n,mediaUrls:[...n.mediaUrls],quotedTweet:n.quotedTweet?{...n.quotedTweet,mediaUrls:[...n.quotedTweet.mediaUrls]}:null}}async function N(n){await new Promise(t=>setTimeout(t,n));}function R(n){let t=`${n.author} ${n.handle}
${n.text}
${n.time}
`;if(n.url&&(t+=`${n.url}
...
`;for(let r=0;r<n.length;r++){const s=n[r],i=R(s),a=r===0?i:o+i;if(e.length+a.length>t){const l=t-e.length;l>o.length&&(e+=a.substring(0,l-3)+"...");break}e+=a;}return e}const Ee=/https?:\/\/[^\s]+/g;class Le{hasThreadUrlIncluded=false;remainingMediaSlots=5;consumeThreadUrl(t){return !t||this.hasThreadUrlIncluded?null:(this.hasThreadUrlIncluded=true,t)}consumeMediaUrls(t){if(this.remainingMediaSlots<=0)return [];const o=t.filter(r=>r.trim().length>0).slice(0,this.remainingMediaSlots);return this.remainingMediaSlots-=o.length,o}}function ot(n){if(!n)return "";const e=n.replace(Ee,"").replace(/[ \t]+\n/g,`
`).split(`
`).map(o=>o.replace(/[ \t]{2,}/g," ").trim());for(let o=e.length-1;o>=0&&e[o]==="";o--)e.pop();return e.join(`
`)}function Ie(n,t){const e=[];e.push(`${n.author} ${n.handle}`);const o=ot(n.text);o.length>0?e.push(o):e.push(""),e.push(n.time);const r=t.consumeThreadUrl(n.url);r&&e.push(r);const s=t.consumeMediaUrls(n.mediaUrls);if(s.length>0&&e.push(...s),n.quotedTweet){const{quotedTweet:i}=n;if(i){const a=[];a.push(`> 引用元: ${i.author} ${i.handle}`);const l=ot(i.text);l.length>0&&a.push(...l.split(`
`).map(h=>`> ${h}`));const u=t.consumeMediaUrls(i.mediaUrls);u.length>0&&a.push(...u.map(h=>`> ${h}`)),a.length>0&&(e.push(""),e.push(...a));}}return `${e.join(`
`)}
`}function $e(n,t){if(!n||n.length===0)return "";const e=new Le;let o="";const r=`

---

`;for(let s=0;s<n.length;s++){const i=n[s],a=Ie(i,e),l=s===0?a:r+a;if(o.length+l.length>t){const u=t-o.length;u>r.length&&(o+=l.substring(0,u-3)+"...");break}o+=l;}return o}function Me(n){if(!n||n.length===0)return "";let t=R(n[0]);for(let e=1;e<n.length;e++)t+=`

---

`+R(n[e]);return t}function Pe(n,t,e,o=null){let r="";if(o&&(r+=`${o}のツイートから`),n.length>0){const s=n[0].author;r+=`${s}のスレッド`;}return r+=`(${n.length}件)をコピーしました。`,r+=`文字数: ${ke(t.length)}`,(e==="shitaraba"||e==="5ch")&&(r+=`/${e==="shitaraba"?4096:2048}`),r}function ke(n){return n<1e3?n.toString():n<1e4?(n/1e3).toFixed(1)+"k":Math.round(n/1e3)+"k"}async function Re(n){if(!n||!n.formattedText){const o=n?"コピーするテキストがありません (formattedText is null)":"コピーするデータがありません (threadData is null)";return d.error(`クリップボードコピー失敗: ${o}`),x.showToast("エラー",o),false}if(n.formattedText.trim().length===0)return d.error("クリップボードコピー失敗: formattedTextが空です"),x.showToast("エラー","コピーするテキストが空です"),false;let t=false,e=null;if(navigator.clipboard&&window.ClipboardItem)try{const o=new Blob([n.formattedText],{type:"text/plain"}),r=new window.ClipboardItem({"text/plain":o});await navigator.clipboard.write([r]),t=!0;}catch(o){e=o,d.error(`ClipboardItem API失敗: ${e.message}`);}if(!t&&navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(n.formattedText),t=!0;}catch(o){e=o,d.error(`Navigator clipboard API失敗: ${e.message}`);}if(!t)try{const o=document.createElement("textarea");if(o.value=n.formattedText,o.style.position="fixed",o.style.left="-9999px",document.body.appendChild(o),o.select(),t=document.execCommand("copy"),document.body.removeChild(o),!t)throw new Error("execCommand returned false")}catch(o){e=o,d.error(`execCommand fallback失敗: ${e.message}`);}if(t)x.showToast("コピーしました",n.summary);else {const o=e?e.message:"不明なエラー";d.error(`クリップボードコピー失敗: ${o}`),x.showToast("エラー","クリップボードへのコピーに失敗しました。");}return t}class Ue{constructor(){this.init();}init(){try{this.observeUrlChanges(),this.updateButtonVisibility(),this.observeUrlChanges(),d.log("Application initialized.");}catch(t){d.error(`初期化中にエラーが発生: ${t.message}`);}}async handleButtonClick(t){try{if(t==="clipboard"){c.collectedThreadData&&(await Re(c.collectedThreadData),c.isSecondStage=!1,c.collectedThreadData=null,x.updateMainButtonText());return}if(c.isCollecting)return;c.isCollecting=!0,x.updateMainButtonText();try{const e=await qt();let o=e;const r=c.selectedTweetIds??[];if(r.length>0){const u=new Map(e.map(m=>[m.id,m])),h=[];for(const m of r){const w=u.get(m);w&&h.push(w);}if(o=h,o.length===0){d.warn("選択済みツイートを取得できませんでした。"),c.collectedThreadData=null,c.isSecondStage=!1,x.showToast("選択エラー","選択したツイートが見つかりませんでした。再度読み込みしてください。");return}}else if(c.startFromTweetId){const u=e.findIndex(h=>h.id===c.startFromTweetId);u!==-1&&(o=e.slice(u));}let s="",i=o,a=!1;if(o.length>0){if(c.translateEnabled)try{c.translationInProgress=!0,x.updateMainButtonText(),x.showToast("翻訳中","翻訳処理を実行しています...");const u=await fe(o);i=u.tweets,a=u.hasTranslation;}catch(u){d.error(`Translation error: ${u}`),x.showToast("翻訳エラー","翻訳中にエラーが発生しましたが、原文をコピーできます"),i=o,a=!1;}finally{c.translationInProgress=!1;}switch(c.copyMode){case "first":s=R(i[0]);break;case "shitaraba":s=$e(i,4096);break;case "5ch":s=Ae(i,2048);break;default:s=Me(i);break}}let l=Pe(o,s,c.copyMode,r.length===0?c.startFromTweetAuthor:null);c.translateEnabled&&a&&s.trim().length>0&&(l+=" (翻訳済み)"),c.collectedThreadData={formattedText:s,summary:l},c.isSecondStage=!0,x.showToast("準備完了",`${l} クリックしてコピーしてください`);}catch(e){d.error(`Error in copy process: ${e}`),x.showToast("エラー","スレッドのコピーに失敗しました");}finally{c.isCollecting=!1,c.translationInProgress=!1,x.updateMainButtonText();}}catch(e){d.error(`Button click handler error: ${e}`),x.showToast("内部エラー","処理中に予期せぬエラーが発生しました。");}}updateButtonVisibility(){this.isTwitterStatusPage()?(x.init(),x.addMainButton(this.handleButtonClick.bind(this)),x.updateAllUI()):x.destroy();}isTwitterStatusPage(){return kt.test(location.href)}observeUrlChanges(){let t=location.href;const e=()=>{location.href!==t&&(t=location.href,setTimeout(()=>this.updateButtonVisibility(),300));},o=history.pushState;history.pushState=function(...i){o.apply(this,i),e();};const r=history.replaceState;history.replaceState=function(...i){r.apply(this,i),e();},window.addEventListener("popstate",()=>e()),new MutationObserver(()=>e()).observe(document.body,{childList:true,subtree:true});}}new Ue;
