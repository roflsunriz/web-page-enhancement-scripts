...
> `)}
`),t+=`> ${e.url}
`;}return t}function de(n,t){if(!n||n.length===0)return "";let e="";const o=`

---

`;for(let r=0;r<n.length;r++){const s=n[r],i=P(s),a=r===0?i:o+i;if(e.length+a.length>t){const c=t-e.length;c>o.length&&(e+=a.substring(0,c-3)+"...");break}e+=a;}return e}const ue=/https?:\/\/[^\s]+/g;class he{hasThreadUrlIncluded=false;remainingMediaSlots=5;consumeThreadUrl(t){return !t||this.hasThreadUrlIncluded?null:(this.hasThreadUrlIncluded=true,t)}consumeMediaUrls(t){if(this.remainingMediaSlots<=0)return [];const o=t.filter(r=>r.trim().length>0).slice(0,this.remainingMediaSlots);return this.remainingMediaSlots-=o.length,o}}function Q(n){if(!n)return "";const e=n.replace(ue,"").replace(/[ \t]+\n/g,`
`).split(`
