...
`+R(n[e]);return t}function Ie(n,t,e,o=null){let r="";if(o&&(r+=`${o}のツイートから`),n.length>0){const s=n[0].author;r+=`${s}のスレッド`;}return r+=`(${n.length}件)をコピーしました。`,r+=`文字数: ${Pe(t.length)}`,(e==="shitaraba"||e==="5ch")&&(r+=`/${e==="shitaraba"?4096:2048}`),r}function Pe(n){return n<1e3?n.toString():n<1e4?(n/1e3).toFixed(1)+"k":Math.round(n/1e3)+"k"}async function Me(n){if(!n||!n.formattedText){const o=n?"コピーするテキストがありません (formattedText is null)":"コピーするデータがありません (threadData is null)";return d.error(`クリップボードコピー失敗: ${o}`),y.showToast("エラー",o),false}if(n.formattedText.trim().length===0)return d.error("クリップボードコピー失敗: formattedTextが空です"),y.showToast("エラー","コピーするテキストが空です"),false;let t=false,e=null;if(navigator.clipboard&&window.ClipboardItem)try{const o=new Blob([n.formattedText],{type:"text/plain"}),r=new window.ClipboardItem({"text/plain":o});await navigator.clipboard.write([r]),t=!0;}catch(o){e=o,d.error(`ClipboardItem API失敗: ${e.message}`);}if(!t&&navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(n.formattedText),t=!0;}catch(o){e=o,d.error(`Navigator clipboard API失敗: ${e.message}`);}if(!t)try{const o=document.createElement("textarea");if(o.value=n.formattedText,o.style.position="fixed",o.style.left="-9999px",document.body.appendChild(o),o.select(),t=document.execCommand("copy"),document.body.removeChild(o),!t)throw new Error("execCommand returned false")}catch(o){e=o,d.error(`execCommand fallback失敗: ${e.message}`);}if(t)y.showToast("コピーしました",n.summary);else {const o=e?e.message:"不明なエラー";d.error(`クリップボードコピー失敗: ${o}`),y.showToast("エラー","クリップボードへのコピーに失敗しました。");}return t}class $e{constructor(){this.init();}init(){try{this.observeUrlChanges(),this.updateButtonVisibility(),this.observeUrlChanges(),d.log("Application initialized.");}catch(t){d.error(`初期化中にエラーが発生: ${t.message}`);}}async handleButtonClick(t){try{if(t==="clipboard"){c.collectedThreadData&&(await Me(c.collectedThreadData),c.isSecondStage=!1,c.collectedThreadData=null,y.updateMainButtonText());return}if(c.isCollecting)return;c.isCollecting=!0,y.updateMainButtonText();try{const e=await Rt();let o=e;const r=c.selectedTweetIds??[];if(r.length>0){const u=new Map(e.map(m=>[m.id,m])),h=[];for(const m of r){const w=u.get(m);w&&h.push(w);}if(o=h,o.length===0){d.warn("選択済みツイートを取得できませんでした。"),c.collectedThreadData=null,c.isSecondStage=!1,y.showToast("選択エラー","選択したツイートが見つかりませんでした。再度読み込みしてください。");return}}else if(c.startFromTweetId){const u=e.findIndex(h=>h.id===c.startFromTweetId);u!==-1&&(o=e.slice(u));}let s="",i=o,a=!1;if(o.length>0){if(c.translateEnabled)try{c.translationInProgress=!0,y.updateMainButtonText(),y.showToast("翻訳中","翻訳処理を実行しています...");const u=await pe(o);i=u.tweets,a=u.hasTranslation;}catch(u){d.error(`Translation error: ${u}`),y.showToast("翻訳エラー","翻訳中にエラーが発生しましたが、原文をコピーできます"),i=o,a=!1;}finally{c.translationInProgress=!1;}switch(c.copyMode){case "first":s=R(i[0]);break;case "shitaraba":s=Ee(i,4096);break;case "5ch":s=ve(i,2048);break;default:s=Le(i);break}}let l=Ie(o,s,c.copyMode,r.length===0?c.startFromTweetAuthor:null);c.translateEnabled&&a&&s.trim().length>0&&(l+=" (翻訳済み)"),c.collectedThreadData={formattedText:s,summary:l},c.isSecondStage=!0,y.showToast("準備完了",`${l} クリックしてコピーしてください`);}catch(e){d.error(`Error in copy process: ${e}`),y.showToast("エラー","スレッドのコピーに失敗しました");}finally{c.isCollecting=!1,c.translationInProgress=!1,y.updateMainButtonText();}}catch(e){d.error(`Button click handler error: ${e}`),y.showToast("内部エラー","処理中に予期せぬエラーが発生しました。");}}updateButtonVisibility(){this.isTwitterStatusPage()?(y.init(),y.addMainButton(this.handleButtonClick.bind(this)),y.updateAllUI()):y.destroy();}isTwitterStatusPage(){return Mt.test(location.href)}observeUrlChanges(){let t=location.href;const e=()=>{location.href!==t&&(t=location.href,setTimeout(()=>this.updateButtonVisibility(),300));},o=history.pushState;history.pushState=function(...i){o.apply(this,i),e();};const r=history.replaceState;history.replaceState=function(...i){r.apply(this,i),e();},window.addEventListener("popstate",()=>e()),new MutationObserver(()=>e()).observe(document.body,{childList:true,subtree:true});}}new $e;
