...
(function () {
    ...
    `,r.classList.remove("visible"),setTimeout(()=>{r?.classList.add("visible"),setTimeout(()=>{r?.classList.remove("visible"),setTimeout(()=>r?.remove(),500);},3e3);},10);}updateAllUI(){this.addControlPanel(),this.addStartPointButtons(),this.updateResetButton();}addStartPointButtons(){document.querySelectorAll('article[data-testid="tweet"]').forEach(t=>{if(Array.from(t.children).find(s=>s.classList.contains("start-point-button")))return;const r=t.querySelector('a[href*="/status/"]');if(!r)return;const n=r.href.split("/").pop()?.split("?")[0]??"";if(!n)return;const i=document.createElement("button");i.className="start-point-button",i.textContent="★",i.title="この位置からコピー開始",i.dataset.tweetId=n,d.startFromTweetId===n&&(i.classList.add("active"),i.textContent="✓",t.classList.add("start-point-set")),i.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation(),this.setStartPoint(t,n);}),t.appendChild(i);});}setStartPoint(t,o){d.startFromTweetId&&document.querySelectorAll(".start-point-set").forEach(s=>{s.classList.remove("start-point-set");const a=s.querySelector(".start-point-button");a&&(a.classList.remove("active"),a.textContent="★");});const r=t.querySelector('div[data-testid="User-Name"]')?.innerText??"",n=t.querySelector('div[data-testid="tweetText"]')?.innerText??"";d.startFromTweetId=o,d.startFromTweetAuthor=r,d.startFromTweetText=n,t.classList.add("start-point-set");const i=t.querySelector(".start-point-button");i&&(i.classList.add("active"),i.textContent="✓"),this.updateResetButton(),this.updateMainButtonText(),this.showToast("起点設定完了",`${r}のツイートを起点に設定しました`),c.log(`Start point set: ${o} by ${r}`);}updateResetButton(){let t=this.querySelector(".reset-start-point");d.startFromTweetId?(t||(t=document.createElement("button"),t.className="reset-start-point",t.textContent="起点をリセット",t.addEventListener("click",()=>this.resetStartPoint()),this.appendChild(t)),t.classList.add("visible")):t?.classList.remove("visible");}resetStartPoint(){d.startFromTweetId=null,d.startFromTweetAuthor="",d.startFromTweetText="",document.querySelectorAll(".start-point-set").forEach(t=>{t.classList.remove("start-point-set");const o=t.querySelector(".start-point-button");o&&(o.classList.remove("active"),o.textContent="★");}),this.updateResetButton(),this.updateMainButtonText(),this.showToast("起点リセット","コピー起点をリセットしました"),c.log("Start point reset");}ensureFloatingContainer(){if(!this.shadowRoot)throw new Error("Shadow root is not initialized");if(!this.floatingContainer){const t=document.createElement("div");t.className="floating-ui-container",this.shadowRoot.appendChild(t);const o=this.loadPosition();o&&(this.customPosition=o,this.applyPosition(o.top,o.left,t)),this.floatingContainer=t;}return this.floatingContainer}initializeDrag(t){if(t.dataset.dragInitialized==="true")return;t.dataset.dragInitialized="true",t.addEventListener("pointerdown",r=>{if(!r.isPrimary)return;const n=this.ensureFloatingContainer(),i=n.getBoundingClientRect();this.dragState={pointerId:r.pointerId,startX:r.clientX,startY:r.clientY,initialTop:this.customPosition?.top??i.top,initialLeft:this.customPosition?.left??i.left,moved:false},n.classList.add("dragging"),t.classList.add("dragging");try{t.setPointerCapture(r.pointerId);}catch(s){c.warn("pointer capture failed",s);}}),t.addEventListener("pointermove",r=>{if(!this.dragState||r.pointerId!==this.dragState.pointerId)return;const n=this.floatingContainer;if(!n)return;const i=r.clientX-this.dragState.startX,s=r.clientY-this.dragState.startY;if(!this.dragState.moved){if(Math.abs(i)<4&&Math.abs(s)<4)return;this.dragState.moved=true,n.classList.add("has-custom-position"),n.style.bottom="auto",n.style.right="auto";}const a=this.dragState.initialTop+s,l=this.dragState.initialLeft+i,{top:u,left:h}=this.clampPosition(a,l,n);this.applyPosition(u,h,n);});const o=r=>{!this.dragState||r.pointerId!==this.dragState.pointerId||(t.hasPointerCapture(r.pointerId)&&t.releasePointerCapture(r.pointerId),t.classList.remove("dragging"),this.floatingContainer?.classList.remove("dragging"),this.dragState.moved&&(this.ignoreNextClick=true,this.handleResize(),this.savePosition()),this.dragState=null);};t.addEventListener("pointerup",o),t.addEventListener("pointercancel",o);}clampPosition(t,o,r){const i=r.offsetHeight||0,s=r.offsetWidth||0,a=Math.max(16,window.innerHeight-i-16),l=Math.max(16,window.innerWidth-s-16);return {top:Math.min(Math.max(16,t),a),left:Math.min(Math.max(16,o),l)}}applyPosition(t,o,r){const n=r??this.ensureFloatingContainer();n.style.top=`${t}px`,n.style.left=`${o}px`,n.style.bottom="auto",n.style.right="auto",n.classList.add("has-custom-position"),this.customPosition={top:t,left:o},this.savePosition();}loadPosition(){try{const t=window.localStorage.getItem(this.storageKey);if(!t)return null;const o=JSON.parse(t);return typeof o.top=="number"&&typeof o.left=="number"&&Number.isFinite(o.top)&&Number.isFinite(o.left)?{top:o.top,left:o.left}:(c.warn("stored position is invalid",o),null)}catch(t){return c.warn("failed to load UI position",t),null}}savePosition(){if(this.customPosition)try{const t=JSON.stringify(this.customPosition);window.localStorage.setItem(this.storageKey,t);}catch(t){c.warn("failed to save UI position",t);}}}const g=new W;async function Y(){try{let e=function(){const a=document.querySelectorAll('article[data-testid="tweet"]');a.length>0&&!r&&(r=I(a[0]),n=$(a[0])),a.forEach(l=>{const u=l.querySelector('a[href*="/status/"]');if(!u)return;const h=u.href.split("/"),p=h.indexOf("status");if(p===-1||p+1>=h.length)return;const x=h[p+1].split("?")[0];if(o.has(x))return;const f=I(l);if(r&&f!==r)return;let m=$(l);f===r&&!m&&n&&(m=n);const y=l.querySelector('div[data-testid="tweetText"]');let w="";y&&(w=H(y));const b=l.querySelector("time"),A=b?b.getAttribute("datetime"):"",F=A?J(new Date(A)):"",O=X(l),D=Q(l),V=u.href?u.href.split("?")[0]:"";t.push({id:x,author:f,handle:m,text:w,time:F,url:V,mediaUrls:O,quotedTweet:D}),o.add(x);});};const t=[],o=new Set;let r="",n="";const i=window.scrollY;async function s(){try{await M(),e();let u=t.length,h=0;for(let p=0;p<30;p++)try{if(window.scrollBy(0,window.innerHeight*.7),await new Promise(x=>setTimeout(x,700)),await M(),e(),t.length===u){if(h++,h>=3)break}else u=t.length,h=0;}catch(x){c.error(`スクロール処理エラー (試行 ${p+1}): ${x.message}`);continue}try{window.scrollTo(0,i);}catch(p){c.error(`スクロール位置復元エラー: ${p.message}`);}}catch(u){throw c.error(`スクロール収集処理エラー: ${u.message}`),u}}return await s(),t.sort((a,l)=>{const u=new Date(a.time.replace(/年|月|日/g,"-").replace(/:/g,"-").split(" ")[0]),h=new Date(l.time.replace(/年|月|日/g,"-").replace(/:/g,"-").split(" ")[0]);return u.getTime()-h.getTime()}),t}catch(e){return c.error(`ツイート収集中にエラーが発生: ${e.message}`),c.error(`エラースタック: ${e.stack}`),[]}}function $(e){try{const o=Array.from(e.querySelectorAll("span")).find(i=>{try{const s=i.textContent?i.textContent.trim():"";return s.startsWith("@")&&!s.includes(" ")&&s.length>1}catch{return !1}});if(o)return o.textContent.trim();const r=Array.from(e.querySelectorAll('a[role="link"][href^="/"]'));for(const i of r)try{const s=i.getAttribute("href");if(s&&!s.includes("/status/")&&s.length>1&&!s.includes("/i/"))return "@"+s.replace(/^\//,"")}catch{continue}const n=e.querySelector('div[data-testid="User-Name"]');if(n){const i=n.querySelectorAll("span");for(const s of Array.from(i))try{const a=s.textContent?s.textContent.trim():"";if(a.startsWith("@")&&!a.includes(" "))return a}catch{continue}}return ""}catch(t){return c.error(`ユーザーハンドル取得エラー: ${t.message}`),""}}function I(e){try{const t=e.querySelector('div[data-testid="User-Name"] a[role="link"] span');return t&&t.textContent?t.textContent.trim():""}catch(t){return c.error(`ユーザー名取得エラー: ${t.message}`),""}}function Q(e){const t=e.querySelector('[data-testid="tweetQuotedLink"]');let o=null;if(t)try{const r=t.closest('div[role="link"]');r&&(o=q(r));}catch(r){c.error(`引用ツイート取得エラー: ${r.message}`);}else try{const r=e.innerText||"",n=r.includes("引用")||r.includes("Quote")||r.includes("quote"),i=e.querySelectorAll('div[role="link"]');if(i.length>0&&n)for(let s=0;s<i.length;s++){const a=i[s],l=a.innerText||"";if(l.includes("@")&&(l.includes("年")||l.includes("時間")||l.includes("分")||l.includes("日")))try{const u=q(a);if(u&&u.author&&u.text){o=u;break}}catch(u){c.error(`代替引用ツイート抽出エラー: ${u.message}`);}}}catch(r){c.error(`代替引用ツイート検索エラー: ${r.message}`);}return o}function q(e){const t=e.querySelector('div[dir="ltr"] > span'),o=t?t.textContent.trim():"",r=e.querySelector('div[dir="ltr"] span:nth-child(2)'),n=r?r.textContent.trim():"";let i="";const s=e.querySelector('div[data-testid="tweetText"]');if(s)i=H(s);else {const f=(e.innerText||"").split(`
`).map(w=>w.trim()).filter(w=>w);let m=-1;for(let w=0;w<f.length;w++)if(f[w].includes(n)){m=w;break}const y=m+1;m>=0&&y<f.length&&(i=f.slice(y+1).join(`
`));}const a=[];e.querySelectorAll('[data-testid="tweetPhoto"]').forEach(x=>{const f=x.querySelector('img[src*="pbs.twimg.com/media"]');if(f){const m=T(f.src);m&&!a.includes(m)&&a.push(m);}}),a.length===0&&(e.querySelectorAll('[role="group"]').forEach(m=>{m.querySelectorAll('img[src*="pbs.twimg.com/media"]').forEach(w=>{const b=T(w.src);b&&!a.includes(b)&&a.push(b);});}),e.querySelectorAll('img[src*="pbs.twimg.com/media"]').forEach(m=>{const y=T(m.src);y&&!a.includes(y)&&a.push(y);}));let u="",h="";const p=e.querySelectorAll('a[href*="/status/"]');for(const x of Array.from(p)){const f=x.href;if(f&&f.includes("/status/")){const m=f.split("/"),y=m.indexOf("status");if(y!==-1&&y+1<m.length){u=m[y+1].split("?")[0],h=f;break}}}return o&&i?{author:o,handle:n,text:i,id:u,url:h,mediaUrls:a}:null}function X(e){const t=[];return e.querySelectorAll('[data-testid="tweetPhoto"]').forEach(n=>{const i=n.querySelector('img[src*="pbs.twimg.com/media"]');if(i){const s=T(i.src);s&&!t.includes(s)&&t.push(s);}}),e.querySelectorAll("video").forEach(n=>{if(n.src&&n.src.startsWith("blob:"))try{if(n.poster&&n.poster.includes("pbs.twimg.com/tweet_video_thumb")){const a=k(n.poster);if(a&&!t.includes(a)){t.push(a);return}}const s=e.querySelector('a[href*="/status/"]');if(s){const a=s.href.split("/"),l=a.indexOf("status");if(l!==-1&&l+1<a.length){const h=`https://twitter.com/i/status/${a[l+1].split("?")[0]}`;t.includes(h)||t.push(`[動画] ${h}`);}}}catch(s){c.error("Error processing blob URL: "+s);}else {if(n.poster&&n.poster.includes("pbs.twimg.com/tweet_video_thumb")){const s=k(n.poster);s&&!t.includes(s)&&t.push(s);}n.src&&n.src.includes("video.twimg.com")&&(t.includes(n.src)||t.push(n.src));}n.querySelectorAll("source").forEach(s=>{if(s.src&&s.src.includes("video.twimg.com"))t.includes(s.src)||t.push(s.src);else if(s.src&&s.src.startsWith("blob:")){const a=e.querySelector('a[href*="/status/"]');if(a){const l=a.href.split("/"),u=l.indexOf("status");if(u!==-1&&u+1<l.length){const p=`https://twitter.com/i/status/${l[u+1].split("?")[0]}`;t.includes(p)||t.push(`[動画] ${p}`);}}}});}),t.length===0&&e.querySelectorAll('[role="group"]').forEach(i=>{i.querySelectorAll('img[src*="pbs.twimg.com/media"]').forEach(a=>{const l=T(a.src);l&&!t.includes(l)&&t.push(l);});}),t.length===0&&e.querySelectorAll('img[src*="pbs.twimg.com/media"]').forEach(i=>{const s=T(i.src);s&&!t.includes(s)&&t.push(s);}),t}function T(e){if(!e||typeof e!="string"||!e.includes("pbs.twimg.com/media"))return null;try{const t=e.match(/format=([^&]+)/),o=t?t[1]:"jpg",r=e.split("?")[0];if(!r||r.length===0)return c.error(`無効なベースURL: ${e}`),null;const n=r+"."+o;try{return new URL(n),n}catch(i){return c.error(`無効なURL形式: ${n}, エラー内容: ${i.message}`),null}}catch(t){return c.error(`メディアURL処理エラー: ${t.message}`),null}}function k(e){if(!e||!e.includes("tweet_video_thumb"))return null;try{const t=e.match(/tweet_video_thumb\/([^.]+)/);return !t||!t[1]?null:`https://video.twimg.com/tweet_video/${t[1]}.mp4`}catch(t){return c.error(`動画URL生成エラー: ${t.message}`),null}}function H(e){try{const t=e.querySelector('[role="button"]');if(t){const o=t.textContent?.trim()??"";if(o==="さらに表示"||o==="Show more"||o.match(/もっと見る/i)){const r=[],n=document.createTreeWalker(e,NodeFilter.SHOW_TEXT,null);let i;for(;i=n.nextNode();)i.textContent?.trim()&&i.parentElement&&!i.parentElement.closest('[role="button"]')&&r.push(i.textContent);return P(r,e).replace(/(さらに表示|Show more|もっと見る)/g,"").trim()}}return P([e.innerText],e)}catch(t){return c.error(`ツイートテキスト取得エラー: ${t.message}`),e.innerText}}function P(e,t){try{const o=Array.from(t.querySelectorAll('p, div[dir="auto"]'));return o.length>1?o.map(r=>r.textContent?.trim()??"").filter(r=>!r.match(/(さらに表示|Show more|もっと見る)/i)).join(`
`):e.join(" ").trim()}catch(o){return c.error(`改行保持処理エラー: ${o.message}`),e.join(" ").trim()}}async function M(){try{const e=document.querySelectorAll('[data-testid="tweet"] [role="button"]');let t=0;for(const o of Array.from(e))try{const r=o.textContent?o.textContent.trim():"";(r==="さらに表示"||r==="Show more"||r.match(/もっと見る/i)||r.match(/show more/i))&&!o.hasAttribute("href")&&!o.querySelector("a")&&o.closest('[data-testid="tweet"]')&&o.click&&(o.click(),t++,await new Promise(i=>setTimeout(i,100)));}catch(r){c.error(`個別ツイート展開エラー: ${r.message}`);continue}return t}catch(e){return c.error(`ツイート展開処理エラー: ${e.message}`),0}}function J(e){const t=e.getFullYear(),o=String(e.getMonth()+1).padStart(2,"0"),r=String(e.getDate()).padStart(2,"0"),n=String(e.getHours()).padStart(2,"0"),i=String(e.getMinutes()).padStart(2,"0");return `${t}年${o}月${r}日 ${n}:${i}`}const K="http://localhost:3002/v1/chat/completions",Z="https://translate.googleapis.com/translate_a/single";async function tt(e){const t=e.map(nt),o=t.map(i=>({tweet:i,textSegments:B(i.text),quotedSegments:i.quotedTweet?B(i.quotedTweet.text):null})),r=[];for(const i of o){for(const s of i.textSegments)E(s,r);if(i.quotedSegments)for(const s of i.quotedSegments)E(s,r);}if(r.length===0)return {tweets:t,hasTranslation:false};let n=false;for(let i=0;i<r.length;i++){const s=r[i];try{const l=await et(s.original);s.translated=l,!n&&l!==s.original&&(n=!0);}catch(l){c.error(`セグメント翻訳に失敗: ${l.message}`),s.translated=s.original;}i<r.length-1&&await R(1e3+Math.random()*500);}for(const i of o)i.tweet.text=U(i.textSegments),i.quotedSegments&&i.tweet.quotedTweet&&(i.tweet.quotedTweet.text=U(i.quotedSegments));return {tweets:t,hasTranslation:n}}function E(e,t){if(e.kind==="text"){if(e.original.trim().length===0){e.translated=e.original;return}t.push(e);}}function U(e){return e.length===0?"":e.map(t=>t.kind==="fixed"?t.value:t.translated??t.original).join("")}function B(e){if(!e)return [];const t=/(https?:\/\/[^\s]+|@[A-Za-z0-9_]{1,15})/g,o=[];let r=0,n;for(;(n=t.exec(e))!==null;)n.index>r&&o.push({kind:"text",original:e.slice(r,n.index)}),o.push({kind:"fixed",value:n[0]}),r=n.index+n[0].length;return r<e.length&&o.push({kind:"text",original:e.slice(r)}),o.length===0&&o.push({kind:"text",original:e}),o}async function et(e){if(e.trim().length===0)return e;const t=await ot(e);if(t)return t;try{return await rt(e)}catch(o){return c.error(`Google翻訳にも失敗しました: ${o.message}`),e}}async function ot(e){try{const t=`<|plamo:op|>dataset
translation
<|plamo:op|>input lang=auto
${e}
<|plamo:op|>output lang=ja`,o=await new Promise((i,s)=>{GM_xmlhttpRequest({method:"POST",url:K,headers:{"Content-Type":"application/json"},data:JSON.stringify({model:"plamo-13b-instruct",messages:[{role:"user",content:t}],temperature:0,max_tokens:4096,stream:!1}),timeout:3e4,onload:a=>a.status>=200&&a.status<300?i(a):s(new Error(`API error: ${a.status}`)),onerror:a=>s(a),ontimeout:()=>s(new Error("Timeout"))});}),n=JSON.parse(o.responseText)?.choices?.[0]?.message?.content;if(n&&n.trim().length>0)return c.log("ローカルAIでの翻訳に成功しました。"),n;throw new Error("ローカルAIからの翻訳結果が空です。")}catch(t){return c.error(`ローカルAIでの翻訳に失敗: ${t.message}`),null}}async function rt(e){const t="auto";let r=0;const n=3;for(;r<n;)try{const i=`${Z}?client=gtx&sl=${t}&tl=ja&dt=t&dt=bd&dj=1&q=${encodeURIComponent(e)}`,s=await new Promise((u,h)=>{GM_xmlhttpRequest({method:"GET",url:i,timeout:15e3,onload:p=>p.status>=200&&p.status<300?u(p):h(new Error(`API error: ${p.status}`)),onerror:p=>h(p),ontimeout:()=>h(new Error("Timeout"))});}),a=JSON.parse(s.responseText);if(!a?.sentences?.length)throw new Error("Invalid translation response format.");const l=a.sentences.map(u=>u?.trans??"").join("");if(!l.trim())throw new Error("Translation result is empty");return l}catch(i){if(r++,c.error(`Google翻訳試行 ${r}/${n} 失敗: ${i.message}`),r>=n)throw i;await R(1e3*Math.pow(2,r));}throw new Error("Google翻訳に失敗しました。")}function nt(e){return {...e,mediaUrls:[...e.mediaUrls],quotedTweet:e.quotedTweet?{...e.quotedTweet,mediaUrls:[...e.quotedTweet.mediaUrls]}:null}}async function R(e){await new Promise(t=>setTimeout(t,e));}function C(e){let t=`${e.author} ${e.handle}
${e.text}
${e.time}
`;if(e.url&&(t+=`${e.url}
