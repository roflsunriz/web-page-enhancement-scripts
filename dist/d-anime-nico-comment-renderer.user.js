...
`;class w{static getCommonStyles(){return dt}static getNotificationStyles(){return ht}static getAutoButtonStyles(){return pt}}const H={success:"✔",warning:"⚠",error:"✖",info:"ℹ"};class d extends M{static instance=null;static notifications=[];hostElement=null;initialized=false;static getInstance(){return this.instance||(this.instance=new d),this.instance}static show(t,n="info",e=3e3){try{const r=this.getInstance();return r.initialize(),r.initialized?r.createNotification(t,n,e):null}catch(r){return console.error("[NotificationManager] show failed",r),null}}static removeNotification(t){this.getInstance().removeNotification(t.element);}show(t,n="info"){d.show(t,n);}initialize(){if(!this.initialized)try{this.hostElement=document.createElement("div"),this.hostElement.className="nico-comment-shadow-host notification-host",this.hostElement.style.cssText=["position: fixed","top: 0","right: 0","z-index: 10000","pointer-events: none"].join(";"),document.body.appendChild(this.hostElement),this.createShadowDOM(this.hostElement),this.addStyles(w.getNotificationStyles()),this.setHTML('<div class="notification-container"></div>'),this.initialized=!0;}catch(t){console.error("[NotificationManager] initialize failed",t),this.initialized=false;}}destroy(){d.notifications.forEach(t=>{t.timerId&&window.clearTimeout(t.timerId);}),d.notifications=[],this.hostElement?.parentNode&&this.hostElement.parentNode.removeChild(this.hostElement),this.hostElement=null,this.initialized=false,d.instance=null;}createNotification(t,n,e){try{const r=this.querySelector(".notification-container");if(!r)throw new Error("Notification container not found");const i=H[n]??H.info,o=document.createElement("div");o.className=`notification-item ${n}`;const a=document.createElement("div");a.className="notification-icon",a.innerHTML=`<span>${i}</span>`,o.appendChild(a);const c=document.createElement("div");c.className="notification-content";const p=document.createElement("div");p.className="notification-type",p.textContent=n.charAt(0).toUpperCase()+n.slice(1),c.appendChild(p);const h=document.createElement("div");h.className="notification-message",h.textContent=t||"No message",c.appendChild(h),o.appendChild(c);const u=document.createElement("button");u.className="notification-close",u.innerHTML="&times;",u.addEventListener("click",()=>{this.removeNotification(o);}),o.appendChild(u),r.appendChild(o),requestAnimationFrame(()=>{o.classList.add("show");});const g={element:o,timerId:window.setTimeout(()=>{this.removeNotification(o);},e)};return d.notifications.push(g),g}catch(r){return console.error("[NotificationManager] createNotification failed",r),null}}removeNotification(t){if(!t)return;const n=d.notifications.find(e=>e.element===t);n?.timerId&&window.clearTimeout(n.timerId),t.classList.remove("show"),window.setTimeout(()=>{try{t.remove(),d.notifications=d.notifications.filter(e=>e.element!==t);}catch(e){console.error("[NotificationManager] removeNotification cleanup failed",e);}},500);}}const Y="https://www.nicovideo.jp",j=`${Y}/search`,ut=`${Y}/watch`,gt={searchBase:j},mt=s=>`${ut}/${s}`,X=s=>`${j}/${encodeURIComponent(s)}`,V=s=>new Promise((t,n)=>{st({url:s.url,method:s.method??"GET",headers:s.headers,data:s.data,responseType:s.responseType??"text",timeout:s.timeout,onprogress:s.onprogress,onload:e=>{t({status:e.status,statusText:e.statusText,response:e.response,finalUrl:e.finalUrl,headers:e.responseHeaders});},onerror:e=>{const r=e?.error??"unknown error";n(new Error(`GM_xmlhttpRequest failed: ${r}`));},ontimeout:()=>{n(new Error("GM_xmlhttpRequest timeout"));}});}),I=x("dAnime:NicoApiFetcher");class N{apiData=null;comments=null;get lastApiData(){return this.apiData}get cachedComments(){return this.comments}async fetchApiData(t){try{const n=this.sanitizeVideoId(t),r=(await V({method:"GET",url:mt(n),headers:{Accept:"text/html"}})).response,a=new DOMParser().parseFromString(r,"text/html").querySelector('meta[name="server-response"]');if(!a)throw new Error("API data element not found in response");const c=a.getAttribute("content");if(!c)throw new Error("API data content is empty");const p=this.decodeServerResponse(c),u=JSON.parse(p).data?.response;if(!u)throw new Error("Invalid API data structure");return this.apiData=u,u}catch(n){throw I.error("NicoApiFetcher.fetchApiData",n),n}}async fetchComments(){try{if(!this.apiData)throw new Error("API data must be fetched first");const t=this.apiData.comment?.nvComment;if(!t?.server||!t.params||!t.threadKey)throw new Error("Required comment server data is missing");const n=await V({method:"POST",url:`${t.server}/v1/threads`,headers:{"Content-Type":"application/json","x-client-os-type":"others","X-Frontend-Id":"6","X-Frontend-Version":"0"},data:JSON.stringify({params:t.params,threadKey:t.threadKey,additionals:{}})}),i=(JSON.parse(n.response).data?.threads??[]).filter(a=>a.fork==="main").sort((a,c)=>(c.commentCount||0)-(a.commentCount||0))[0];if(!i)throw new Error("Main thread not found in comment response");const o=(i.comments??[]).map(a=>({text:a.body??"",vpos:a.vposMs??0,commands:a.commands??[]}));return this.comments=o,o}catch(t){throw I.error("NicoApiFetcher.fetchComments",t),t}}async fetchAllData(t){return await this.fetchApiData(t),this.fetchComments()}sanitizeVideoId(t){try{let n=encodeURIComponent(t);return n=n.replace(/%([0-9A-F]{2})/gi,(e,r)=>{const i=parseInt(r,16);return i>=65&&i<=90||i>=97&&i<=122||i>=48&&i<=57||i===45||i===95||i===46||i===126?String.fromCharCode(i):e}),n}catch(n){return I.error("NicoApiFetcher.sanitizeVideoId",n,{videoId:t}),t}}decodeServerResponse(t){try{return decodeURIComponent(t)}catch(n){try{const e=t.replace(/%(?![0-9A-F]{2})/gi,"%25");return decodeURIComponent(e)}catch{throw new Error(`API data decode failed: ${n.message}`)}}}}const $=x("dAnime:NicoVideoSearcher");class J{cache=new Map;async search(t){if(!t.trim())return [];if(this.cache.has(t))return this.cache.get(t)??[];const n=X(t),e=await this.fetchText(n),r=this.parseServerContext(e),i=[],o=new Set;for(const a of r)a?.videoId&&(o.has(a.videoId)||(o.add(a.videoId),i.push(a)));return i.sort((a,c)=>c.viewCount-a.viewCount),this.cache.set(t,i),i}async fetchText(t){return (await V({method:"GET",url:t})).response}parseServerContext(t){try{const e=new DOMParser().parseFromString(t,"text/html").querySelector('meta[name="server-response"]');if(!e)return [];const r=e.getAttribute("content")??"",i=this.decodeHtmlEntities(r);let o;try{o=JSON.parse(i);}catch(a){return $.error("NicoVideoSearcher.parseServerContext",a),[]}return this.extractVideoItems(o??{})}catch(n){return $.error("NicoVideoSearcher.parseServerContext",n),[]}}decodeHtmlEntities(t){if(!t)return "";let n=t.replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">");return n=n.replace(/&#(\d+);/g,(e,r)=>String.fromCharCode(parseInt(r,10))),n=n.replace(/&#x([0-9a-fA-F]+);/g,(e,r)=>String.fromCharCode(parseInt(r,16))),n}extractVideoItems(t){const n=[],e=i=>{const o=(i?.id??i?.contentId??i?.watchId??"").toString();if(!o)return;const a=(i?.title??i?.shortTitle??"").toString(),c=i?.count??{},p=Number(c.view??i?.viewCounter??0)||0,h=Number(c.comment??i?.commentCounter??0)||0,u=Number(c.mylist??i?.mylistCounter??0)||0,g=i?.thumbnail??{},v=(g.nHdUrl||g.listingUrl||g.largeUrl||g.middleUrl||g.url||i?.thumbnailUrl||"").toString(),nt=(i?.registeredAt||i?.startTime||i?.postedDateTime||"")?.toString?.()??"",et=i?.owner&&typeof i.owner=="object"?{nickname:(i.owner.nickname??i.owner.name??"")||void 0,name:(i.owner.name??i.owner.nickname??"")||void 0}:null,rt=(i?.isChannelVideo||i?.owner?.ownerType==="channel")&&i?.owner?{name:i.owner.name??""}:null;a&&n.push({videoId:o,title:a,viewCount:p,commentCount:h,mylistCount:u,thumbnail:v,postedAt:nt,owner:et,channel:rt});},r=i=>{if(!i)return;if(Array.isArray(i)){i.forEach(r);return}if(typeof i!="object"||i===null)return;const o=i;(o.id||o.contentId||o.watchId)&&e(o),Object.values(i).forEach(r);};return r(t),n}}class L{delay;timers=new Map;funcIds=new Map;nextId=1;constructor(t){this.delay=t;}getFuncId(t){return this.funcIds.has(t)||this.funcIds.set(t,this.nextId++),this.funcIds.get(t)??0}exec(t){const n=this.getFuncId(t),e=Date.now(),r=this.timers.get(n)?.lastExec??0,i=e-r;if(i>this.delay)t(),this.timers.set(n,{lastExec:e});else {clearTimeout(this.timers.get(n)?.timerId??void 0);const o=setTimeout(()=>{t(),this.timers.set(n,{lastExec:Date.now()});},this.delay-i);this.timers.set(n,{timerId:o,lastExec:r});}}execOnce(t){const n=this.getFuncId(t),e=this.timers.get(n);if(e?.executedOnce){e.timerId&&clearTimeout(e.timerId);return}e?.timerId&&clearTimeout(e.timerId);const r=setTimeout(()=>{try{t();}finally{this.timers.set(n,{executedOnce:true,lastExec:Date.now(),timerId:null});}},this.delay);this.timers.set(n,{timerId:r,executedOnce:false,scheduled:true});}cancel(t){const n=this.getFuncId(t),e=this.timers.get(n);e?.timerId&&clearTimeout(e.timerId),this.timers.delete(n);}resetExecution(t){const n=this.getFuncId(t),e=this.timers.get(n);e&&(e.timerId&&clearTimeout(e.timerId),this.timers.set(n,{executedOnce:false,scheduled:false}));}clearAll(){for(const[,t]of this.timers)t.timerId&&clearTimeout(t.timerId);this.timers.clear(),this.funcIds.clear();}}const ft=1e3,bt=100,xt=30,yt=/watch\/(?:([a-z]{2}))?(\d+)/gi,q=s=>{if(!s?.video)return null;const t=s.video.registeredAt,n=t?new Date(t).toLocaleString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false}):void 0;return {videoId:s.video.id,title:s.video.title,viewCount:s.video.count?.view,mylistCount:s.video.count?.mylist,commentCount:s.video.count?.comment,postedAt:n,thumbnail:s.video.thumbnail?.url,owner:s.owner??null,channel:s.channel??null}},vt=s=>{const t=[];let n;for(;(n=yt.exec(s))!==null;){const[,e="",r=""]=n;r&&t.push(`${e}${r}`);}return t};class Z{constructor(t,n,e,r=ft,i=bt){this.renderer=t,this.fetcher=n,this.settingsManager=e,this.monitorInterval=r,this.debounce=new L(i);}nextVideoId=null;preloadedComments=null;lastPreloadedComments=null;lastVideoId=null;checkIntervalId=null;isSwitching=false;debounce;startMonitoring(){this.stopMonitoring(),this.checkIntervalId=window.setInterval(()=>{this.checkVideoEnd();},this.monitorInterval);}stopMonitoring(){this.checkIntervalId&&(window.clearInterval(this.checkIntervalId),this.checkIntervalId=null);}async onVideoSwitch(t){if(!this.isSwitching){this.isSwitching=true;try{const n=this.preloadedComments??this.lastPreloadedComments??null,e=t?.dataset?.videoId??t?.getAttribute?.("data-video-id")??null,r=this.nextVideoId??e??this.lastVideoId;if(!t||!r&&!n){this.handleMissingVideoInfo(n);return}d.show("動画の切り替わりを検知しました...","info");let i=null;r&&(i=await this.fetchVideoApiData(r,n),i&&(this.persistVideoMetadata(i),this.lastVideoId=r)),this.renderer.clearComments();const o=await this.populateComments(r,n);if(o===0&&d.show("コメントを取得できませんでした","warning"),this.nextVideoId=null,this.preloadedComments=null,i){const a=q(i);if(a){const c=`コメントソースを更新しました: ${a.title??"不明なタイトル"}（${o}件）`;d.show(c,o>0?"success":"warning");}}}catch(n){console.error("[VideoSwitchHandler] 動画切り替え中にエラーが発生しました",n),d.show(`動画切り替えエラー: ${n.message}`,"error"),this.renderer.clearComments();}finally{this.isSwitching=false;}}}async checkVideoEnd(){const t=this.renderer.getVideoElement();if(!(!t||!Number.isFinite(t.duration)||t.duration-t.currentTime>xt)){if(!this.nextVideoId){const e=async()=>{await this.findNextVideoId();};this.debounce.execOnce(e);}if(this.nextVideoId&&!this.preloadedComments){const e=async()=>{await this.preloadComments();};this.debounce.execOnce(e);}}}handleMissingVideoInfo(t){t||(this.renderer.clearComments(),d.show("次の動画のコメントを取得できませんでした。コメント表示をクリアします。","warning"));}async fetchVideoApiData(t,n){try{return await this.fetcher.fetchApiData(t)}catch(e){if(console.error("[VideoSwitchHandler] API取得エラー",e),!n)throw e;return null}}persistVideoMetadata(t){const n=q(t);n&&this.settingsManager.saveVideoData(n.title??"",n);}async populateComments(t,n){let e=null;if(Array.isArray(n)&&n.length>0)e=n;else if(t)try{e=await this.fetcher.fetchAllData(t);}catch(i){console.error("[VideoSwitchHandler] コメント取得エラー",i),d.show(`コメント取得エラー: ${i.message}`,"error"),e=null;}if(!e||e.length===0)return 0;const r=e.filter(i=>!this.renderer.isNGComment(i.text));return r.forEach(i=>{this.renderer.addComment(i.text,i.vpos,i.commands);}),this.lastPreloadedComments=[...r],r.length}async findNextVideoId(){try{const t=this.fetcher.lastApiData;if(!t)return;const n=t.series?.video?.next?.id;if(n){this.nextVideoId=n;return}const e=t.video?.description??"";if(!e)return;const r=vt(e);if(r.length===0)return;const i=[...r].sort((o,a)=>{const c=parseInt(o.replace(/^[a-z]{2}/i,""),10)||0;return (parseInt(a.replace(/^[a-z]{2}/i,""),10)||0)-c});this.nextVideoId=i[0]??null;}catch(t){console.error("[VideoSwitchHandler] 次の動画ID取得エラー",t);}}async preloadComments(){if(this.nextVideoId)try{const n=(await this.fetcher.fetchAllData(this.nextVideoId)).filter(e=>!this.renderer.isNGComment(e.text));this.preloadedComments=n.length>0?n:null;}catch(t){console.error("[VideoSwitchHandler] コメントプリロードエラー",t),this.preloadedComments=null;}}}const wt=`
.nico-comment-shadow-host {
  display: block;
  position: relative;
}
`;class Q{static initialize(){ot(wt);}}var St="M9,22A1,1 0 0,1 8,21V18H4A2,2 0 0,1 2,16V4C2,2.89 2.9,2 4,2H20A2,2 0 0,1 22,4V16A2,2 0 0,1 20,18H13.9L10.2,21.71C10,21.9 9.75,22 9.5,22V22H9M10,16V19.08L13.08,16H20V4H4V16H10Z",Ct="M12,17A2,2 0 0,0 14,15C14,13.89 13.1,13 12,13A2,2 0 0,0 10,15A2,2 0 0,0 12,17M18,8A2,2 0 0,1 20,10V20A2,2 0 0,1 18,22H6A2,2 0 0,1 4,20V10C4,8.89 4.9,8 6,8H7V6A5,5 0 0,1 12,1A5,5 0 0,1 17,6V8H18M12,3A3,3 0 0,0 9,6V8H15V6A3,3 0 0,0 12,3Z",kt="M17.5,12A1.5,1.5 0 0,1 16,10.5A1.5,1.5 0 0,1 17.5,9A1.5,1.5 0 0,1 19,10.5A1.5,1.5 0 0,1 17.5,12M14.5,8A1.5,1.5 0 0,1 13,6.5A1.5,1.5 0 0,1 14.5,5A1.5,1.5 0 0,1 16,6.5A1.5,1.5 0 0,1 14.5,8M9.5,8A1.5,1.5 0 0,1 8,6.5A1.5,1.5 0 0,1 9.5,5A1.5,1.5 0 0,1 11,6.5A1.5,1.5 0 0,1 9.5,8M6.5,12A1.5,1.5 0 0,1 5,10.5A1.5,1.5 0 0,1 6.5,9A1.5,1.5 0 0,1 8,10.5A1.5,1.5 0 0,1 6.5,12M12,3A9,9 0 0,0 3,12A9,9 0 0,0 12,21A1.5,1.5 0 0,0 13.5,19.5C13.5,19.11 13.35,18.76 13.11,18.5C12.88,18.23 12.73,17.88 12.73,17.5A1.5,1.5 0 0,1 14.23,16H16A5,5 0 0,0 21,11C21,6.58 16.97,3 12,3Z";function z(s,t=24){const n=String(t),e=String(t);return `<svg xmlns="http://www.w3.org/2000/svg" width="${n}" height="${e}" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true" focusable="false"><path d="${s}"></path></svg>`}const At=z(kt),B=z(Ct),It=z(St),b={mypageHeaderTitle:".p-mypageHeader__title",mypageListContainer:".p-mypageMain",watchVideoElement:"video#video",mypageItem:".itemModule.list",mypageItemTitle:".line1",mypageEpisodeNumber:".number.line1 span",mypageEpisodeTitle:".episode.line1 span"},y=x("dAnime:SettingsUI"),l={searchInput:"#searchInput",searchButton:"#searchButton",openSearchPage:"#openSearchPageDirect",searchResults:"#searchResults",saveButton:"#saveSettings",opacitySelect:"#commentOpacity",visibilityToggle:"#commentVisibilityToggle",currentTitle:"#currentTitle",currentVideoId:"#currentVideoId",currentOwner:"#currentOwner",currentViewCount:"#currentViewCount",currentCommentCount:"#currentCommentCount",currentMylistCount:"#currentMylistCount",currentPostedAt:"#currentPostedAt",currentThumbnail:"#currentThumbnail",colorValue:".color-value",colorPreview:".color-preview",colorPicker:"#colorPicker",colorPickerInput:"#colorPickerInput",openColorPicker:"#openColorPicker",ngWords:"#ngWords",ngRegexps:"#ngRegexps",showNgWords:"#showNgWords",showNgRegexps:"#showNgRegexp",playCurrentVideo:"#playCurrentVideo"};class tt extends M{settingsManager;fetcher;searcher;settings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;constructor(t,n=new N,e=new J){super(),this.settingsManager=t,this.fetcher=n,this.searcher=e,this.settings=this.settingsManager.getSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData();}insertIntoMypage(){const t=document.querySelector(b.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling));}addAutoCommentButtons(){document.querySelectorAll(b.mypageItem).forEach(n=>{const e=n.querySelector(b.mypageItemTitle);if(!e||e.querySelector(".auto-comment-button-host"))return;const r=e.querySelector("span")?.textContent?.trim()??"",i=n.querySelector(b.mypageEpisodeNumber)?.textContent?.trim()??"",o=n.querySelector(b.mypageEpisodeTitle)?.textContent?.trim()??"",a=document.createElement("div");a.className="nico-comment-shadow-host auto-comment-button-host",a.style.cssText="position:absolute;left:150px;top:3px;margin-left:8px;";const c=a.attachShadow({mode:"closed"}),p=document.createElement("style");p.textContent=w.getAutoButtonStyles(),c.appendChild(p);const h=document.createElement("button");h.className="auto-comment-button",h.innerHTML=It,h.setAttribute("aria-label","コメント設定"),h.setAttribute("title","コメント設定"),h.setAttribute("type","button"),h.addEventListener("click",async u=>{u.preventDefault(),u.stopPropagation(),u.stopImmediatePropagation();try{const g=[r,i,o].filter(Boolean).join(" ");this.scrollToSettings(),this.setSearchKeyword(g),this.lastAutoButtonElement=a;const v=await this.executeSearch(g);if(v.length===0)return;await this.applySearchResult(v[0]);}catch(g){y.error("SettingsUI.autoCommentButton",g);}}),c.appendChild(h),e.appendChild(a),this.lastAutoButtonElement=a;});}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(w.getCommonStyles());const n=this.buildSettingsHtml();return this.setHTML(n),this.applySettingsToUI(),this.setupEventListeners(),t}buildSettingsHtml(){const t=r=>typeof r=="number"?r.toLocaleString():"-",n=r=>{if(!r)return "-";try{return new Date(r).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return r}},e=this.currentVideoInfo;return `
        ...
        <div class="setting-group display-settings-group">
          ...
          <div class="color-picker-container">
              <button id="openColorPicker" class="color-picker-button" type="button">
                <span class="color-picker-button__icon" aria-hidden="true">${At}</span>
        ...
        <div class="setting-group">
          ...
          <div class="ng-words-container">
            <button id="showNgWords" class="mask-button" type="button">
              <span class="mask-button__icon" aria-hidden="true">${B}</span>
...
`).map(r=>r.trim()).filter(Boolean)),this.settingsManager.updateSettings(this.settings)?d.show("設定を保存しました","success"):d.show("設定の保存に失敗しました","error");}updateCurrentVideoInfo(t){this.currentVideoInfo=t,[["currentTitle",t.title??"-"],["currentVideoId",t.videoId??"-"],["currentOwner",t.owner?.nickname??t.channel?.name??"-"],["currentViewCount",this.formatNumber(t.viewCount)],["currentCommentCount",this.formatNumber(t.commentCount)],["currentMylistCount",this.formatNumber(t.mylistCount)],["currentPostedAt",this.formatSearchResultDate(t.postedAt)]].forEach(([r,i])=>{const o=this.querySelector(l[r]);o&&(o.textContent=i);});const e=this.querySelector(l.currentThumbnail);e&&t.thumbnail&&(e.src=t.thumbnail,e.alt=t.title??"サムネイル");try{this.settingsManager.saveVideoData(t.title??"",t);}catch(r){y.error("SettingsUI.updateCurrentVideoInfo",r);}this.updatePlayButtonState(t);}formatNumber(t){return typeof t=="number"?t.toLocaleString():"-"}formatSearchResultDate(t){if(!t)return "-";const n=new Date(t);return Number.isNaN(n.getTime())?t:new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23"}).format(n)}setupPlayButton(){const t=this.querySelector(l.playCurrentVideo);t&&(t.addEventListener("click",()=>{try{if(!this.currentVideoInfo?.videoId){d.show("再生できる動画が設定されていません","warning");return}if(this.lastAutoButtonElement){const r=this.lastAutoButtonElement.closest(".itemModule.list")?.querySelector('.thumbnailContainer > a, .thumbnail-container > a, .thumbnailContainer a, .thumbnail-container a, a[href*="/watch/"]');if(r){d.show(`「${this.currentVideoInfo?.title||"動画"}」を再生します...`,"success"),setTimeout(()=>{r.click();},300);return}}d.show("再生リンクが見つかりませんでした。自動再生はサムネイル内のリンクを介してのみ行われます","warning");}catch(n){y.error("SettingsUI.playCurrentVideo",n),d.show(`再生エラー: ${n.message}`,"error");}}),this.updatePlayButtonState(this.currentVideoInfo));}updatePlayButtonState(t){const n=this.querySelector(l.playCurrentVideo);if(!n)return;const e=!!t?.videoId;n.disabled=!e,n.setAttribute("aria-disabled",(!e).toString());}updateVisibilityToggleState(t){t.textContent=this.settings.isCommentVisible?"表示中":"非表示中",t.classList.toggle("off",!this.settings.isCommentVisible);}updateMaskButtonText(t,n){const e=t.querySelector(".mask-button__text");e&&(e.textContent=n);}}const W=async()=>{},Et=()=>{const s=T();if(!s.dAniRenderer){const t={};s.dAniRenderer={classes:{Comment:U,CommentRenderer:K,NicoApiFetcher:N,NotificationManager:d,StyleManager:Q,SettingsUI:tt,NicoVideoSearcher:J,VideoSwitchHandler:Z,SettingsManager:R,KeyboardShortcutHandler:G,DebounceExecutor:L,ShadowDOMComponent:M,ShadowStyleManager:w},instances:t,utils:{initialize:W,initializeWithVideo:W},debug:{showState:()=>{console.log("Current instances:",t);},showSettings:()=>{const n=t.settingsManager;if(!n){console.log("SettingsManager not initialized");return}console.log("Current settings:",n.getSettings());},showComments:()=>{const n=t.renderer;if(!n){console.log("CommentRenderer not initialized");return}console.log("Current comments:",n.getCommentsSnapshot());}},defaultSettings:f};}return s.dAniRenderer},Vt=100,Tt=1e3,Rt=3e3;class Mt{constructor(t){this.global=t;}initialized=false;switchDebounce=new L(Tt);switchCallback=null;lastSwitchTimestamp=0;async initialize(){await this.ensureDocumentReady(),this.waitForVideoElement();}async ensureDocumentReady(){document.readyState!=="complete"&&await new Promise(t=>{window.addEventListener("load",()=>t(),{once:true});});}waitForVideoElement(){if(this.initialized)return;const t=document.querySelector(b.watchVideoElement);if(!t){window.setTimeout(()=>this.waitForVideoElement(),Vt);return}if(t.readyState===0){t.addEventListener("loadedmetadata",()=>{this.initializeWithVideo(t);},{once:true});return}this.initializeWithVideo(t);}async initializeWithVideo(t){if(!this.initialized){this.initialized=true;try{d.show("コメントローダーを初期化中...");const n=d.getInstance(),e=this.global.settingsManager??new R(n);this.global.settingsManager=e,this.global.instances.settingsManager=e;const r=e.loadVideoData();if(!r?.videoId)throw new Error("動画データが見つかりません。マイページで設定してください。");const i=new N;this.global.instances.fetcher=i,await i.fetchApiData(r.videoId);const o=await i.fetchComments(),a=this.mergeSettings(e.loadSettings()),c=new K(a);c.initialize(t),this.global.instances.renderer=c,e.addObserver(h=>{c.settings=this.mergeSettings(h);}),o.forEach(h=>{c.addComment(h.text,h.vpos,h.commands);});const p=new Z(c,i,e);p.startMonitoring(),this.global.instances.switchHandler=p,this.setupSwitchHandling(t,p),d.show(`コメントの読み込みが完了しました（${o.length}件）`,"success");}catch(n){throw this.initialized=false,d.show(`初期化エラー: ${n.message}`,"error"),n}}}mergeSettings(t){const n=f();return {...n,...t,ngWords:[...t.ngWords??n.ngWords],ngRegexps:[...t.ngRegexps??n.ngRegexps]}}setupSwitchHandling(t,n){this.switchCallback=()=>{const r=Date.now();r-this.lastSwitchTimestamp<Rt||(this.lastSwitchTimestamp=r,n.onVideoSwitch(t));},new MutationObserver(r=>{if(this.switchCallback)for(const i of r)i.type==="attributes"&&i.attributeName==="src"&&(this.switchDebounce.resetExecution(this.switchCallback),this.switchDebounce.execOnce(this.switchCallback));}).observe(t,{attributes:true,attributeFilter:["src"]}),t.addEventListener("ended",()=>{this.switchCallback&&(this.switchDebounce.resetExecution(this.switchCallback),this.switchDebounce.execOnce(this.switchCallback));}),this.global.utils.initializeWithVideo=async r=>{await n.onVideoSwitch(r);};}}const Nt=100;class Lt{constructor(t){this.global=t;}initialize(){const t=d.getInstance(),n=this.global.settingsManager??new R(t);this.global.settingsManager=n,this.global.instances.settingsManager=n;const e=new tt(n);this.waitForHeader(e);}waitForHeader(t){if(!document.querySelector(b.mypageHeaderTitle)){window.setTimeout(()=>this.waitForHeader(t),Nt);return}t.insertIntoMypage(),t.addAutoCommentButtons(),this.observeList(t);}observeList(t){const n=document.querySelector(b.mypageListContainer);if(!n)return;new MutationObserver(()=>{try{t.addAutoCommentButtons();}catch(r){console.error("[MypageController] auto comment buttons update failed",r);}}).observe(n,{childList:true,subtree:true});}}class zt{log;global=Et();watchController=null;mypageController=null;constructor(){this.log=x("DanimeApp");}start(){this.log.info("starting renderer"),Q.initialize(),this.global.utils.initialize=()=>this.initialize(),window.addEventListener("load",()=>{this.initialize();});}async initialize(){if(!this.acquireInstanceLock()){this.log.info("renderer already initialized, skipping");return}const t=location.pathname.toLowerCase();try{t.includes("/animestore/sc_d_pc")?(this.watchController=new Mt(this.global),await this.watchController.initialize()):t.includes("/animestore/mp_viw_pc")?(this.mypageController=new Lt(this.global),this.mypageController.initialize()):this.log.info("page not supported, initialization skipped");}catch(n){this.log.error("initialization failed",n);}}acquireInstanceLock(){const t=T();return t.__dAnimeNicoCommentRenderer2Instance=(t.__dAnimeNicoCommentRenderer2Instance??0)+1,t.__dAnimeNicoCommentRenderer2Instance===1}}const E=x("dAnimeNicoCommentRenderer2");async function Ft(){E.info("bootstrap start");try{new zt().start(),E.info("bootstrap completed");}catch(s){E.error("bootstrap failed",s);}}Ft();
