...
(function () {
  ...
  const Lt={debug:"debug",info:"info",warn:"warn",error:"error"},F=e=>{const t=`[${e}]`,n={};return Object.keys(Lt).forEach(i=>{const s=Lt[i];n[i]=(...r)=>{(console[s]??console.log)(t,...r);};}),n};function xt(){return typeof unsafeWindow<"u"?unsafeWindow:window}const De={small:.8,medium:1,big:1.4},He={defont:'"MS PGothic","Hiragino Kaku Gothic ProN","Hiragino Kaku Gothic Pro","Yu Gothic UI","Yu Gothic","Meiryo","Segoe UI","Osaka","Noto Sans CJK JP","Noto Sans JP","Source Han Sans JP","IPAPGothic","TakaoPGothic","Roboto","Helvetica Neue","Helvetica","Arial","sans-serif"',gothic:'"Noto Sans CJK JP","Noto Sans JP","Source Han Sans JP","Yu Gothic","Yu Gothic Medium","Meiryo","MS PGothic","Hiragino Kaku Gothic ProN","Segoe UI","Helvetica","Arial","sans-serif"',mincho:'"MS PMincho","MS Mincho","Hiragino Mincho ProN","Hiragino Mincho Pro","Yu Mincho","Noto Serif CJK JP","Noto Serif JP","Source Han Serif JP","Times New Roman","serif"'},ie={white:"#FFFFFC",red:"#FF8888",pink:"#FFA5CC",orange:"#FFBA66",yellow:"#FFFFAA",green:"#88FF88",cyan:"#88FFFF",blue:"#8899FF",purple:"#D9A5FF",black:"#444444",white2:"#CC9",red2:"#C03",pink2:"#F3C",orange2:"#F60",yellow2:"#990",green2:"#0C6",cyan2:"#0CC",blue2:"#39F",purple2:"#63C",black2:"#666"},wt=/^#([0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i,Oe=/^[,.:;]+/,Ne=/[,.:;]+$/,ze=e=>{const t=e.trim();return t?wt.test(t)?t:t.replace(Oe,"").replace(Ne,""):""},$e=e=>wt.test(e)?e.toUpperCase():null,re=e=>{const t=e.trim();if(!t)return null;const n=t.toLowerCase().endsWith("px")?t.slice(0,-2):t,i=Number.parseFloat(n);return Number.isFinite(i)?i:null},We=e=>{const t=e.trim();if(!t)return null;if(t.endsWith("%")){const n=Number.parseFloat(t.slice(0,-1));return Number.isFinite(n)?n/100:null}return re(t)},Be=e=>Number.isFinite(e)?Math.min(100,Math.max(-100,e)):0,qe=e=>!Number.isFinite(e)||e===0?1:Math.min(5,Math.max(.25,e)),Ue=e=>e==="naka"||e==="ue"||e==="shita",Ge=e=>e==="small"||e==="medium"||e==="big",Ke=e=>e==="defont"||e==="gothic"||e==="mincho",Ye=e=>e in ie,je=(e,t)=>{let n="naka",i="medium",s="defont",r=null,a=1,o=null,l=false,d=0,u=1;for(const p of e){const y=ze(typeof p=="string"?p:"");if(!y)continue;if(wt.test(y)){const M=$e(y);if(M){r=M;continue}}const v=y.toLowerCase();if(Ue(v)){n=v;continue}if(Ge(v)){i=v;continue}if(Ke(v)){s=v;continue}if(Ye(v)){r=ie[v].toUpperCase();continue}if(v==="_live"){o=.5;continue}if(v==="invisible"){a=0,l=true;continue}if(v.startsWith("ls:")||v.startsWith("letterspacing:")){const M=y.indexOf(":");if(M>=0){const S=re(y.slice(M+1));S!==null&&(d=Be(S));}continue}if(v.startsWith("lh:")||v.startsWith("lineheight:")){const M=y.indexOf(":");if(M>=0){const S=We(y.slice(M+1));S!==null&&(u=qe(S));}continue}}const c=Math.max(0,Math.min(1,a)),h=(r??t.defaultColor).toUpperCase(),m=typeof o=="number"?Math.max(0,Math.min(1,o)):null;return {layout:n,size:i,sizeScale:De[i],font:s,fontFamily:He[s],resolvedColor:h,colorOverride:r,opacityMultiplier:c,opacityOverride:m,isInvisible:l,letterSpacing:d,lineHeight:u}},Xe=/^#([0-9A-F]{3}|[0-9A-F]{4}|[0-9A-F]{6}|[0-9A-F]{8})$/i,J=e=>e.length===1?e.repeat(2):e,O=e=>Number.parseInt(e,16),B=e=>!Number.isFinite(e)||e<=0?0:e>=1?1:e,se=(e,t)=>{const n=Xe.exec(e);if(!n)return e;const i=n[1];let s,r,a,o=1;i.length===3||i.length===4?(s=O(J(i[0])),r=O(J(i[1])),a=O(J(i[2])),i.length===4&&(o=O(J(i[3]))/255)):(s=O(i.slice(0,2)),r=O(i.slice(2,4)),a=O(i.slice(4,6)),i.length===8&&(o=O(i.slice(6,8))/255));const l=B(o*B(t));return `rgba(${s}, ${r}, ${a}, ${l})`},Je=()=>({now:()=>typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}),ae=()=>Je(),R=e=>e*1e3,Ze=e=>!Number.isFinite(e)||e<0?null:Math.round(e),St=4e3,Rt=1800,Qe=3,tn=.25,en=32,nn=48,rt=120,rn=4e3,dt=120,sn=800,an=2,K=4e3,W=4e3,_=W+St,on=1e3,Ft=1,oe=12,le=24,k=.001,z=50,ln=5,cn=2,_t=8,dn=12,hn=500,un=3e3,Dt={debug:0,info:1,warn:2,error:3},mn=(e,t,n)=>{const i=[`[${t}]`,...n];switch(e){case "debug":console.debug(...i);break;case "info":console.info(...i);break;case "warn":console.warn(...i);break;case "error":console.error(...i);break;default:console.log(...i);}},ce=(e,t={})=>{const{level:n="info",emitter:i=mn}=t,s=Dt[n],r=(a,o)=>{Dt[a]<s||i(a,e,o);};return {debug:(...a)=>r("debug",a),info:(...a)=>r("info",a),warn:(...a)=>r("warn",a),error:(...a)=>r("error",a)}},Ct=ce("CommentEngine:Comment"),Ht=new WeakMap,pn=e=>{let t=Ht.get(e);return t||(t=new Map,Ht.set(e,t)),t},Mt=(e,t)=>{if(!e)return 0;const n=`${e.font??""}::${t}`,i=pn(e),s=i.get(n);if(s!==void 0)return s;const r=e.measureText(t).width;return i.set(n,r),r},gn=e=>{if(e.includes(`
`)){const t=e.split(/\r?\n/);return t.length>0?t:[""]}return [e]},Ot=e=>Math.max(24,e),ht=(e,t)=>{let n=0;const i=e.letterSpacing;for(const a of e.lines){const o=Mt(t,a),l=a.length>1?i*(a.length-1):0,d=Math.max(0,o+l);d>n&&(n=d);}e.width=n;const s=Math.max(1,Math.floor(e.fontSize*e.lineHeightMultiplier));e.lineHeightPx=s;const r=e.lines.length>1?(e.lines.length-1)*s:0;e.height=e.fontSize+r;},fn=(e,t,n,i,s)=>{try{if(!t)throw new Error("Canvas context is required");if(!Number.isFinite(n)||!Number.isFinite(i))throw new Error("Canvas dimensions must be numbers");if(!s)throw new Error("Prepare options are required");const r=Math.max(n,1),a=Ot(Math.floor(i*.05)),o=Ot(Math.floor(a*e.sizeScale));e.fontSize=o,t.font=`${e.fontSize}px ${e.fontFamily}`,e.lines=gn(e.text),ht(e,t);const l=!e.isScrolling&&(e.layout==="ue"||e.layout==="shita");if(l){const L=Math.max(1,r-_t*2);if(e.width>L){const H=Math.max(dn,Math.min(e.fontSize,Math.floor(a*.6))),ct=L/Math.max(e.width,1),q=Math.max(H,Math.floor(e.fontSize*Math.min(ct,1)));q<e.fontSize&&(e.fontSize=q,t.font=`${e.fontSize}px ${e.fontFamily}`,ht(e,t));let It=0;for(;e.width>L&&e.fontSize>H&&It<5;){const _e=L/Math.max(e.width,1),At=Math.max(H,Math.floor(e.fontSize*Math.max(_e,.7)));At>=e.fontSize?e.fontSize=Math.max(H,e.fontSize-1):e.fontSize=At,t.font=`${e.fontSize}px ${e.fontFamily}`,ht(e,t),It+=1;}}}if(!e.isScrolling){e.bufferWidth=0;const L=l?_t:0,H=Math.max((r-e.width)/2,L),ct=Math.max(L,r-e.width-L),q=Math.min(H,Math.max(ct,L));e.virtualStartX=q,e.x=q,e.baseSpeed=0,e.speed=0,e.speedPixelsPerMs=0,e.visibleDurationMs=W,e.preCollisionDurationMs=W,e.totalDurationMs=W,e.reservationWidth=e.width,e.staticExpiryTimeMs=e.vposMs+W,e.lastUpdateTime=e.getTimeSource().now(),e.isPaused=!1;return}e.staticExpiryTimeMs=null;const d=Mt(t,"??".repeat(150)),u=e.width*Math.max(s.bufferRatio,0);e.bufferWidth=Math.max(s.baseBufferPx,u);const c=Math.max(s.entryBufferPx,e.bufferWidth),h=e.scrollDirection,m=h==="rtl"?r+s.virtualExtension:-e.width-e.bufferWidth-s.virtualExtension,p=h==="rtl"?-e.width-e.bufferWidth-c:r+c,y=h==="rtl"?r+c:-c,v=h==="rtl"?m+e.width+e.bufferWidth:m-e.bufferWidth;e.virtualStartX=m,e.x=m,e.exitThreshold=p;const M=r>0?e.width/r:0,S=s.maxVisibleDurationMs===s.minVisibleDurationMs;let A=s.maxVisibleDurationMs;if(!S&&M>1){const L=Math.min(M,s.maxWidthRatio),H=s.maxVisibleDurationMs/Math.max(L,1);A=Math.max(s.minVisibleDurationMs,Math.floor(H));}const T=r+e.width+e.bufferWidth+c,Ie=Math.max(A,1),lt=T/Ie,Ae=lt*1e3/60;e.baseSpeed=Ae,e.speed=e.baseSpeed,e.speedPixelsPerMs=lt;const Le=Math.abs(p-m),Re=h==="rtl"?Math.max(0,v-y):Math.max(0,y-v),Pt=Math.max(lt,Number.EPSILON);e.visibleDurationMs=A,e.preCollisionDurationMs=Math.max(0,Math.ceil(Re/Pt)),e.totalDurationMs=Math.max(e.preCollisionDurationMs,Math.ceil(Le/Pt));const Fe=e.width+e.bufferWidth+c;e.reservationWidth=Math.min(d,Fe),e.lastUpdateTime=e.getTimeSource().now(),e.isPaused=!1;}catch(r){throw Ct.error("Comment.prepare",r,{text:e.text,visibleWidth:n,canvasHeight:i,hasContext:!!t}),r}},de=5,D={enabled:false,maxLogsPerCategory:de},it=new Map,bn=e=>{if(e===void 0||!Number.isFinite(e))return de;const t=Math.max(1,Math.floor(e));return Math.min(1e4,t)},yn=e=>{D.enabled=!!e.enabled,D.maxLogsPerCategory=bn(e.maxLogsPerCategory),D.enabled||it.clear();},N=()=>D.enabled,vn=e=>{const t=it.get(e)??0;return t>=D.maxLogsPerCategory?(t===D.maxLogsPerCategory&&(console.debug(`[CommentOverlay][${e}]`,"Further logs suppressed."),it.set(e,t+1)),false):(it.set(e,t+1),true)},C=(e,...t)=>{D.enabled&&vn(e)&&console.debug(`[CommentOverlay][${e}]`,...t);},Y=(e,t=32)=>e.length<=t?e:`${e.slice(0,t)}…`,xn=(e,t)=>{D.enabled&&(console.group(`[CommentOverlay][state-dump] ${e}`),console.table({"Current Time":`${t.currentTime.toFixed(2)}ms`,Duration:`${t.duration.toFixed(2)}ms`,"Is Playing":t.isPlaying,"Epoch ID":t.epochId,"Total Comments":t.totalComments,"Active Comments":t.activeComments,"Reserved Lanes":t.reservedLanes,"Final Phase":t.finalPhaseActive,"Playback Begun":t.playbackHasBegun,"Is Stalled":t.isStalled}),console.groupEnd());},wn=(e,t,n)=>{D.enabled&&C("epoch-change",`Epoch changed: ${e} → ${t} (reason: ${n})`);},x={hits:0,misses:0,creates:0,fallbacks:0,outlineCallsInCache:0,fillCallsInCache:0,outlineCallsInFallback:0,fillCallsInFallback:0,letterSpacingComments:0,normalComments:0,multiLineComments:0,totalCharactersDrawn:0,lastReported:0},Nt=()=>{if(!N())return;const e=performance.now();if(e-x.lastReported<=5e3)return;const t=x.hits+x.misses,n=t>0?x.hits/t*100:0,i=x.creates>0?(x.totalCharactersDrawn/x.creates).toFixed(1):"0",s=x.outlineCallsInCache+x.outlineCallsInFallback,r=x.fillCallsInCache+x.fillCallsInFallback;console.log("[TextureCache Stats]",`
...
`;class ot extends Tt{static FAB_HOST_ID="danime-settings-fab-host";settingsManager;fetcher;searcher;settings;playbackSettings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;activeTab="search";modalElement=null;overlayElement=null;closeButtonElement=null;fabElement=null;fabHostElement=null;fabShadowRoot=null;handleFabClick=t=>{t.preventDefault(),this.openSettingsModal();};handleOverlayClick=()=>{this.closeSettingsModal();};handleCloseClick=()=>{this.closeSettingsModal();};handlePlaybackSettingsChanged;constructor(t,n=new at,i=new kt){super(),this.settingsManager=t,this.fetcher=n,this.searcher=i,this.settings=this.settingsManager.getSettings(),this.playbackSettings=this.settingsManager.getPlaybackSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData(),this.handlePlaybackSettingsChanged=s=>{this.playbackSettings=s,this.applyPlaybackSettingsToUI();},this.settingsManager.addPlaybackObserver(this.handlePlaybackSettingsChanged);}insertIntoMypage(){const t=document.querySelector(I.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling));}addAutoCommentButtons(){}async waitMypageListStable(){}tryRestoreByDanimeIds(){return  false}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(st.getCommonStyles());const n=this.buildSettingsHtml();return this.setHTML(n),this.applySettingsToUI(),this.addStyles(te),this.setupEventListeners(),t}buildSettingsHtml(){const t=s=>typeof s=="number"?s.toLocaleString():"-",n=s=>{if(!s)return "-";try{return new Date(s).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return s}},i=this.currentVideoInfo;return `
        ...
        <div class="settings-modal__content">
          ...
          <div class="settings-modal__tabs" role="tablist">
            <button class="settings-modal__tab is-active" type="button" data-tab="search" role="tab" aria-selected="true" aria-controls="settingsPaneSearch" id="settingsTabSearch">
              <span class="settings-modal__tab-icon" aria-hidden="true">${Qt}</span>
