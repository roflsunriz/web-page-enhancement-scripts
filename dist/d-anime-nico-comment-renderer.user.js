...
`)?this.text.split(/\r?\n/):[this.text];this.lines=l.length>0?l:[""];let c=0;const h=this.letterSpacing;for(const F of this.lines){const ct=K(t,F),pe=F.length>1?h*(F.length-1):0,kt=Math.max(0,ct+pe);kt>c&&(c=kt);}this.width=c;const u=Math.max(1,Math.floor(this.fontSize*this.lineHeightMultiplier));if(this.lineHeightPx=u,this.height=this.fontSize+(this.lines.length>1?(this.lines.length-1)*u:0),!this.isScrolling){this.bufferWidth=0;const F=Math.max((i-this.width)/2,0);this.virtualStartX=F,this.x=F,this.baseSpeed=0,this.speed=0,this.speedPixelsPerMs=0,this.visibleDurationMs=q,this.preCollisionDurationMs=q,this.totalDurationMs=q,this.reservationWidth=this.width,this.staticExpiryTimeMs=this.vposMs+q,this.lastUpdateTime=this.timeSource.now(),this.isPaused=!1;return}this.staticExpiryTimeMs=null;const d=K(t,"??".repeat(150)),m=this.width*Math.max(r.bufferRatio,0);this.bufferWidth=Math.max(r.baseBufferPx,m);const g=Math.max(r.entryBufferPx,this.bufferWidth),f=this.scrollDirection,b=f==="rtl"?i+r.virtualExtension:-this.width-this.bufferWidth-r.virtualExtension,C=f==="rtl"?-this.width-this.bufferWidth-g:i+g,M=f==="rtl"?i+g:-g,v=f==="rtl"?b+this.width+this.bufferWidth:b-this.bufferWidth;this.virtualStartX=b,this.x=b,this.exitThreshold=C;const w=i>0?this.width/i:0,S=r.maxVisibleDurationMs===r.minVisibleDurationMs;let L=r.maxVisibleDurationMs;if(!S&&w>1){const F=Math.min(w,r.maxWidthRatio),ct=r.maxVisibleDurationMs/Math.max(F,1);L=Math.max(r.minVisibleDurationMs,Math.floor(ct));}const H=i+this.width+this.bufferWidth+g,N=Math.max(L,1),I=H/N,U=I*1e3/60;this.baseSpeed=U,this.speed=this.baseSpeed,this.speedPixelsPerMs=I;const G=Math.abs(C-b),X=f==="rtl"?Math.max(0,v-M):Math.max(0,M-v),et=Math.max(I,Number.EPSILON);this.visibleDurationMs=L,this.preCollisionDurationMs=Math.max(0,Math.ceil(X/et)),this.totalDurationMs=Math.max(this.preCollisionDurationMs,Math.ceil(G/et));const ue=this.width+this.bufferWidth+g;this.reservationWidth=Math.min(d,ue),this.lastUpdateTime=this.timeSource.now(),this.isPaused=!1;}catch(i){throw dt.error("Comment.prepare",i,{text:this.text,visibleWidth:e,canvasHeight:n,hasContext:!!t}),i}}update(t=1,e=false){try{if(!this.isActive){this.isPaused=e;return}const n=this.timeSource.now();if(!this.isScrolling){this.isPaused=e,this.lastUpdateTime=n;return}if(e){this.isPaused=!0,this.lastUpdateTime=n;return}const r=(n-this.lastUpdateTime)/(1e3/60);this.speed=this.baseSpeed*t,this.x+=this.speed*r*this.directionSign,(this.scrollDirection==="rtl"&&this.x<=this.exitThreshold||this.scrollDirection==="ltr"&&this.x>=this.exitThreshold)&&(this.isActive=!1),this.lastUpdateTime=n,this.isPaused=!1;}catch(n){dt.error("Comment.update",n,{text:this.text,playbackRate:t,isPaused:e,isActive:this.isActive});}}generateTextureCacheKey(){return `v2::${this.text}::${this.fontSize}::${this.fontFamily}::${this.color}::${this.opacity}::${this.renderStyle}::${this.letterSpacing}::${this.lines.length}`}static cacheStats={hits:0,misses:0,creates:0,fallbacks:0,strokeCallsInCache:0,fillCallsInCache:0,strokeCallsInFallback:0,fillCallsInFallback:0,letterSpacingComments:0,normalComments:0,multiLineComments:0,totalCharactersDrawn:0,strokeSkippedCount:0,strokeUsedCount:0,smallFontComments:0,largeFontComments:0,lastReported:0};static reportCacheStats(){if(!P())return;const t=performance.now();if(t-p.cacheStats.lastReported>5e3){const e=p.cacheStats.hits+p.cacheStats.misses,n=e>0?p.cacheStats.hits/e*100:0,r=p.cacheStats.creates>0?(p.cacheStats.totalCharactersDrawn/p.cacheStats.creates).toFixed(1):"0",i=p.cacheStats.strokeUsedCount+p.cacheStats.strokeSkippedCount>0?(p.cacheStats.strokeSkippedCount/(p.cacheStats.strokeUsedCount+p.cacheStats.strokeSkippedCount)*100).toFixed(1):"0";console.log("[TextureCache Stats]",`
  ...
  Stroke: Used=${p.cacheStats.strokeUsedCount}, Skipped=${p.cacheStats.strokeSkippedCount}, Reduction=${i}%`,`
...
`;class tt{static getCommonStyles(){return an}static getNotificationStyles(){return on}static getAutoButtonStyles(){return ln}}const $t={success:"✔",warning:"⚠",error:"✖",info:"ℹ"};class x extends Ct{static instance=null;static notifications=[];hostElement=null;initialized=false;static getInstance(){return this.instance||(this.instance=new x),this.instance}static show(t,e="info",n=3e3){try{const r=this.getInstance();return r.initialize(),r.initialized?r.createNotification(t,e,n):null}catch(r){return console.error("[NotificationManager] show failed",r),null}}static removeNotification(t){this.getInstance().removeNotification(t.element);}show(t,e="info"){x.show(t,e);}initialize(){if(!this.initialized)try{this.hostElement=document.createElement("div"),this.hostElement.className="nico-comment-shadow-host notification-host",this.hostElement.style.cssText=["position: fixed","top: 0","right: 0","z-index: 2147483647","pointer-events: none"].join(";"),document.body.appendChild(this.hostElement),this.createShadowDOM(this.hostElement),this.addStyles(tt.getNotificationStyles()),this.setHTML('<div class="notification-container"></div>'),this.initialized=!0;}catch(t){console.error("[NotificationManager] initialize failed",t),this.initialized=false;}}destroy(){x.notifications.forEach(t=>{t.timerId&&window.clearTimeout(t.timerId);}),x.notifications=[],this.hostElement?.parentNode&&this.hostElement.parentNode.removeChild(this.hostElement),this.hostElement=null,this.initialized=false,x.instance=null;}createNotification(t,e,n){try{const r=this.querySelector(".notification-container");if(!r)throw new Error("Notification container not found");const i=$t[e]??$t.info,s=document.createElement("div");s.className=`notification-item ${e}`;const a=document.createElement("div");a.className="notification-icon",a.innerHTML=`<span>${i}</span>`,s.appendChild(a);const l=document.createElement("div");l.className="notification-content";const c=document.createElement("div");c.className="notification-type",c.textContent=e.charAt(0).toUpperCase()+e.slice(1),l.appendChild(c);const h=document.createElement("div");h.className="notification-message",h.textContent=t||"No message",l.appendChild(h),s.appendChild(l);const u=document.createElement("button");u.className="notification-close",u.innerHTML="&times;",u.addEventListener("click",()=>{this.removeNotification(s);}),s.appendChild(u),r.appendChild(s),requestAnimationFrame(()=>{s.classList.add("show");});const d={element:s,timerId:window.setTimeout(()=>{this.removeNotification(s);},n)};return x.notifications.push(d),d}catch(r){return console.error("[NotificationManager] createNotification failed",r),null}}removeNotification(t){if(!t)return;const e=x.notifications.find(n=>n.element===t);e?.timerId&&window.clearTimeout(e.timerId),t.classList.remove("show"),window.setTimeout(()=>{try{t.remove(),x.notifications=x.notifications.filter(n=>n.element!==t);}catch(n){console.error("[NotificationManager] removeNotification cleanup failed",n);}},500);}}const ie="https://www.nicovideo.jp",se=`${ie}/search`,ae=`${ie}/watch`,Wt={searchBase:se,watchBase:ae},cn=o=>`${ae}/${o}`,oe=o=>`${se}/${encodeURIComponent(o)}`,vt=o=>new Promise((t,e)=>{Qe({url:o.url,method:o.method??"GET",headers:o.headers,data:o.data,responseType:o.responseType??"text",timeout:o.timeout,onprogress:o.onprogress,onload:n=>{t({status:n.status,statusText:n.statusText,response:n.response,finalUrl:n.finalUrl,headers:n.responseHeaders});},onerror:n=>{const r=n?.error??"unknown error";e(new Error(`GM_xmlhttpRequest failed: ${r}`));},ontimeout:()=>{e(new Error("GM_xmlhttpRequest timeout"));}});}),gt=O("dAnime:NicoApiFetcher");class Mt{apiData=null;comments=null;get lastApiData(){return this.apiData}get cachedComments(){return this.comments}async fetchApiData(t){try{const e=this.sanitizeVideoId(t),r=(await vt({method:"GET",url:cn(e),headers:{Accept:"text/html"}})).response,a=new DOMParser().parseFromString(r,"text/html").querySelector('meta[name="server-response"]');if(!a)throw new Error("API data element not found in response");const l=a.getAttribute("content");if(!l)throw new Error("API data content is empty");const c=this.decodeServerResponse(l),u=JSON.parse(c).data?.response;if(!u)throw new Error("Invalid API data structure");return this.apiData=u,u}catch(e){throw gt.error("NicoApiFetcher.fetchApiData",e),e}}async fetchComments(){try{if(!this.apiData)throw new Error("API data must be fetched first");const t=this.apiData.comment?.nvComment;if(!t?.server||!t.params||!t.threadKey)throw new Error("Required comment server data is missing");const e=await vt({method:"POST",url:`${t.server}/v1/threads`,headers:{"Content-Type":"application/json","x-client-os-type":"others","X-Frontend-Id":"6","X-Frontend-Version":"0"},data:JSON.stringify({params:t.params,threadKey:t.threadKey,additionals:{}})}),i=(JSON.parse(e.response).data?.threads??[]).filter(a=>a.fork==="main").sort((a,l)=>(l.commentCount||0)-(a.commentCount||0))[0];if(!i)throw new Error("Main thread not found in comment response");const s=(i.comments??[]).map(a=>({text:a.body??"",vposMs:a.vposMs??0,commands:a.commands??[]}));return this.comments=s,s}catch(t){throw gt.error("NicoApiFetcher.fetchComments",t),t}}async fetchAllData(t){return await this.fetchApiData(t),this.fetchComments()}sanitizeVideoId(t){try{let e=encodeURIComponent(t);return e=e.replace(/%([0-9A-F]{2})/gi,(n,r)=>{const i=parseInt(r,16);return i>=65&&i<=90||i>=97&&i<=122||i>=48&&i<=57||i===45||i===95||i===46||i===126?String.fromCharCode(i):n}),e}catch(e){return gt.error("NicoApiFetcher.sanitizeVideoId",e,{videoId:t}),t}}decodeServerResponse(t){try{return decodeURIComponent(t)}catch(e){try{const n=t.replace(/%(?![0-9A-F]{2})/gi,"%25");return decodeURIComponent(n)}catch{throw new Error(`API data decode failed: ${e.message}`)}}}}const qt=O("dAnime:NicoVideoSearcher");class le{cache=new Map;async search(t){if(!t.trim())return [];if(this.cache.has(t))return this.cache.get(t)??[];const e=oe(t),n=await this.fetchText(e),r=this.parseServerContext(n).map(a=>{const l=this.calculateLevenshteinDistance(t,a.title),c=Math.max(t.length,a.title.length),h=c>0?(1-l/c)*100:0;return {...a,levenshteinDistance:l,similarity:h}}),i=[],s=new Set;for(const a of r)a?.videoId&&(s.has(a.videoId)||(s.add(a.videoId),i.push(a)));return i.sort((a,l)=>{const c=a.similarity??-1,h=l.similarity??-1;return c!==h?h-c:l.viewCount-a.viewCount}),this.cache.set(t,i),i}async fetchText(t){return (await vt({method:"GET",url:t})).response}parseServerContext(t){try{const n=new DOMParser().parseFromString(t,"text/html").querySelector('meta[name="server-response"]');if(!n)return [];const r=n.getAttribute("content")??"",i=this.decodeHtmlEntities(r);let s;try{s=JSON.parse(i);}catch(a){return qt.error("NicoVideoSearcher.parseServerContext",a),[]}return this.extractVideoItems(s??{})}catch(e){return qt.error("NicoVideoSearcher.parseServerContext",e),[]}}decodeHtmlEntities(t){if(!t)return "";let e=t.replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">");return e=e.replace(/&#(\d+);/g,(n,r)=>String.fromCharCode(parseInt(r,10))),e=e.replace(/&#x([0-9a-fA-F]+);/g,(n,r)=>String.fromCharCode(parseInt(r,16))),e}extractVideoItems(t){const e=[],n=i=>{const s=(i?.id??i?.contentId??i?.watchId??"").toString();if(!s)return;const a=(i?.title??i?.shortTitle??"").toString(),l=i?.count??{},c=Number(l.view??i?.viewCounter??0)||0,h=Number(l.comment??i?.commentCounter??0)||0,u=Number(l.mylist??i?.mylistCounter??0)||0,d=i?.thumbnail??{},m=(d.nHdUrl||d.listingUrl||d.largeUrl||d.middleUrl||d.url||i?.thumbnailUrl||"").toString(),g=(i?.registeredAt||i?.startTime||i?.postedDateTime||"")?.toString?.()??"",f=i?.owner&&typeof i.owner=="object"?{nickname:(i.owner.nickname??i.owner.name??"")||void 0,name:(i.owner.name??i.owner.nickname??"")||void 0}:null,b=(i?.isChannelVideo||i?.owner?.ownerType==="channel")&&i?.owner?{name:i.owner.name??""}:null;a&&e.push({videoId:s,title:a,viewCount:c,commentCount:h,mylistCount:u,thumbnail:m,postedAt:g,owner:f,channel:b});},r=i=>{if(!i)return;if(Array.isArray(i)){i.forEach(r);return}if(typeof i!="object"||i===null)return;const s=i;(s.id||s.contentId||s.watchId)&&n(s),Object.values(i).forEach(r);};return r(t),e}calculateLevenshteinDistance(t,e){const n=t?t.length:0,r=e?e.length:0;if(n===0)return r;if(r===0)return n;const i=new Array(r+1);for(let a=0;a<=r;++a){const l=i[a]=new Array(n+1);l[0]=a;}const s=i[0];for(let a=1;a<=n;++a)s[a]=a;for(let a=1;a<=r;++a)for(let l=1;l<=n;++l){const c=t[l-1]===e[a-1]?0:1;i[a][l]=Math.min(i[a-1][l]+1,i[a][l-1]+1,i[a-1][l-1]+c);}return i[r][n]}}const A={mypageHeaderTitle:".p-mypageHeader__title",mypageListContainer:".p-mypageMain",watchVideoElement:"video#video",mypageItem:".itemModule.list",mypageItemTitle:".line1",mypageEpisodeNumber:".number.line1 span",mypageEpisodeTitle:".episode.line1 span"};class Et{delay;timers=new Map;funcIds=new Map;nextId=1;constructor(t){this.delay=t;}getFuncId(t){return this.funcIds.has(t)||this.funcIds.set(t,this.nextId++),this.funcIds.get(t)??0}exec(t){const e=this.getFuncId(t),n=Date.now(),r=this.timers.get(e)?.lastExec??0,i=n-r;if(i>this.delay)t(),this.timers.set(e,{lastExec:n});else {clearTimeout(this.timers.get(e)?.timerId??void 0);const s=setTimeout(()=>{t(),this.timers.set(e,{lastExec:Date.now()});},this.delay-i);this.timers.set(e,{timerId:s,lastExec:r});}}execOnce(t){const e=this.getFuncId(t),n=this.timers.get(e);if(n?.executedOnce){n.timerId&&clearTimeout(n.timerId);return}n?.timerId&&clearTimeout(n.timerId);const r=setTimeout(()=>{try{t();}finally{this.timers.set(e,{executedOnce:true,lastExec:Date.now(),timerId:null});}},this.delay);this.timers.set(e,{timerId:r,executedOnce:false,scheduled:true});}cancel(t){const e=this.getFuncId(t),n=this.timers.get(e);n?.timerId&&clearTimeout(n.timerId),this.timers.delete(e);}resetExecution(t){const e=this.getFuncId(t),n=this.timers.get(e);n&&(n.timerId&&clearTimeout(n.timerId),this.timers.set(e,{executedOnce:false,scheduled:false}));}clearAll(){for(const[,t]of this.timers)t.timerId&&clearTimeout(t.timerId);this.timers.clear(),this.funcIds.clear();}}const dn=1e3,hn=100,un=30,Bt=1e4,Ut=100,pn=/watch\/(?:([a-z]{2}))?(\d+)/gi,k=O("dAnime:VideoSwitchHandler"),Gt=o=>{if(!o?.video)return null;const t=o.video.registeredAt,e=t?new Date(t).toLocaleString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false}):void 0;return {videoId:o.video.id,title:o.video.title,viewCount:o.video.count?.view,mylistCount:o.video.count?.mylist,commentCount:o.video.count?.comment,postedAt:e,thumbnail:o.video.thumbnail?.url,owner:o.owner??null,channel:o.channel??null}},mn=o=>{const t=[];let e;for(;(e=pn.exec(o))!==null;){const[,n="",r=""]=e;r&&t.push(`${n}${r}`);}return t};class ce{constructor(t,e,n,r=dn,i=hn){this.renderer=t,this.fetcher=e,this.settingsManager=n,this.monitorInterval=r,this.debounce=new Et(i);}nextVideoId=null;preloadedComments=null;lastPreloadedComments=null;lastVideoId=null;lastVideoSource=null;checkIntervalId=null;isSwitching=false;debounce;startMonitoring(){this.stopMonitoring(),this.checkIntervalId=window.setInterval(()=>{this.checkVideoEnd();},this.monitorInterval);}stopMonitoring(){this.checkIntervalId&&(window.clearInterval(this.checkIntervalId),this.checkIntervalId=null);}async onVideoSwitch(t){if(!this.isSwitching){this.isSwitching=true;try{const e=await this.resolveVideoElement(t)??null,n=this.preloadedComments??this.lastPreloadedComments??null,r=e?.dataset?.videoId??e?.getAttribute?.("data-video-id")??null,i=this.nextVideoId??r??this.lastVideoId;if(!e||!i&&!n){this.handleMissingVideoInfo(n);return}k.info("videoSwitch:start",{videoId:i??null,elementVideoId:e.dataset?.videoId??null,preloadedCount:n?.length??0}),x.show("動画の切り替わりを検知しました...","info"),this.resetRendererState(e);const s=this.renderer.getVideoElement();if(s!==e&&e)k.debug("videoSwitch:rebind",{previousSrc:this.renderer.getCurrentVideoSource(),newSrc:typeof e.currentSrc=="string"&&e.currentSrc.length>0?e.currentSrc:e.getAttribute("src")??null}),this.renderer.initialize(e),this.renderer.resetState();else if(s===e&&e&&this.hasVideoSourceChanged(e))k.debug("videoSwitch:rebind:sameElement",{previousSrc:this.lastVideoSource??null,newSrc:this.getVideoSource(e)}),this.renderer.clearComments(),this.renderer.destroy(),this.renderer.initialize(e),this.renderer.resetState();else if(!s&&!e){k.warn("videoSwitch:missingVideoElement",{lastVideoId:this.lastVideoId,nextVideoId:this.nextVideoId}),this.isSwitching=!1;return}let a=null;i&&(a=await this.fetchVideoApiData(i,n),a&&(this.persistVideoMetadata(a),this.lastVideoId=i));const l=await this.populateComments(i,n);if(l===0?(this.renderer.clearComments(),x.show("コメントを取得できませんでした","warning"),k.warn("videoSwitch:commentsMissing",{videoId:i??null})):k.info("videoSwitch:commentsLoaded",{videoId:i??null,count:l}),this.nextVideoId=null,this.preloadedComments=null,this.lastVideoSource=this.getVideoSource(e),a){const c=Gt(a);if(c){const h=`コメントソースを更新しました: ${c.title??"不明なタイトル"}（${l}件）`;x.show(h,l>0?"success":"warning");}}}catch(e){k.error("videoSwitch:error",e,{nextVideoId:this.nextVideoId,lastVideoId:this.lastVideoId}),x.show(`動画切り替えエラー: ${e.message}`,"error"),this.renderer.clearComments();}finally{this.isSwitching=false;}}}async resolveVideoElement(t){if(t){const r=this.getVideoSource(t),i=this.lastVideoSource;return (!r||r===i)&&await this.waitForSourceChange(t),t}const e=Date.now()+Bt;let n=null;for(;Date.now()<e;){const r=document.querySelector(A.watchVideoElement);if(r){n=r;const i=this.hasVideoSourceChanged(r);if(r.readyState>=2||!r.paused||i)return i&&(this.lastVideoSource=null),r}await new Promise(i=>window.setTimeout(i,Ut));}return n}async waitForSourceChange(t){const e=this.getVideoSource(t);if(!e)return;const n=Date.now()+Bt;for(;Date.now()<n;){const r=this.getVideoSource(t);if(r&&r!==e){this.lastVideoSource=null;return}await new Promise(i=>window.setTimeout(i,Ut));}}hasVideoSourceChanged(t){const e=this.getVideoSource(t);return e?this.lastVideoSource?this.lastVideoSource!==e:true:false}getVideoSource(t){if(!t)return null;const e=typeof t.currentSrc=="string"?t.currentSrc:"";if(e.length>0)return e;const n=t.getAttribute("src")??"";if(n.length>0)return n;const r=t.querySelector("source[src]");return r&&typeof r.src=="string"&&r.src.length>0?r.src:null}resetRendererState(t){try{t.currentTime=0;}catch(e){k.debug("videoSwitch:resetCurrentTimeFailed",e);}this.renderer.clearComments();}async checkVideoEnd(){const t=this.renderer.getVideoElement();if(!(!t||!Number.isFinite(t.duration)||t.duration-t.currentTime>un)){if(!this.nextVideoId){const n=async()=>{await this.findNextVideoId();};this.debounce.execOnce(n);}if(this.nextVideoId&&!this.preloadedComments){const n=async()=>{await this.preloadComments();};this.debounce.execOnce(n);}}}handleMissingVideoInfo(t){t||(this.renderer.clearComments(),x.show("次の動画のコメントを取得できませんでした。コメント表示をクリアします。","warning"));}async fetchVideoApiData(t,e){try{const n=await this.fetcher.fetchApiData(t);return k.debug("videoSwitch:apiFetched",{videoId:t}),n}catch(n){if(k.error("videoSwitch:apiFetchError",n,{videoId:t}),!e)throw n;return null}}persistVideoMetadata(t){const e=Gt(t);e&&this.settingsManager.saveVideoData(e.title??"",e);}async populateComments(t,e){let n=null;if(Array.isArray(e)&&e.length>0)n=e;else if(t)try{n=await this.fetcher.fetchAllData(t),k.debug("videoSwitch:commentsFetched",{videoId:t,count:n.length});}catch(i){k.error("videoSwitch:commentsFetchError",i,{videoId:t}),x.show(`コメント取得エラー: ${i.message}`,"error"),n=null;}if(!n||n.length===0)return 0;const r=n.filter(i=>!this.renderer.isNGComment(i.text));return r.forEach(i=>{this.renderer.addComment(i.text,i.vposMs,i.commands);}),this.lastPreloadedComments=[...r],r.length}async findNextVideoId(){try{const t=this.fetcher.lastApiData;if(!t)return;const e=t.series?.video?.next?.id;if(e){this.nextVideoId=e,k.debug("videoSwitch:detectedNext",{videoId:e});return}const n=t.video?.description??"";if(!n)return;const r=mn(n);if(r.length===0)return;const i=[...r].sort((s,a)=>{const l=parseInt(s.replace(/^[a-z]{2}/i,""),10)||0;return (parseInt(a.replace(/^[a-z]{2}/i,""),10)||0)-l});this.nextVideoId=i[0]??null,this.nextVideoId&&k.debug("videoSwitch:candidateFromDescription",{candidate:this.nextVideoId});}catch(t){k.error("videoSwitch:nextIdError",t,{lastVideoId:this.lastVideoId});}}async preloadComments(){if(this.nextVideoId)try{const e=(await this.fetcher.fetchAllData(this.nextVideoId)).filter(n=>!this.renderer.isNGComment(n.text));this.preloadedComments=e.length>0?e:null,k.debug("videoSwitch:preloaded",{videoId:this.nextVideoId,count:e.length});}catch(t){k.error("videoSwitch:preloadError",t,{videoId:this.nextVideoId}),this.preloadedComments=null;}}}const gn=`
...
`;class lt extends Ct{static FAB_HOST_ID="danime-settings-fab-host";settingsManager;fetcher;searcher;settings;playbackSettings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;activeTab="search";modalElement=null;overlayElement=null;closeButtonElement=null;fabElement=null;fabHostElement=null;fabShadowRoot=null;handleFabClick=t=>{t.preventDefault(),this.openSettingsModal();};handleOverlayClick=()=>{this.closeSettingsModal();};handleCloseClick=()=>{this.closeSettingsModal();};handlePlaybackSettingsChanged;constructor(t,e=new Mt,n=new le){super(),this.settingsManager=t,this.fetcher=e,this.searcher=n,this.settings=this.settingsManager.getSettings(),this.playbackSettings=this.settingsManager.getPlaybackSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData(),this.handlePlaybackSettingsChanged=r=>{this.playbackSettings=r,this.applyPlaybackSettingsToUI();},this.settingsManager.addPlaybackObserver(this.handlePlaybackSettingsChanged);}insertIntoMypage(){const t=document.querySelector(A.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}}));}addAutoCommentButtons(){document.querySelectorAll(A.mypageItem).forEach(e=>{const n=e.querySelector(A.mypageItemTitle);if(!n||n.querySelector(".auto-comment-button-host"))return;const r=n.querySelector("span")?.textContent?.trim()??"",i=e.querySelector(A.mypageEpisodeNumber)?.textContent?.trim()??"",s=e.querySelector(A.mypageEpisodeTitle)?.textContent?.trim()??"",a=document.createElement("div");a.className="nico-comment-shadow-host auto-comment-button-host",a.style.cssText="position:absolute;left:150px;top:3px;margin-left:8px;";const l=a.attachShadow({mode:"closed"}),c=document.createElement("style");c.textContent=tt.getAutoButtonStyles(),l.appendChild(c);const h=document.createElement("button");h.className="auto-comment-button",h.innerHTML=ft,h.setAttribute("aria-label","コメント設定"),h.setAttribute("title","コメント設定"),h.setAttribute("type","button"),h.addEventListener("click",async u=>{u.preventDefault(),u.stopPropagation(),u.stopImmediatePropagation();try{const d=[r,i,s].filter(Boolean).join(" ");this.openSettingsModal(!1),this.setSearchKeyword(d),this.lastAutoButtonElement=a;try{const g=e.querySelector('input[name="workId"]')?.value?.trim()??"",f=e.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),b=e.querySelector('a.textContainer[href*="partId="]');let C="";const v=(f?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);v?C=v[1]:b?.href&&(C=(new URL(b.href,location.origin).searchParams.get("partId")??"").trim()),g&&C&&this.settingsManager.saveLastDanimeIds({workId:g,partId:C});}catch(g){console.warn("[SettingsUI] save (workId, partId) skipped:",g);}const m=await this.executeSearch(d);if(m.length===0)return;await this.applySearchResult(m[0]);}catch(d){Y.error("SettingsUI.autoCommentButton",d);}}),l.appendChild(h),n.appendChild(a),this.lastAutoButtonElement=a;}),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}});}async waitMypageListStable(){const t=document.querySelector(A.mypageListContainer);if(!t)return;let e=t.querySelectorAll(A.mypageItem).length;const n=Date.now()+1500;return new Promise(r=>{const i=new MutationObserver(()=>{const s=t.querySelectorAll(A.mypageItem).length;if(s!==e){e=s;return}Date.now()>=n&&(i.disconnect(),r());});i.observe(t,{childList:true,subtree:true}),setTimeout(()=>{try{i.disconnect();}catch(s){Y.debug("waitMypageListStable: observer already disconnected",s);}r();},1600);})}tryRestoreByDanimeIds(){const t=this.settingsManager.loadLastDanimeIds();if(!t)return  false;const e=Array.from(document.querySelectorAll(A.mypageItem));for(const n of e){if(n.querySelector('input[name="workId"]')?.value?.trim()!==t.workId)continue;const i=n.querySelector('a.textContainer[href*="partId="]'),s=i?.href?(new URL(i.href,location.origin).searchParams.get("partId")??"")===t.partId:false,a=n.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),l=(()=>{const h=(a?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);return !!h&&h[1]===t.partId})();if(s||l){const c=n.querySelector(".auto-comment-button-host")??n;return this.lastAutoButtonElement=c,this.updatePlayButtonState(this.currentVideoInfo),true}}return  false}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(tt.getCommonStyles());const e=this.buildSettingsHtml();return this.setHTML(e),this.applySettingsToUI(),this.addStyles(Yt),this.setupEventListeners(),t}buildSettingsHtml(){const t=r=>typeof r=="number"?r.toLocaleString():"-",e=r=>{if(!r)return "-";try{return new Date(r).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return r}},n=this.currentVideoInfo;return `
