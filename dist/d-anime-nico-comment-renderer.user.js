...
(function () {
  ...
  const Lt={debug:"debug",info:"info",warn:"warn",error:"error"},F=e=>{const t=`[${e}]`,n={};return Object.keys(Lt).forEach(i=>{const r=Lt[i];n[i]=(...s)=>{(console[r]??console.log)(t,...s);};}),n};function xt(){return typeof unsafeWindow<"u"?unsafeWindow:window}const He={small:.8,medium:1,big:1.4},Ne={defont:'"MS PGothic","Hiragino Kaku Gothic ProN","Hiragino Kaku Gothic Pro","Yu Gothic UI","Yu Gothic","Meiryo","Segoe UI","Osaka","Noto Sans CJK JP","Noto Sans JP","Source Han Sans JP","IPAPGothic","TakaoPGothic","Roboto","Helvetica Neue","Helvetica","Arial","sans-serif"',gothic:'"Noto Sans CJK JP","Noto Sans JP","Source Han Sans JP","Yu Gothic","Yu Gothic Medium","Meiryo","MS PGothic","Hiragino Kaku Gothic ProN","Segoe UI","Helvetica","Arial","sans-serif"',mincho:'"MS PMincho","MS Mincho","Hiragino Mincho ProN","Hiragino Mincho Pro","Yu Mincho","Noto Serif CJK JP","Noto Serif JP","Source Han Serif JP","Times New Roman","serif"'},re={white:"#FFFFFC",red:"#FF8888",pink:"#FFA5CC",orange:"#FFBA66",yellow:"#FFFFAA",green:"#88FF88",cyan:"#88FFFF",blue:"#8899FF",purple:"#D9A5FF",black:"#444444",white2:"#CC9",red2:"#C03",pink2:"#F3C",orange2:"#F60",yellow2:"#990",green2:"#0C6",cyan2:"#0CC",blue2:"#39F",purple2:"#63C",black2:"#666"},wt=/^#([0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i,Oe=/^[,.:;]+/,ze=/[,.:;]+$/,$e=e=>{const t=e.trim();return t?wt.test(t)?t:t.replace(Oe,"").replace(ze,""):""},We=e=>wt.test(e)?e.toUpperCase():null,se=e=>{const t=e.trim();if(!t)return null;const n=t.toLowerCase().endsWith("px")?t.slice(0,-2):t,i=Number.parseFloat(n);return Number.isFinite(i)?i:null},Be=e=>{const t=e.trim();if(!t)return null;if(t.endsWith("%")){const n=Number.parseFloat(t.slice(0,-1));return Number.isFinite(n)?n/100:null}return se(t)},qe=e=>Number.isFinite(e)?Math.min(100,Math.max(-100,e)):0,Ue=e=>!Number.isFinite(e)||e===0?1:Math.min(5,Math.max(.25,e)),Ge=e=>e==="naka"||e==="ue"||e==="shita",Ke=e=>e==="small"||e==="medium"||e==="big",Ye=e=>e==="defont"||e==="gothic"||e==="mincho",je=e=>e in re,Xe=(e,t)=>{let n="naka",i="medium",r="defont",s=null,a=1,o=null,l=false,d=0,h=1;for(const p of e){const v=$e(typeof p=="string"?p:"");if(!v)continue;if(wt.test(v)){const E=We(v);if(E){s=E;continue}}const x=v.toLowerCase();if(Ge(x)){n=x;continue}if(Ke(x)){i=x;continue}if(Ye(x)){r=x;continue}if(je(x)){s=re[x].toUpperCase();continue}if(x==="_live"){o=.5;continue}if(x==="invisible"){a=0,l=true;continue}if(x.startsWith("ls:")||x.startsWith("letterspacing:")){const E=v.indexOf(":");if(E>=0){const S=se(v.slice(E+1));S!==null&&(d=qe(S));}continue}if(x.startsWith("lh:")||x.startsWith("lineheight:")){const E=v.indexOf(":");if(E>=0){const S=Be(v.slice(E+1));S!==null&&(h=Ue(S));}continue}}const c=Math.max(0,Math.min(1,a)),u=(s??t.defaultColor).toUpperCase(),m=typeof o=="number"?Math.max(0,Math.min(1,o)):null;return {layout:n,size:i,sizeScale:He[i],font:r,fontFamily:Ne[r],resolvedColor:u,colorOverride:s,opacityMultiplier:c,opacityOverride:m,isInvisible:l,letterSpacing:d,lineHeight:h}},Je=/^#([0-9A-F]{3}|[0-9A-F]{4}|[0-9A-F]{6}|[0-9A-F]{8})$/i,Q=e=>e.length===1?e.repeat(2):e,N=e=>Number.parseInt(e,16),B=e=>!Number.isFinite(e)||e<=0?0:e>=1?1:e,ae=(e,t)=>{const n=Je.exec(e);if(!n)return e;const i=n[1];let r,s,a,o=1;i.length===3||i.length===4?(r=N(Q(i[0])),s=N(Q(i[1])),a=N(Q(i[2])),i.length===4&&(o=N(Q(i[3]))/255)):(r=N(i.slice(0,2)),s=N(i.slice(2,4)),a=N(i.slice(4,6)),i.length===8&&(o=N(i.slice(6,8))/255));const l=B(o*B(t));return `rgba(${r}, ${s}, ${a}, ${l})`},Ze=()=>({now:()=>typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}),oe=()=>Ze(),R=e=>e*1e3,Qe=e=>!Number.isFinite(e)||e<0?null:Math.round(e),St=4e3,Rt=1800,tn=3,en=.25,nn=32,rn=48,rt=120,sn=4e3,dt=120,an=800,on=2,j=4e3,W=4e3,_=W+St,ln=1e3,Ft=1,le=12,ce=24,V=.001,z=50,cn=5,dn=2,_t=8,hn=12,un=500,mn=3e3,Dt={debug:0,info:1,warn:2,error:3},pn=(e,t,n)=>{const i=[`[${t}]`,...n];switch(e){case "debug":console.debug(...i);break;case "info":console.info(...i);break;case "warn":console.warn(...i);break;case "error":console.error(...i);break;default:console.log(...i);}},de=(e,t={})=>{const{level:n="info",emitter:i=pn}=t,r=Dt[n],s=(a,o)=>{Dt[a]<r||i(a,e,o);};return {debug:(...a)=>s("debug",a),info:(...a)=>s("info",a),warn:(...a)=>s("warn",a),error:(...a)=>s("error",a)}},Ct=de("CommentEngine:Comment"),Ht=new WeakMap,gn=e=>{let t=Ht.get(e);return t||(t=new Map,Ht.set(e,t)),t},Mt=(e,t)=>{if(!e)return 0;const n=`${e.font??""}::${t}`,i=gn(e),r=i.get(n);if(r!==void 0)return r;const s=e.measureText(t).width;return i.set(n,s),s},fn=e=>{if(e.includes(`
`)){const t=e.split(/\r?\n/);return t.length>0?t:[""]}return [e]},Nt=e=>Math.max(24,e),ht=(e,t)=>{let n=0;const i=e.letterSpacing;for(const a of e.lines){const o=Mt(t,a),l=a.length>1?i*(a.length-1):0,d=Math.max(0,o+l);d>n&&(n=d);}e.width=n;const r=Math.max(1,Math.floor(e.fontSize*e.lineHeightMultiplier));e.lineHeightPx=r;const s=e.lines.length>1?(e.lines.length-1)*r:0;e.height=e.fontSize+s;},bn=(e,t,n,i,r)=>{try{if(!t)throw new Error("Canvas context is required");if(!Number.isFinite(n)||!Number.isFinite(i))throw new Error("Canvas dimensions must be numbers");if(!r)throw new Error("Prepare options are required");const s=Math.max(n,1),a=Nt(Math.floor(i*.05)),o=Nt(Math.floor(a*e.sizeScale));e.fontSize=o,t.font=`${e.fontSize}px ${e.fontFamily}`,e.lines=fn(e.text),ht(e,t);const l=!e.isScrolling&&(e.layout==="ue"||e.layout==="shita");if(l){const L=Math.max(1,s-_t*2);if(e.width>L){const H=Math.max(hn,Math.min(e.fontSize,Math.floor(a*.6))),ct=L/Math.max(e.width,1),q=Math.max(H,Math.floor(e.fontSize*Math.min(ct,1)));q<e.fontSize&&(e.fontSize=q,t.font=`${e.fontSize}px ${e.fontFamily}`,ht(e,t));let At=0;for(;e.width>L&&e.fontSize>H&&At<5;){const De=L/Math.max(e.width,1),It=Math.max(H,Math.floor(e.fontSize*Math.max(De,.7)));It>=e.fontSize?e.fontSize=Math.max(H,e.fontSize-1):e.fontSize=It,t.font=`${e.fontSize}px ${e.fontFamily}`,ht(e,t),At+=1;}}}if(!e.isScrolling){e.bufferWidth=0;const L=l?_t:0,H=Math.max((s-e.width)/2,L),ct=Math.max(L,s-e.width-L),q=Math.min(H,Math.max(ct,L));e.virtualStartX=q,e.x=q,e.baseSpeed=0,e.speed=0,e.speedPixelsPerMs=0,e.visibleDurationMs=W,e.preCollisionDurationMs=W,e.totalDurationMs=W,e.reservationWidth=e.width,e.staticExpiryTimeMs=e.vposMs+W,e.lastUpdateTime=e.getTimeSource().now(),e.isPaused=!1;return}e.staticExpiryTimeMs=null;const d=Mt(t,"??".repeat(150)),h=e.width*Math.max(r.bufferRatio,0);e.bufferWidth=Math.max(r.baseBufferPx,h);const c=Math.max(r.entryBufferPx,e.bufferWidth),u=e.scrollDirection,m=u==="rtl"?s+r.virtualExtension:-e.width-e.bufferWidth-r.virtualExtension,p=u==="rtl"?-e.width-e.bufferWidth-c:s+c,v=u==="rtl"?s+c:-c,x=u==="rtl"?m+e.width+e.bufferWidth:m-e.bufferWidth;e.virtualStartX=m,e.x=m,e.exitThreshold=p;const E=s>0?e.width/s:0,S=r.maxVisibleDurationMs===r.minVisibleDurationMs;let I=r.maxVisibleDurationMs;if(!S&&E>1){const L=Math.min(E,r.maxWidthRatio),H=r.maxVisibleDurationMs/Math.max(L,1);I=Math.max(r.minVisibleDurationMs,Math.floor(H));}const C=s+e.width+e.bufferWidth+c,Ie=Math.max(I,1),lt=C/Ie,Le=lt*1e3/60;e.baseSpeed=Le,e.speed=e.baseSpeed,e.speedPixelsPerMs=lt;const Re=Math.abs(p-m),Fe=u==="rtl"?Math.max(0,x-v):Math.max(0,v-x),Pt=Math.max(lt,Number.EPSILON);e.visibleDurationMs=I,e.preCollisionDurationMs=Math.max(0,Math.ceil(Fe/Pt)),e.totalDurationMs=Math.max(e.preCollisionDurationMs,Math.ceil(Re/Pt));const _e=e.width+e.bufferWidth+c;e.reservationWidth=Math.min(d,_e),e.lastUpdateTime=e.getTimeSource().now(),e.isPaused=!1;}catch(s){throw Ct.error("Comment.prepare",s,{text:e.text,visibleWidth:n,canvasHeight:i,hasContext:!!t}),s}},he=5,D={enabled:false,maxLogsPerCategory:he},it=new Map,yn=e=>{if(e===void 0||!Number.isFinite(e))return he;const t=Math.max(1,Math.floor(e));return Math.min(1e4,t)},vn=e=>{D.enabled=!!e.enabled,D.maxLogsPerCategory=yn(e.maxLogsPerCategory),D.enabled||it.clear();},O=()=>D.enabled,xn=e=>{const t=it.get(e)??0;return t>=D.maxLogsPerCategory?(t===D.maxLogsPerCategory&&(console.debug(`[CommentOverlay][${e}]`,"Further logs suppressed."),it.set(e,t+1)),false):(it.set(e,t+1),true)},M=(e,...t)=>{D.enabled&&xn(e)&&console.debug(`[CommentOverlay][${e}]`,...t);},X=(e,t=32)=>e.length<=t?e:`${e.slice(0,t)}…`,wn=(e,t)=>{D.enabled&&(console.group(`[CommentOverlay][state-dump] ${e}`),console.table({"Current Time":`${t.currentTime.toFixed(2)}ms`,Duration:`${t.duration.toFixed(2)}ms`,"Is Playing":t.isPlaying,"Epoch ID":t.epochId,"Total Comments":t.totalComments,"Active Comments":t.activeComments,"Reserved Lanes":t.reservedLanes,"Final Phase":t.finalPhaseActive,"Playback Begun":t.playbackHasBegun,"Is Stalled":t.isStalled}),console.groupEnd());},Sn=(e,t,n)=>{D.enabled&&M("epoch-change",`Epoch changed: ${e} → ${t} (reason: ${n})`);},w={hits:0,misses:0,creates:0,fallbacks:0,outlineCallsInCache:0,fillCallsInCache:0,outlineCallsInFallback:0,fillCallsInFallback:0,letterSpacingComments:0,normalComments:0,multiLineComments:0,totalCharactersDrawn:0,lastReported:0},Ot=()=>{if(!O())return;const e=performance.now();if(e-w.lastReported<=5e3)return;const t=w.hits+w.misses,n=t>0?w.hits/t*100:0,i=w.creates>0?(w.totalCharactersDrawn/w.creates).toFixed(1):"0",r=w.outlineCallsInCache+w.outlineCallsInFallback,s=w.fillCallsInCache+w.fillCallsInFallback;console.log("[TextureCache Stats]",`
...
`;class ot extends Tt{static FAB_HOST_ID="danime-settings-fab-host";settingsManager;fetcher;searcher;settings;playbackSettings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;activeTab="search";modalElement=null;overlayElement=null;closeButtonElement=null;fabElement=null;fabHostElement=null;fabShadowRoot=null;handleFabClick=t=>{t.preventDefault(),this.openSettingsModal();};handleOverlayClick=()=>{this.closeSettingsModal();};handleCloseClick=()=>{this.closeSettingsModal();};handlePlaybackSettingsChanged;constructor(t,n=new at,i=new kt){super(),this.settingsManager=t,this.fetcher=n,this.searcher=i,this.settings=this.settingsManager.getSettings(),this.playbackSettings=this.settingsManager.getPlaybackSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData(),this.handlePlaybackSettingsChanged=r=>{this.playbackSettings=r,this.applyPlaybackSettingsToUI();},this.settingsManager.addPlaybackObserver(this.handlePlaybackSettingsChanged);}insertIntoMypage(){const t=document.querySelector(k.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling));}addAutoCommentButtons(){}async waitMypageListStable(){}tryRestoreByDanimeIds(){return  false}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(st.getCommonStyles());const n=this.buildSettingsHtml();return this.setHTML(n),this.applySettingsToUI(),this.addStyles(te),this.setupEventListeners(),t}buildSettingsHtml(){const t=r=>typeof r=="number"?r.toLocaleString():"-",n=r=>{if(!r)return "-";try{return new Date(r).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return r}},i=this.currentVideoInfo;return `
