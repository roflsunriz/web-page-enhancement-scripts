...
(function () {
  ...
  const Q={debug:"debug",info:"info",warn:"warn",error:"error"},S=r=>{const t=`[${r}]`,e={};return Object.keys(Q).forEach(n=>{const i=Q[n];e[n]=(...o)=>{(console[i]??console.log)(t,...o);};}),e};function W(){return typeof unsafeWindow<"u"?unsafeWindow:window}const It={commentColor:"#FFFFFF",commentOpacity:.75,isCommentVisible:true,ngWords:[],ngRegexps:[]},L=It,x=()=>({...L,ngWords:[...L.ngWords],ngRegexps:[...L.ngRegexps]});var Et=typeof GM_addStyle<"u"?GM_addStyle:void 0,N=typeof GM_getValue<"u"?GM_getValue:void 0,z=typeof GM_setValue<"u"?GM_setValue:void 0,At=typeof GM_xmlhttpRequest<"u"?GM_xmlhttpRequest:void 0;const tt="settings",et="currentVideo",nt="lastDanimeIds",_t=r=>({...r,ngWords:[...r.ngWords],ngRegexps:[...r.ngRegexps]});class B{constructor(t){this.notifier=t,this.settings=x(),this.currentVideo=null,this.loadSettings(),this.currentVideo=this.loadVideoData();}settings;currentVideo;observers=new Set;getSettings(){return _t(this.settings)}loadSettings(){try{const t=N(tt,null);if(!t)return this.settings=x(),this.settings;if(typeof t=="string"){const e=JSON.parse(t);this.settings={...x(),...e,ngWords:Array.isArray(e?.ngWords)?[...e.ngWords]:[],ngRegexps:Array.isArray(e?.ngRegexps)?[...e.ngRegexps]:[]};}else this.settings={...x(),...t,ngWords:Array.isArray(t.ngWords)?[...t.ngWords]:[],ngRegexps:Array.isArray(t.ngRegexps)?[...t.ngRegexps]:[]};return this.notifyObservers(),this.settings}catch(t){return console.error("[SettingsManager] 設定の読み込みに失敗しました",t),this.notify("設定の読み込みに失敗しました","error"),this.settings=x(),this.settings}}saveSettings(){try{return z(tt,JSON.stringify(this.settings)),this.notifyObservers(),this.notify("設定を保存しました","success"),!0}catch(t){return console.error("[SettingsManager] 設定の保存に失敗しました",t),this.notify("設定の保存に失敗しました","error"),false}}updateSettings(t){return this.settings={...this.settings,...t,ngWords:t.ngWords?[...t.ngWords]:[...this.settings.ngWords],ngRegexps:t.ngRegexps?[...t.ngRegexps]:[...this.settings.ngRegexps]},this.saveSettings()}addObserver(t){this.observers.add(t);}removeObserver(t){this.observers.delete(t);}notifyObservers(){const t=this.getSettings();for(const e of this.observers)try{e(t);}catch(n){console.error("[SettingsManager] 設定変更通知でエラー",n);}}loadVideoData(){try{return N(et,null)??null}catch(t){return console.error("[SettingsManager] 動画データの読み込みに失敗しました",t),this.notify("動画データの読み込みに失敗しました","error"),null}}saveVideoData(t,e){try{const n={videoId:e.videoId,title:e.title,viewCount:e.viewCount,commentCount:e.commentCount,mylistCount:e.mylistCount,postedAt:e.postedAt,thumbnail:e.thumbnail,owner:e.owner??null,channel:e.channel??null};return z(et,n),this.currentVideo=n,!0}catch(n){return console.error("[SettingsManager] 動画データの保存に失敗しました",n),this.notify("動画データの保存に失敗しました","error"),false}}getCurrentVideo(){return this.currentVideo?{...this.currentVideo}:null}saveLastDanimeIds(t){try{return z(nt,t),!0}catch(e){return console.error("[SettingsManager] saveLastDanimeIds failed",e),this.notify("ID情報の保存に失敗しました","error"),false}}loadLastDanimeIds(){try{return N(nt,null)??null}catch(t){return console.error("[SettingsManager] loadLastDanimeIds failed",t),this.notify("ID情報の読込に失敗しました","error"),null}}notify(t,e="info"){this.notifier?.show(t,e);}}const D=S("dAnime:Comment");class Tt{text;vpos;commands;x=0;y=0;width=0;height=0;baseSpeed=0;speed=0;lane=-1;color;fontSize=0;fontFamily="Arial";opacity;isActive=false;hasShown=false;isPaused=false;lastUpdateTime=0;reservationWidth=0;constructor(t,e,n=[],i){if(typeof t!="string")throw new Error("Comment text must be a string");if(!Number.isFinite(e)||e<0)throw new Error("Comment vpos must be a non-negative number");this.text=t,this.vpos=e,this.commands=Array.isArray(n)?n:[],this.color=i.commentColor,this.opacity=i.commentOpacity;}prepare(t,e,n){try{if(!t)throw new Error("Canvas context is required");if(!Number.isFinite(e)||!Number.isFinite(n))throw new Error("Canvas dimensions must be numbers");this.fontSize=Math.max(24,Math.floor(n*.05)),t.font=`${this.fontSize}px ${this.fontFamily}`,this.width=t.measureText(this.text).width,this.height=this.fontSize;const i=t.measureText("あ".repeat(150)).width;this.reservationWidth=Math.min(i,this.width*5),this.x=e,this.baseSpeed=(e+this.reservationWidth)/720,this.speed=this.baseSpeed,this.lastUpdateTime=performance.now();}catch(i){throw D.error("Comment.prepare",i,{text:this.text,canvasWidth:e,canvasHeight:n,hasContext:!!t}),i}}update(t=1,e=false){try{if(!this.isActive||this.isPaused)return;const n=performance.now(),i=(n-this.lastUpdateTime)/(1e3/60);this.speed=this.baseSpeed*t,this.x-=this.speed*i,this.x<-this.width&&(this.isActive=!1),this.lastUpdateTime=n,this.isPaused=e;}catch(n){D.error("Comment.update",n,{text:this.text,playbackRate:t,isPaused:e,isActive:this.isActive});}}draw(t,e=null){try{if(!this.isActive||!t)return;t.save(),t.globalAlpha=this.opacity,t.font=`${this.fontSize}px ${this.fontFamily}`;const n=e??this.x,i=this.y+this.fontSize;t.strokeStyle="#000000",t.lineWidth=Math.max(3,this.fontSize/8),t.lineJoin="round",t.strokeText(this.text,n,i),t.fillStyle=this.color,t.fillText(this.text,n,i),t.restore();}catch(n){D.error("Comment.draw",n,{text:this.text,isActive:this.isActive,hasContext:!!t,interpolatedX:e});}}}const Vt=new Set(["INPUT","TEXTAREA"]),F=r=>r.length===1?r.toUpperCase():r,Mt=r=>r?`${r}+`:"";class ht{shortcuts=new Map;boundHandler;isEnabled=true;isListening=false;constructor(){this.boundHandler=this.handleKeyDown.bind(this);}addShortcut(t,e,n){const i=this.createShortcutKey(F(t),e);this.shortcuts.set(i,n);}removeShortcut(t,e){const n=this.createShortcutKey(F(t),e);this.shortcuts.delete(n);}startListening(){this.isListening||(document.addEventListener("keydown",this.boundHandler,false),this.isListening=true);}stopListening(){this.isListening&&(document.removeEventListener("keydown",this.boundHandler,false),this.isListening=false);}setEnabled(t){this.isEnabled=t;}createShortcutKey(t,e){return `${Mt(e)}${t}`}extractModifier(t){const e=[];return t.ctrlKey&&e.push("Ctrl"),t.altKey&&e.push("Alt"),t.shiftKey&&e.push("Shift"),t.metaKey&&e.push("Meta"),e.length>0?e.join("+"):null}handleKeyDown(t){if(!this.isEnabled)return;const n=t.target?.tagName??"";if(Vt.has(n))return;const i=this.extractModifier(t),o=this.createShortcutKey(F(t.key),i),s=this.shortcuts.get(o);s&&(t.preventDefault(),s());}}var Rt=(function(){if(typeof document>"u")return "transform";for(var r=["oTransform","msTransform","mozTransform","webkitTransform","transform"],t=document.createElement("div").style,e=0;e<r.length;e++)if(r[e]in t)return r[e];return "transform"})();function Lt(r){var t=document.createElement("div");if(t.style.cssText="position:absolute;",typeof r.render=="function"){var e=r.render();if(e instanceof HTMLElement)return t.appendChild(e),t}if(t.textContent=r.text,r.style)for(var n in r.style)t.style[n]=r.style[n];return t}function Nt(){var r=document.createElement("div");return r.style.cssText="overflow:hidden;white-space:nowrap;transform:translateZ(0);",r}function zt(r){for(var t=r.lastChild;t;)r.removeChild(t),t=r.lastChild;}function Dt(r,t,e){r.style.width=t+"px",r.style.height=e+"px";}function Ft(){}function Ot(r,t){var e=document.createDocumentFragment(),n=0,i=null;for(n=0;n<t.length;n++)i=t[n],i.node=i.node||Lt(i),e.appendChild(i.node);for(t.length&&r.appendChild(e),n=0;n<t.length;n++)i=t[n],i.width=i.width||i.node.offsetWidth,i.height=i.height||i.node.offsetHeight;}function Pt(r,t){t.node.style[Rt]="translate("+t.x+"px,"+t.y+"px)";}function qt(r,t){r.removeChild(t.node),this.media||(t.node=null);}var $t={name:"dom",init:Nt,clear:zt,resize:Dt,framing:Ft,setup:Ot,render:Pt,remove:qt},w=typeof window<"u"&&window.devicePixelRatio||1,O=Object.create(null);function Wt(r,t){if(O[r])return O[r];var e=12,n=/(\d+(?:\.\d+)?)(px|%|em|rem)(?:\s*\/\s*(\d+(?:\.\d+)?)(px|%|em|rem)?)?/,i=r.match(n);if(i){var o=i[1]*1||10,s=i[2],a=i[3]*1||1.2,l=i[4];s==="%"&&(o*=t.container/100),s==="em"&&(o*=t.container),s==="rem"&&(o*=t.root),l==="px"&&(e=a),l==="%"&&(e=o*a/100),l==="em"&&(e=o*a),l==="rem"&&(e=t.root*a),l===void 0&&(e=o*a);}return O[r]=e,e}function Bt(r,t){if(typeof r.render=="function"){var e=r.render();if(e instanceof HTMLCanvasElement)return r.width=e.width,r.height=e.height,e}var n=document.createElement("canvas"),i=n.getContext("2d"),o=r.style||{};o.font=o.font||"10px sans-serif",o.textBaseline=o.textBaseline||"bottom";var s=o.lineWidth*1;s=s>0&&s!==1/0?Math.ceil(s):!!o.strokeStyle*1,i.font=o.font,r.width=r.width||Math.max(1,Math.ceil(i.measureText(r.text).width)+s*2),r.height=r.height||Math.ceil(Wt(o.font,t))+s*2,n.width=r.width*w,n.height=r.height*w,i.scale(w,w);for(var a in o)i[a]=o[a];var l=0;switch(o.textBaseline){case "top":case "hanging":l=s;break;case "middle":l=r.height>>1;break;default:l=r.height-s;}return o.strokeStyle&&i.strokeText(r.text,s,l),i.fillText(r.text,s,l),n}function rt(r){return window.getComputedStyle(r,null).getPropertyValue("font-size").match(/(.+)px/)[1]*1}function Ht(r){var t=document.createElement("canvas");return t.context=t.getContext("2d"),t._fontSize={root:rt(document.getElementsByTagName("html")[0]),container:rt(r)},t}function Ut(r,t){r.context.clearRect(0,0,r.width,r.height);for(var e=0;e<t.length;e++)t[e].canvas=null;}function Gt(r,t,e){r.width=t*w,r.height=e*w,r.style.width=t+"px",r.style.height=e+"px";}function jt(r){r.context.clearRect(0,0,r.width,r.height);}function Kt(r,t){for(var e=0;e<t.length;e++){var n=t[e];n.canvas=Bt(n,r._fontSize);}}function Yt(r,t){r.context.drawImage(t.canvas,t.x*w,t.y*w);}function Xt(r,t){t.canvas=null;}var Jt={name:"canvas",init:Ht,clear:Ut,resize:Gt,framing:jt,setup:Kt,render:Yt,remove:Xt},it=(function(){if(typeof window<"u"){var r=window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame;if(r)return r.bind(window)}return function(t){return setTimeout(t,50/3)}})(),Zt=(function(){if(typeof window<"u"){var r=window.cancelAnimationFrame||window.mozCancelAnimationFrame||window.webkitCancelAnimationFrame;if(r)return r.bind(window)}return clearTimeout})();function ut(r,t,e){for(var n=0,i=0,o=r.length;i<o-1;)n=i+o>>1,e>=r[n][t]?i=n:o=n;return r[i]&&e<r[i][t]?i:o}function pt(r){return /^(ltr|top|bottom)$/i.test(r)?r.toLowerCase():"rtl"}function _(){var r=9007199254740991;return [{range:0,time:-r,width:r,height:0},{range:r,time:r,width:0,height:0}]}function mt(r){r.ltr=_(),r.rtl=_(),r.top=_(),r.bottom=_();}function R(){return typeof window.performance<"u"&&window.performance.now?window.performance.now():Date.now()}function Qt(r){var t=this,e=this.media?this.media.currentTime:R()/1e3,n=this.media?this.media.playbackRate:1;function i(f,g){if(g.mode==="top"||g.mode==="bottom")return e-f.time<t._.duration;var E=t._.width+f.width,C=E*(e-f.time)*n/t._.duration;if(f.width>C)return  true;var k=t._.duration+f.time-e,J=t._.width+g.width,A=t.media?g.time:g._utc,Z=J*(e-A)*n/t._.duration,Ct=t._.width-Z,kt=t._.duration*Ct/(t._.width+g.width);return k>kt}for(var o=this._.space[r.mode],s=0,a=0,l=1;l<o.length;l++){var c=o[l],d=r.height;if((r.mode==="top"||r.mode==="bottom")&&(d+=c.height),c.range-c.height-o[s].range>=d){a=l;break}i(c,r)&&(s=l);}var h=o[s].range,m={range:h+r.height,time:this.media?r.time:r._utc,width:r.width,height:r.height};return o.splice(s+1,a-s-1,m),r.mode==="bottom"?this._.height-r.height-h%this._.height:h%(this._.height-r.height)}function te(r,t,e,n){return function(i){r(this._.stage);var o=i||R(),s=o/1e3,a=this.media?this.media.currentTime:s,l=this.media?this.media.playbackRate:1,c=null,d=0,h=0;for(h=this._.runningList.length-1;h>=0;h--)c=this._.runningList[h],d=this.media?c.time:c._utc,a-d>this._.duration&&(n(this._.stage,c),this._.runningList.splice(h,1));for(var m=[];this._.position<this.comments.length&&(c=this.comments[this._.position],d=this.media?c.time:c._utc,!(d>=a));){if(a-d>this._.duration){++this._.position;continue}this.media&&(c._utc=s-(this.media.currentTime-c.time)),m.push(c),++this._.position;}for(t(this._.stage,m),h=0;h<m.length;h++)c=m[h],c.y=Qt.call(this,c),this._.runningList.push(c);for(h=0;h<this._.runningList.length;h++){c=this._.runningList[h];var f=this._.width+c.width,g=f*(s-c._utc)*l/this._.duration;c.mode==="ltr"&&(c.x=g-c.width),c.mode==="rtl"&&(c.x=this._.width-g),(c.mode==="top"||c.mode==="bottom")&&(c.x=this._.width-c.width>>1),e(this._.stage,c);}}}function H(){if(!this._.visible||!this._.paused)return this;if(this._.paused=false,this.media)for(var r=0;r<this._.runningList.length;r++){var t=this._.runningList[r];t._utc=R()/1e3-(this.media.currentTime-t.time);}var e=this,n=te(this._.engine.framing.bind(this),this._.engine.setup.bind(this),this._.engine.render.bind(this),this._.engine.remove.bind(this));function i(o){n.call(e,o),e._.requestID=it(i);}return this._.requestID=it(i),this}function U(){return !this._.visible||this._.paused?this:(this._.paused=true,Zt(this._.requestID),this._.requestID=0,this)}function G(){if(!this.media)return this;this.clear(),mt(this._.space);var r=ut(this.comments,"time",this.media.currentTime);return this._.position=Math.max(0,r-1),this}function ee(r){r.play=H.bind(this),r.pause=U.bind(this),r.seeking=G.bind(this),this.media.addEventListener("play",r.play),this.media.addEventListener("pause",r.pause),this.media.addEventListener("playing",r.play),this.media.addEventListener("waiting",r.pause),this.media.addEventListener("seeking",r.seeking);}function ne(r){this.media.removeEventListener("play",r.play),this.media.removeEventListener("pause",r.pause),this.media.removeEventListener("playing",r.play),this.media.removeEventListener("waiting",r.pause),this.media.removeEventListener("seeking",r.seeking),r.play=null,r.pause=null,r.seeking=null;}function re(r){this._={},this.container=r.container||document.createElement("div"),this.media=r.media,this._.visible=true,this.engine=(r.engine||"DOM").toLowerCase(),this._.engine=this.engine==="canvas"?Jt:$t,this._.requestID=0,this._.speed=Math.max(0,r.speed)||144,this._.duration=4,this.comments=r.comments||[],this.comments.sort(function(e,n){return e.time-n.time});for(var t=0;t<this.comments.length;t++)this.comments[t].mode=pt(this.comments[t].mode);return this._.runningList=[],this._.position=0,this._.paused=true,this.media&&(this._.listener={},ee.call(this,this._.listener)),this._.stage=this._.engine.init(this.container),this._.stage.style.cssText+="position:relative;pointer-events:none;",this.resize(),this.container.appendChild(this._.stage),this._.space={},mt(this._.space),(!this.media||!this.media.paused)&&(G.call(this),H.call(this)),this}function ie(){if(!this.container)return this;U.call(this),this.clear(),this.container.removeChild(this._.stage),this.media&&ne.call(this,this._.listener);for(var r in this)Object.prototype.hasOwnProperty.call(this,r)&&(this[r]=null);return this}var T=["mode","time","text","render","style"];function oe(r){if(!r||Object.prototype.toString.call(r)!=="[object Object]")return this;for(var t={},e=0;e<T.length;e++)r[T[e]]!==void 0&&(t[T[e]]=r[T[e]]);if(t.text=(t.text||"").toString(),t.mode=pt(t.mode),t._utc=R()/1e3,this.media){var n=0;t.time===void 0?(t.time=this.media.currentTime,n=this._.position):(n=ut(this.comments,"time",t.time),n<this._.position&&(this._.position+=1)),this.comments.splice(n,0,t);}else this.comments.push(t);return this}function se(){return this._.visible?this:(this._.visible=true,this.media&&this.media.paused?this:(G.call(this),H.call(this),this))}function ae(){return this._.visible?(U.call(this),this.clear(),this._.visible=false,this):this}function ce(){return this._.engine.clear(this._.stage,this._.runningList),this._.runningList=[],this}function le(){return this._.width=this.container.offsetWidth,this._.height=this.container.offsetHeight,this._.engine.resize(this._.stage,this._.width,this._.height),this._.duration=this._.width/this._.speed,this}var de={get:function(){return this._.speed},set:function(r){return typeof r!="number"||isNaN(r)||!isFinite(r)||r<=0?this._.speed:(this._.speed=r,this._.width&&(this._.duration=this._.width/r),r)}};function v(r){r&&re.call(this,r);}v.prototype.destroy=function(){return ie.call(this)};v.prototype.emit=function(r){return oe.call(this,r)};v.prototype.show=function(){return se.call(this)};v.prototype.hide=function(){return ae.call(this)};v.prototype.clear=function(){return ce.call(this)};v.prototype.resize=function(){return le.call(this)};Object.defineProperty(v.prototype,"speed",de);const y=S("dAnime:CommentRenderer"),V=r=>r*1e3,he="bold 24px 'MS PGothic', 'sans-serif'",ue="1px 1px 2px #000, -1px -1px 2px #000, 1px -1px 2px #000, -1px 1px 2px #000",pe=3,ot=2e3;class gt{_settings;danmaku=null;allComments=[];lastEmittedIndex=-1;canvas=null;videoElement=null;currentTime=0;duration=0;finalPhaseActive=false;keyboardHandler=null;constructor(t){this._settings=t?{...t}:x();}get settings(){return this._settings}set settings(t){this._settings={...t};}initialize(t){try{this.danmaku&&this.destroy(),this.videoElement=t,this.duration=V(t.duration);const e=document.createElement("div");e.style.position="absolute",e.style.pointerEvents="none",e.style.zIndex="1000";const n=t.getBoundingClientRect();e.style.width=`${n.width}px`,e.style.height=`${n.height}px`;const i=t.parentElement??document.body;i.style.position=i.style.position||"relative",i.appendChild(e),this.danmaku=new v({container:e,media:t,comments:[],engine:"canvas",speed:144}),this.canvas=e.querySelector("canvas"),this.setupVideoEventListeners(t),this.setupKeyboardShortcuts(),this.setupResizeListener();}catch(e){throw y.error("CommentRenderer.initialize",e),e}}addComment(t,e,n){if(this.isNGComment(t)||this.allComments.some(s=>s.text===t&&s.time===e/1e3))return null;const o={text:t,time:e/1e3,style:this.createCommentStyle(),commands:n?[...n]:void 0};return this.allComments.push(o),this.allComments.sort((s,a)=>s.time-a.time),o}clearComments(){this.allComments=[],this.danmaku?.clear();}destroy(){this.keyboardHandler?.stopListening(),this.keyboardHandler=null,this.danmaku?.destroy(),this.danmaku=null,this.videoElement=null;}updateSettings(t){const e=this.settings;this.settings=t,this.danmaku&&this.syncWithDanmaku(e);}getVideoElement(){return this.videoElement}getCommentsSnapshot(){return this.allComments.map(t=>({...t,style:{...t.style},commands:t.commands?[...t.commands]:void 0}))}isNGComment(t){try{return typeof t!="string"||Array.isArray(this._settings.ngWords)&&this._settings.ngWords.some(e=>typeof e=="string"&&e.length&&t.includes(e))?!0:Array.isArray(this._settings.ngRegexps)?this._settings.ngRegexps.some(e=>{try{return typeof e=="string"&&e.length?new RegExp(e).test(t):!1}catch(n){return y.error("CommentRenderer.isNGComment.regex",n,{pattern:e,text:t}),!1}}):!1}catch(e){return y.error("CommentRenderer.isNGComment",e,{text:t}),true}}updateComments(){const t=this.videoElement;if(!t||!this.danmaku)return;this.currentTime=V(t.currentTime);const e=this.duration>0&&this.duration-this.currentTime<=1e4;e&&!this.finalPhaseActive&&(this.finalPhaseActive=true,this.danmaku.clear()),!e&&this.finalPhaseActive&&(this.finalPhaseActive=false);const n=this.currentTime+2e3;for(let i=this.lastEmittedIndex+1;i<this.allComments.length;i++){const o=this.allComments[i];if(o.time*1e3<n)this.isNGComment(o.text)||this.danmaku.emit({...o,style:this.createCommentStyle(o.style)}),this.lastEmittedIndex=i;else break}}onSeek(){if(!this.videoElement||!this.danmaku)return;this.finalPhaseActive=false,this.currentTime=V(this.videoElement.currentTime),this.danmaku.clear();const t=this.videoElement.currentTime;let e=-1;for(let n=0;n<this.allComments.length;n++)if(this.allComments[n].time>=t){e=n-1;break}this.lastEmittedIndex=e;}resize(){this.danmaku&&this.danmaku.resize();}setupVideoEventListeners(t){try{t.addEventListener("seeking",()=>this.onSeek()),t.addEventListener("ratechange",()=>{this.danmaku&&(this.danmaku.speed=144*t.playbackRate);}),t.addEventListener("timeupdate",()=>this.updateComments());}catch(e){throw y.error("CommentRenderer.setupVideoEventListeners",e),e}}setupKeyboardShortcuts(){try{this.keyboardHandler=new ht,this.keyboardHandler.addShortcut("C","Shift",()=>{try{this._settings.isCommentVisible=!this._settings.isCommentVisible,this._settings.isCommentVisible?this.danmaku?.show():this.danmaku?.hide(),window.dAniRenderer?.settingsManager?.updateSettings(this._settings);}catch(t){y.error(t,"CommentRenderer.keyboardShortcut");}}),this.keyboardHandler.startListening();}catch(t){y.error(t,"CommentRenderer.setupKeyboardShortcuts");}}setupResizeListener(){try{window.addEventListener("resize",()=>{try{this.resize();}catch(t){y.error(t,"CommentRenderer.resize");}});}catch(t){y.error(t,"CommentRenderer.setupResizeListener");}}createCommentStyle(t){const e=Number.isFinite(this._settings.commentOpacity)?Math.max(0,Math.min(1,this._settings.commentOpacity)):1;return {font:t?.font??he,textShadow:t?.textShadow??ue,color:this._settings.commentColor,opacity:e.toFixed(pe)}}applySettingsToComments(){this.allComments=this.allComments.map(t=>({...t,style:this.createCommentStyle(t.style)}));}syncWithDanmaku(t){const e=this.danmaku;if(!e||(this.settings.isCommentVisible?e.show():e.hide(),!(t.commentColor!==this.settings.commentColor||t.commentOpacity!==this.settings.commentOpacity||!this.areNgListsEqual(t,this.settings))))return;this.applySettingsToComments();const i=this.videoElement,o=i?V(i.currentTime):0,s=o-ot;e.clear(),this.allComments.filter(l=>{const c=l.time*1e3;return c>o+ot||c<s?false:!this.isNGComment(l.text)}).forEach(l=>{e.emit({...l,style:this.createCommentStyle(l.style)});});}areNgListsEqual(t,e){const n=t.ngWords??[],i=e.ngWords??[],o=t.ngRegexps??[],s=e.ngRegexps??[];return n.length!==i.length||o.length!==s.length||n.some((c,d)=>c!==i[d])?false:!o.some((c,d)=>c!==s[d])}}class j{shadowRoot=null;container=null;createShadowDOM(t,e={mode:"closed"}){if(!t)throw new Error("Host element is required for shadow DOM creation");return this.shadowRoot=t.attachShadow(e),this.container=document.createElement("div"),this.shadowRoot.appendChild(this.container),this.shadowRoot}addStyles(t){if(!this.shadowRoot)throw new Error("Shadow root not initialized");const e=document.createElement("style");e.textContent=t,this.shadowRoot.appendChild(e);}querySelector(t){return this.shadowRoot?this.shadowRoot.querySelector(t):null}querySelectorAll(t){return this.shadowRoot?this.shadowRoot.querySelectorAll(t):document.createDocumentFragment().childNodes}setHTML(t){if(!this.container)throw new Error("Container not initialized");this.container.innerHTML=t;}destroy(){this.shadowRoot?.host&&this.shadowRoot.host.remove(),this.shadowRoot=null,this.container=null;}}const me=`\r
/* 基本カラーパレット - ダークモード */\r
:host {\r
...
`;class M{static getCommonStyles(){return me}static getNotificationStyles(){return ge}static getAutoButtonStyles(){return fe}}const st={success:"✔",warning:"⚠",error:"✖",info:"ℹ"};class p extends j{static instance=null;static notifications=[];hostElement=null;initialized=false;static getInstance(){return this.instance||(this.instance=new p),this.instance}static show(t,e="info",n=3e3){try{const i=this.getInstance();return i.initialize(),i.initialized?i.createNotification(t,e,n):null}catch(i){return console.error("[NotificationManager] show failed",i),null}}static removeNotification(t){this.getInstance().removeNotification(t.element);}show(t,e="info"){p.show(t,e);}initialize(){if(!this.initialized)try{this.hostElement=document.createElement("div"),this.hostElement.className="nico-comment-shadow-host notification-host",this.hostElement.style.cssText=["position: fixed","top: 0","right: 0","z-index: 10000","pointer-events: none"].join(";"),document.body.appendChild(this.hostElement),this.createShadowDOM(this.hostElement),this.addStyles(M.getNotificationStyles()),this.setHTML('<div class="notification-container"></div>'),this.initialized=!0;}catch(t){console.error("[NotificationManager] initialize failed",t),this.initialized=false;}}destroy(){p.notifications.forEach(t=>{t.timerId&&window.clearTimeout(t.timerId);}),p.notifications=[],this.hostElement?.parentNode&&this.hostElement.parentNode.removeChild(this.hostElement),this.hostElement=null,this.initialized=false,p.instance=null;}createNotification(t,e,n){try{const i=this.querySelector(".notification-container");if(!i)throw new Error("Notification container not found");const o=st[e]??st.info,s=document.createElement("div");s.className=`notification-item ${e}`;const a=document.createElement("div");a.className="notification-icon",a.innerHTML=`<span>${o}</span>`,s.appendChild(a);const l=document.createElement("div");l.className="notification-content";const c=document.createElement("div");c.className="notification-type",c.textContent=e.charAt(0).toUpperCase()+e.slice(1),l.appendChild(c);const d=document.createElement("div");d.className="notification-message",d.textContent=t||"No message",l.appendChild(d),s.appendChild(l);const h=document.createElement("button");h.className="notification-close",h.innerHTML="&times;",h.addEventListener("click",()=>{this.removeNotification(s);}),s.appendChild(h),i.appendChild(s),requestAnimationFrame(()=>{s.classList.add("show");});const m={element:s,timerId:window.setTimeout(()=>{this.removeNotification(s);},n)};return p.notifications.push(m),m}catch(i){return console.error("[NotificationManager] createNotification failed",i),null}}removeNotification(t){if(!t)return;const e=p.notifications.find(n=>n.element===t);e?.timerId&&window.clearTimeout(e.timerId),t.classList.remove("show"),window.setTimeout(()=>{try{t.remove(),p.notifications=p.notifications.filter(n=>n.element!==t);}catch(n){console.error("[NotificationManager] removeNotification cleanup failed",n);}},500);}}const ft="https://www.nicovideo.jp",bt=`${ft}/search`,be=`${ft}/watch`,ye={searchBase:bt},xe=r=>`${be}/${r}`,yt=r=>`${bt}/${encodeURIComponent(r)}`,$=r=>new Promise((t,e)=>{At({url:r.url,method:r.method??"GET",headers:r.headers,data:r.data,responseType:r.responseType??"text",timeout:r.timeout,onprogress:r.onprogress,onload:n=>{t({status:n.status,statusText:n.statusText,response:n.response,finalUrl:n.finalUrl,headers:n.responseHeaders});},onerror:n=>{const i=n?.error??"unknown error";e(new Error(`GM_xmlhttpRequest failed: ${i}`));},ontimeout:()=>{e(new Error("GM_xmlhttpRequest timeout"));}});}),P=S("dAnime:NicoApiFetcher");class K{apiData=null;comments=null;get lastApiData(){return this.apiData}get cachedComments(){return this.comments}async fetchApiData(t){try{const e=this.sanitizeVideoId(t),i=(await $({method:"GET",url:xe(e),headers:{Accept:"text/html"}})).response,a=new DOMParser().parseFromString(i,"text/html").querySelector('meta[name="server-response"]');if(!a)throw new Error("API data element not found in response");const l=a.getAttribute("content");if(!l)throw new Error("API data content is empty");const c=this.decodeServerResponse(l),h=JSON.parse(c).data?.response;if(!h)throw new Error("Invalid API data structure");return this.apiData=h,h}catch(e){throw P.error("NicoApiFetcher.fetchApiData",e),e}}async fetchComments(){try{if(!this.apiData)throw new Error("API data must be fetched first");const t=this.apiData.comment?.nvComment;if(!t?.server||!t.params||!t.threadKey)throw new Error("Required comment server data is missing");const e=await $({method:"POST",url:`${t.server}/v1/threads`,headers:{"Content-Type":"application/json","x-client-os-type":"others","X-Frontend-Id":"6","X-Frontend-Version":"0"},data:JSON.stringify({params:t.params,threadKey:t.threadKey,additionals:{}})}),o=(JSON.parse(e.response).data?.threads??[]).filter(a=>a.fork==="main").sort((a,l)=>(l.commentCount||0)-(a.commentCount||0))[0];if(!o)throw new Error("Main thread not found in comment response");const s=(o.comments??[]).map(a=>({text:a.body??"",vpos:a.vposMs??0,commands:a.commands??[]}));return this.comments=s,s}catch(t){throw P.error("NicoApiFetcher.fetchComments",t),t}}async fetchAllData(t){return await this.fetchApiData(t),this.fetchComments()}sanitizeVideoId(t){try{let e=encodeURIComponent(t);return e=e.replace(/%([0-9A-F]{2})/gi,(n,i)=>{const o=parseInt(i,16);return o>=65&&o<=90||o>=97&&o<=122||o>=48&&o<=57||o===45||o===95||o===46||o===126?String.fromCharCode(o):n}),e}catch(e){return P.error("NicoApiFetcher.sanitizeVideoId",e,{videoId:t}),t}}decodeServerResponse(t){try{return decodeURIComponent(t)}catch(e){try{const n=t.replace(/%(?![0-9A-F]{2})/gi,"%25");return decodeURIComponent(n)}catch{throw new Error(`API data decode failed: ${e.message}`)}}}}const at=S("dAnime:NicoVideoSearcher");class xt{cache=new Map;async search(t){if(!t.trim())return [];if(this.cache.has(t))return this.cache.get(t)??[];const e=yt(t),n=await this.fetchText(e),i=this.parseServerContext(n),o=[],s=new Set;for(const a of i)a?.videoId&&(s.has(a.videoId)||(s.add(a.videoId),o.push(a)));return o.sort((a,l)=>l.viewCount-a.viewCount),this.cache.set(t,o),o}async fetchText(t){return (await $({method:"GET",url:t})).response}parseServerContext(t){try{const n=new DOMParser().parseFromString(t,"text/html").querySelector('meta[name="server-response"]');if(!n)return [];const i=n.getAttribute("content")??"",o=this.decodeHtmlEntities(i);let s;try{s=JSON.parse(o);}catch(a){return at.error("NicoVideoSearcher.parseServerContext",a),[]}return this.extractVideoItems(s??{})}catch(e){return at.error("NicoVideoSearcher.parseServerContext",e),[]}}decodeHtmlEntities(t){if(!t)return "";let e=t.replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">");return e=e.replace(/&#(\d+);/g,(n,i)=>String.fromCharCode(parseInt(i,10))),e=e.replace(/&#x([0-9a-fA-F]+);/g,(n,i)=>String.fromCharCode(parseInt(i,16))),e}extractVideoItems(t){const e=[],n=o=>{const s=(o?.id??o?.contentId??o?.watchId??"").toString();if(!s)return;const a=(o?.title??o?.shortTitle??"").toString(),l=o?.count??{},c=Number(l.view??o?.viewCounter??0)||0,d=Number(l.comment??o?.commentCounter??0)||0,h=Number(l.mylist??o?.mylistCounter??0)||0,m=o?.thumbnail??{},f=(m.nHdUrl||m.listingUrl||m.largeUrl||m.middleUrl||m.url||o?.thumbnailUrl||"").toString(),g=(o?.registeredAt||o?.startTime||o?.postedDateTime||"")?.toString?.()??"",E=o?.owner&&typeof o.owner=="object"?{nickname:(o.owner.nickname??o.owner.name??"")||void 0,name:(o.owner.name??o.owner.nickname??"")||void 0}:null,C=(o?.isChannelVideo||o?.owner?.ownerType==="channel")&&o?.owner?{name:o.owner.name??""}:null;a&&e.push({videoId:s,title:a,viewCount:c,commentCount:d,mylistCount:h,thumbnail:f,postedAt:g,owner:E,channel:C});},i=o=>{if(!o)return;if(Array.isArray(o)){o.forEach(i);return}if(typeof o!="object"||o===null)return;const s=o;(s.id||s.contentId||s.watchId)&&n(s),Object.values(o).forEach(i);};return i(t),e}}class Y{delay;timers=new Map;funcIds=new Map;nextId=1;constructor(t){this.delay=t;}getFuncId(t){return this.funcIds.has(t)||this.funcIds.set(t,this.nextId++),this.funcIds.get(t)??0}exec(t){const e=this.getFuncId(t),n=Date.now(),i=this.timers.get(e)?.lastExec??0,o=n-i;if(o>this.delay)t(),this.timers.set(e,{lastExec:n});else {clearTimeout(this.timers.get(e)?.timerId??void 0);const s=setTimeout(()=>{t(),this.timers.set(e,{lastExec:Date.now()});},this.delay-o);this.timers.set(e,{timerId:s,lastExec:i});}}execOnce(t){const e=this.getFuncId(t),n=this.timers.get(e);if(n?.executedOnce){n.timerId&&clearTimeout(n.timerId);return}n?.timerId&&clearTimeout(n.timerId);const i=setTimeout(()=>{try{t();}finally{this.timers.set(e,{executedOnce:true,lastExec:Date.now(),timerId:null});}},this.delay);this.timers.set(e,{timerId:i,executedOnce:false,scheduled:true});}cancel(t){const e=this.getFuncId(t),n=this.timers.get(e);n?.timerId&&clearTimeout(n.timerId),this.timers.delete(e);}resetExecution(t){const e=this.getFuncId(t),n=this.timers.get(e);n&&(n.timerId&&clearTimeout(n.timerId),this.timers.set(e,{executedOnce:false,scheduled:false}));}clearAll(){for(const[,t]of this.timers)t.timerId&&clearTimeout(t.timerId);this.timers.clear(),this.funcIds.clear();}}const we=1e3,ve=100,Se=30,Ce=/watch\/(?:([a-z]{2}))?(\d+)/gi,ct=r=>{if(!r?.video)return null;const t=r.video.registeredAt,e=t?new Date(t).toLocaleString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false}):void 0;return {videoId:r.video.id,title:r.video.title,viewCount:r.video.count?.view,mylistCount:r.video.count?.mylist,commentCount:r.video.count?.comment,postedAt:e,thumbnail:r.video.thumbnail?.url,owner:r.owner??null,channel:r.channel??null}},ke=r=>{const t=[];let e;for(;(e=Ce.exec(r))!==null;){const[,n="",i=""]=e;i&&t.push(`${n}${i}`);}return t};class wt{constructor(t,e,n,i=we,o=ve){this.renderer=t,this.fetcher=e,this.settingsManager=n,this.monitorInterval=i,this.debounce=new Y(o);}nextVideoId=null;preloadedComments=null;lastPreloadedComments=null;lastVideoId=null;checkIntervalId=null;isSwitching=false;debounce;startMonitoring(){this.stopMonitoring(),this.checkIntervalId=window.setInterval(()=>{this.checkVideoEnd();},this.monitorInterval);}stopMonitoring(){this.checkIntervalId&&(window.clearInterval(this.checkIntervalId),this.checkIntervalId=null);}async onVideoSwitch(t){if(!this.isSwitching){this.isSwitching=true;try{const e=this.preloadedComments??this.lastPreloadedComments??null,n=t?.dataset?.videoId??t?.getAttribute?.("data-video-id")??null,i=this.nextVideoId??n??this.lastVideoId;if(!t||!i&&!e){this.handleMissingVideoInfo(e);return}p.show("動画の切り替わりを検知しました...","info");let o=null;i&&(o=await this.fetchVideoApiData(i,e),o&&(this.persistVideoMetadata(o),this.lastVideoId=i)),this.renderer.clearComments();const s=await this.populateComments(i,e);if(s===0&&p.show("コメントを取得できませんでした","warning"),this.nextVideoId=null,this.preloadedComments=null,o){const a=ct(o);if(a){const l=`コメントソースを更新しました: ${a.title??"不明なタイトル"}（${s}件）`;p.show(l,s>0?"success":"warning");}}}catch(e){console.error("[VideoSwitchHandler] 動画切り替え中にエラーが発生しました",e),p.show(`動画切り替えエラー: ${e.message}`,"error"),this.renderer.clearComments();}finally{this.isSwitching=false;}}}async checkVideoEnd(){const t=this.renderer.getVideoElement();if(!(!t||!Number.isFinite(t.duration)||t.duration-t.currentTime>Se)){if(!this.nextVideoId){const n=async()=>{await this.findNextVideoId();};this.debounce.execOnce(n);}if(this.nextVideoId&&!this.preloadedComments){const n=async()=>{await this.preloadComments();};this.debounce.execOnce(n);}}}handleMissingVideoInfo(t){t||(this.renderer.clearComments(),p.show("次の動画のコメントを取得できませんでした。コメント表示をクリアします。","warning"));}async fetchVideoApiData(t,e){try{return await this.fetcher.fetchApiData(t)}catch(n){if(console.error("[VideoSwitchHandler] API取得エラー",n),!e)throw n;return null}}persistVideoMetadata(t){const e=ct(t);e&&this.settingsManager.saveVideoData(e.title??"",e);}async populateComments(t,e){let n=null;if(Array.isArray(e)&&e.length>0)n=e;else if(t)try{n=await this.fetcher.fetchAllData(t);}catch(o){console.error("[VideoSwitchHandler] コメント取得エラー",o),p.show(`コメント取得エラー: ${o.message}`,"error"),n=null;}if(!n||n.length===0)return 0;const i=n.filter(o=>!this.renderer.isNGComment(o.text));return i.forEach(o=>{this.renderer.addComment(o.text,o.vpos,o.commands);}),this.lastPreloadedComments=[...i],i.length}async findNextVideoId(){try{const t=this.fetcher.lastApiData;if(!t)return;const e=t.series?.video?.next?.id;if(e){this.nextVideoId=e;return}const n=t.video?.description??"";if(!n)return;const i=ke(n);if(i.length===0)return;const o=[...i].sort((s,a)=>{const l=parseInt(s.replace(/^[a-z]{2}/i,""),10)||0;return (parseInt(a.replace(/^[a-z]{2}/i,""),10)||0)-l});this.nextVideoId=o[0]??null;}catch(t){console.error("[VideoSwitchHandler] 次の動画ID取得エラー",t);}}async preloadComments(){if(this.nextVideoId)try{const e=(await this.fetcher.fetchAllData(this.nextVideoId)).filter(n=>!this.renderer.isNGComment(n.text));this.preloadedComments=e.length>0?e:null;}catch(t){console.error("[VideoSwitchHandler] コメントプリロードエラー",t),this.preloadedComments=null;}}}const Ie=`
.nico-comment-shadow-host {
  display: block;
  position: relative;
}
`;class vt{static initialize(){Et(Ie);}}var Ee="M9,22A1,1 0 0,1 8,21V18H4A2,2 0 0,1 2,16V4C2,2.89 2.9,2 4,2H20A2,2 0 0,1 22,4V16A2,2 0 0,1 20,18H13.9L10.2,21.71C10,21.9 9.75,22 9.5,22V22H9M10,16V19.08L13.08,16H20V4H4V16H10Z",Ae="M12,17A2,2 0 0,0 14,15C14,13.89 13.1,13 12,13A2,2 0 0,0 10,15A2,2 0 0,0 12,17M18,8A2,2 0 0,1 20,10V20A2,2 0 0,1 18,22H6A2,2 0 0,1 4,20V10C4,8.89 4.9,8 6,8H7V6A5,5 0 0,1 12,1A5,5 0 0,1 17,6V8H18M12,3A3,3 0 0,0 9,6V8H15V6A3,3 0 0,0 12,3Z",_e="M17.5,12A1.5,1.5 0 0,1 16,10.5A1.5,1.5 0 0,1 17.5,9A1.5,1.5 0 0,1 19,10.5A1.5,1.5 0 0,1 17.5,12M14.5,8A1.5,1.5 0 0,1 13,6.5A1.5,1.5 0 0,1 14.5,5A1.5,1.5 0 0,1 16,6.5A1.5,1.5 0 0,1 14.5,8M9.5,8A1.5,1.5 0 0,1 8,6.5A1.5,1.5 0 0,1 9.5,5A1.5,1.5 0 0,1 11,6.5A1.5,1.5 0 0,1 9.5,8M6.5,12A1.5,1.5 0 0,1 5,10.5A1.5,1.5 0 0,1 6.5,9A1.5,1.5 0 0,1 8,10.5A1.5,1.5 0 0,1 6.5,12M12,3A9,9 0 0,0 3,12A9,9 0 0,0 12,21A1.5,1.5 0 0,0 13.5,19.5C13.5,19.11 13.35,18.76 13.11,18.5C12.88,18.23 12.73,17.88 12.73,17.5A1.5,1.5 0 0,1 14.23,16H16A5,5 0 0,0 21,11C21,6.58 16.97,3 12,3Z";function X(r,t=24){const e=String(t),n=String(t);return `<svg xmlns="http://www.w3.org/2000/svg" width="${e}" height="${n}" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true" focusable="false"><path d="${r}"></path></svg>`}const Te=X(_e),lt=X(Ae),Ve=X(Ee),b={mypageHeaderTitle:".p-mypageHeader__title",mypageListContainer:".p-mypageMain",watchVideoElement:"video#video",mypageItem:".itemModule.list",mypageItemTitle:".line1",mypageEpisodeNumber:".number.line1 span",mypageEpisodeTitle:".episode.line1 span"},I=S("dAnime:SettingsUI"),u={searchInput:"#searchInput",searchButton:"#searchButton",openSearchPage:"#openSearchPageDirect",searchResults:"#searchResults",saveButton:"#saveSettings",opacitySelect:"#commentOpacity",visibilityToggle:"#commentVisibilityToggle",currentTitle:"#currentTitle",currentVideoId:"#currentVideoId",currentOwner:"#currentOwner",currentViewCount:"#currentViewCount",currentCommentCount:"#currentCommentCount",currentMylistCount:"#currentMylistCount",currentPostedAt:"#currentPostedAt",currentThumbnail:"#currentThumbnail",colorValue:".color-value",colorPreview:".color-preview",colorPicker:"#colorPicker",colorPickerInput:"#colorPickerInput",openColorPicker:"#openColorPicker",ngWords:"#ngWords",ngRegexps:"#ngRegexps",showNgWords:"#showNgWords",showNgRegexps:"#showNgRegexp",playCurrentVideo:"#playCurrentVideo"};class St extends j{settingsManager;fetcher;searcher;settings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;constructor(t,e=new K,n=new xt){super(),this.settingsManager=t,this.fetcher=e,this.searcher=n,this.settings=this.settingsManager.getSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData();}insertIntoMypage(){const t=document.querySelector(b.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}}));}addAutoCommentButtons(){document.querySelectorAll(b.mypageItem).forEach(e=>{const n=e.querySelector(b.mypageItemTitle);if(!n||n.querySelector(".auto-comment-button-host"))return;const i=n.querySelector("span")?.textContent?.trim()??"",o=e.querySelector(b.mypageEpisodeNumber)?.textContent?.trim()??"",s=e.querySelector(b.mypageEpisodeTitle)?.textContent?.trim()??"",a=document.createElement("div");a.className="nico-comment-shadow-host auto-comment-button-host",a.style.cssText="position:absolute;left:150px;top:3px;margin-left:8px;";const l=a.attachShadow({mode:"closed"}),c=document.createElement("style");c.textContent=M.getAutoButtonStyles(),l.appendChild(c);const d=document.createElement("button");d.className="auto-comment-button",d.innerHTML=Ve,d.setAttribute("aria-label","コメント設定"),d.setAttribute("title","コメント設定"),d.setAttribute("type","button"),d.addEventListener("click",async h=>{h.preventDefault(),h.stopPropagation(),h.stopImmediatePropagation();try{const m=[i,o,s].filter(Boolean).join(" ");this.scrollToSettings(),this.setSearchKeyword(m),this.lastAutoButtonElement=a;try{const g=e.querySelector('input[name="workId"]')?.value?.trim()??"",E=e.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),C=e.querySelector('a.textContainer[href*="partId="]');let k="";const A=(E?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);A?k=A[1]:C?.href&&(k=(new URL(C.href,location.origin).searchParams.get("partId")??"").trim()),g&&k&&this.settingsManager.saveLastDanimeIds({workId:g,partId:k});}catch(g){console.warn("[SettingsUI] save (workId, partId) skipped:",g);}const f=await this.executeSearch(m);if(f.length===0)return;await this.applySearchResult(f[0]);}catch(m){I.error("SettingsUI.autoCommentButton",m);}}),l.appendChild(d),n.appendChild(a),this.lastAutoButtonElement=a;}),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}});}async waitMypageListStable(){const t=document.querySelector(b.mypageListContainer);if(!t)return;let e=t.querySelectorAll(b.mypageItem).length;const n=Date.now()+1500;return new Promise(i=>{const o=new MutationObserver(()=>{const s=t.querySelectorAll(b.mypageItem).length;if(s!==e){e=s;return}Date.now()>=n&&(o.disconnect(),i());});o.observe(t,{childList:true,subtree:true}),setTimeout(()=>{try{o.disconnect();}catch(s){I.debug("waitMypageListStable: observer already disconnected",s);}i();},1600);})}tryRestoreByDanimeIds(){const t=this.settingsManager.loadLastDanimeIds();if(!t)return  false;const e=Array.from(document.querySelectorAll(b.mypageItem));for(const n of e){if(n.querySelector('input[name="workId"]')?.value?.trim()!==t.workId)continue;const o=n.querySelector('a.textContainer[href*="partId="]'),s=o?.href?(new URL(o.href,location.origin).searchParams.get("partId")??"")===t.partId:false,a=n.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),l=(()=>{const d=(a?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);return !!d&&d[1]===t.partId})();if(s||l){const c=n.querySelector(".auto-comment-button-host")??n;return this.lastAutoButtonElement=c,this.updatePlayButtonState(this.currentVideoInfo),true}}return  false}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(M.getCommonStyles());const e=this.buildSettingsHtml();return this.setHTML(e),this.applySettingsToUI(),this.setupEventListeners(),t}buildSettingsHtml(){const t=i=>typeof i=="number"?i.toLocaleString():"-",e=i=>{if(!i)return "-";try{return new Date(i).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return i}},n=this.currentVideoInfo;return `
        ...
        <div class="setting-group display-settings-group">
          ...
          <div class="color-picker-container">
              <button id="openColorPicker" class="color-picker-button" type="button">
                <span class="color-picker-button__icon" aria-hidden="true">${Te}</span>
        ...
        <div class="setting-group">
          ...
          <div class="ng-words-container">
            <button id="showNgWords" class="mask-button" type="button">
              <span class="mask-button__icon" aria-hidden="true">${lt}</span>
