...
`;class st extends wt{static FAB_HOST_ID="danime-settings-fab-host";settingsManager;fetcher;searcher;settings;playbackSettings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;activeTab="search";modalElement=null;overlayElement=null;closeButtonElement=null;fabElement=null;fabHostElement=null;fabShadowRoot=null;handleFabClick=t=>{t.preventDefault(),this.openSettingsModal();};handleOverlayClick=()=>{this.closeSettingsModal();};handleCloseClick=()=>{this.closeSettingsModal();};handlePlaybackSettingsChanged;constructor(t,e=new Ct,n=new he){super(),this.settingsManager=t,this.fetcher=e,this.searcher=n,this.settings=this.settingsManager.getSettings(),this.playbackSettings=this.settingsManager.getPlaybackSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData(),this.handlePlaybackSettingsChanged=i=>{this.playbackSettings=i,this.applyPlaybackSettingsToUI();},this.settingsManager.addPlaybackObserver(this.handlePlaybackSettingsChanged);}insertIntoMypage(){const t=document.querySelector(V.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}}));}addAutoCommentButtons(){document.querySelectorAll(V.mypageItem).forEach(e=>{const n=e.querySelector(V.mypageItemTitle);if(!n||n.querySelector(".auto-comment-button-host"))return;const i=n.querySelector("span")?.textContent?.trim()??"",r=e.querySelector(V.mypageEpisodeNumber)?.textContent?.trim()??"",s=e.querySelector(V.mypageEpisodeTitle)?.textContent?.trim()??"",a=document.createElement("div");a.className="nico-comment-shadow-host auto-comment-button-host",a.style.cssText="position:absolute;left:150px;top:3px;margin-left:8px;";const l=a.attachShadow({mode:"closed"}),h=document.createElement("style");h.textContent=J.getAutoButtonStyles(),l.appendChild(h);const d=document.createElement("button");d.className="auto-comment-button",d.innerHTML=gt,d.setAttribute("aria-label","コメント設定"),d.setAttribute("title","コメント設定"),d.setAttribute("type","button"),d.addEventListener("click",async u=>{u.preventDefault(),u.stopPropagation(),u.stopImmediatePropagation();try{const c=[i,r,s].filter(Boolean).join(" ");this.openSettingsModal(!1),this.setSearchKeyword(c),this.lastAutoButtonElement=a;try{const p=e.querySelector('input[name="workId"]')?.value?.trim()??"",f=e.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),g=e.querySelector('a.textContainer[href*="partId="]');let y="";const w=(f?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);w?y=w[1]:g?.href&&(y=(new URL(g.href,location.origin).searchParams.get("partId")??"").trim()),p&&y&&this.settingsManager.saveLastDanimeIds({workId:p,partId:y});}catch(p){console.warn("[SettingsUI] save (workId, partId) skipped:",p);}const m=await this.executeSearch(c);if(m.length===0)return;await this.applySearchResult(m[0]);}catch(c){U.error("SettingsUI.autoCommentButton",c);}}),l.appendChild(d),n.appendChild(a),this.lastAutoButtonElement=a;}),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}});}async waitMypageListStable(){const t=document.querySelector(V.mypageListContainer);if(!t)return;let e=t.querySelectorAll(V.mypageItem).length;const n=Date.now()+1500;return new Promise(i=>{const r=new MutationObserver(()=>{const s=t.querySelectorAll(V.mypageItem).length;if(s!==e){e=s;return}Date.now()>=n&&(r.disconnect(),i());});r.observe(t,{childList:true,subtree:true}),setTimeout(()=>{try{r.disconnect();}catch(s){U.debug("waitMypageListStable: observer already disconnected",s);}i();},1600);})}tryRestoreByDanimeIds(){const t=this.settingsManager.loadLastDanimeIds();if(!t)return  false;const e=Array.from(document.querySelectorAll(V.mypageItem));for(const n of e){if(n.querySelector('input[name="workId"]')?.value?.trim()!==t.workId)continue;const r=n.querySelector('a.textContainer[href*="partId="]'),s=r?.href?(new URL(r.href,location.origin).searchParams.get("partId")??"")===t.partId:false,a=n.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),l=(()=>{const d=(a?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);return !!d&&d[1]===t.partId})();if(s||l){const h=n.querySelector(".auto-comment-button-host")??n;return this.lastAutoButtonElement=h,this.updatePlayButtonState(this.currentVideoInfo),true}}return  false}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(J.getCommonStyles());const e=this.buildSettingsHtml();return this.setHTML(e),this.applySettingsToUI(),this.addStyles(Xt),this.setupEventListeners(),t}buildSettingsHtml(){const t=i=>typeof i=="number"?i.toLocaleString():"-",e=i=>{if(!i)return "-";try{return new Date(i).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return i}},n=this.currentVideoInfo;return `
      <div class="nico-comment-settings">
        <h2>
          <span class="settings-title">d-anime-nico-comment-renderer</span>
          <span class="version-badge" aria-label="バージョン">${an}</span>
        </h2>
        ...
        <div class="settings-modal__content">
          ...
          <div class="settings-modal__tabs" role="tablist">
            ...
            <button class="settings-modal__tab" type="button" data-tab="display" role="tab" aria-selected="false" aria-controls="settingsPaneDisplay" id="settingsTabDisplay" tabindex="-1">
              <span class="settings-modal__tab-icon" aria-hidden="true">${zn}</span>
