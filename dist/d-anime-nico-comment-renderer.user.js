...
`;class N extends Q{static FAB_HOST_ID="danime-settings-fab-host";settingsManager;fetcher;searcher;settings;currentVideoInfo;hostElement=null;lastAutoButtonElement=null;activeTab="search";modalElement=null;overlayElement=null;closeButtonElement=null;fabElement=null;fabHostElement=null;fabShadowRoot=null;handleFabClick=t=>{t.preventDefault(),this.openSettingsModal();};handleOverlayClick=()=>{this.closeSettingsModal();};handleCloseClick=()=>{this.closeSettingsModal();};constructor(t,e=new tt,n=new Rt){super(),this.settingsManager=t,this.fetcher=e,this.searcher=n,this.settings=this.settingsManager.getSettings(),this.currentVideoInfo=this.settingsManager.loadVideoData();}insertIntoMypage(){const t=document.querySelector(x.mypageHeaderTitle);t&&(this.hostElement=this.createSettingsUI(),t.parentElement?.insertBefore(this.hostElement,t.nextSibling),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}}));}addAutoCommentButtons(){document.querySelectorAll(x.mypageItem).forEach(e=>{const n=e.querySelector(x.mypageItemTitle);if(!n||n.querySelector(".auto-comment-button-host"))return;const r=n.querySelector("span")?.textContent?.trim()??"",s=e.querySelector(x.mypageEpisodeNumber)?.textContent?.trim()??"",o=e.querySelector(x.mypageEpisodeTitle)?.textContent?.trim()??"",a=document.createElement("div");a.className="nico-comment-shadow-host auto-comment-button-host",a.style.cssText="position:absolute;left:150px;top:3px;margin-left:8px;";const l=a.attachShadow({mode:"closed"}),c=document.createElement("style");c.textContent=M.getAutoButtonStyles(),l.appendChild(c);const d=document.createElement("button");d.className="auto-comment-button",d.innerHTML=U,d.setAttribute("aria-label","コメント設定"),d.setAttribute("title","コメント設定"),d.setAttribute("type","button"),d.addEventListener("click",async h=>{h.preventDefault(),h.stopPropagation(),h.stopImmediatePropagation();try{const p=[r,s,o].filter(Boolean).join(" ");this.scrollToSettings(),this.setSearchKeyword(p),this.lastAutoButtonElement=a;try{const f=e.querySelector('input[name="workId"]')?.value?.trim()??"",g=e.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),S=e.querySelector('a.textContainer[href*="partId="]');let A="";const L=(g?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);L?A=L[1]:S?.href&&(A=(new URL(S.href,location.origin).searchParams.get("partId")??"").trim()),f&&A&&this.settingsManager.saveLastDanimeIds({workId:f,partId:A});}catch(f){console.warn("[SettingsUI] save (workId, partId) skipped:",f);}const v=await this.executeSearch(p);if(v.length===0)return;await this.applySearchResult(v[0]);}catch(p){I.error("SettingsUI.autoCommentButton",p);}}),l.appendChild(d),n.appendChild(a),this.lastAutoButtonElement=a;}),this.waitMypageListStable().then(()=>{try{this.tryRestoreByDanimeIds();}catch(e){console.warn("[SettingsUI] restore failed:",e);}});}async waitMypageListStable(){const t=document.querySelector(x.mypageListContainer);if(!t)return;let e=t.querySelectorAll(x.mypageItem).length;const n=Date.now()+1500;return new Promise(r=>{const s=new MutationObserver(()=>{const o=t.querySelectorAll(x.mypageItem).length;if(o!==e){e=o;return}Date.now()>=n&&(s.disconnect(),r());});s.observe(t,{childList:true,subtree:true}),setTimeout(()=>{try{s.disconnect();}catch(o){I.debug("waitMypageListStable: observer already disconnected",o);}r();},1600);})}tryRestoreByDanimeIds(){const t=this.settingsManager.loadLastDanimeIds();if(!t)return  false;const e=Array.from(document.querySelectorAll(x.mypageItem));for(const n of e){if(n.querySelector('input[name="workId"]')?.value?.trim()!==t.workId)continue;const s=n.querySelector('a.textContainer[href*="partId="]'),o=s?.href?(new URL(s.href,location.origin).searchParams.get("partId")??"")===t.partId:false,a=n.querySelector(".thumbnailContainer > a, .thumbnail-container > a"),l=(()=>{const d=(a?.getAttribute("onclick")??"").match(/mypageContentPlayWrapper\(["'](\d+)["']\)/);return !!d&&d[1]===t.partId})();if(o||l){const c=n.querySelector(".auto-comment-button-host")??n;return this.lastAutoButtonElement=c,this.updatePlayButtonState(this.currentVideoInfo),true}}return  false}createSettingsUI(){const t=document.createElement("div");t.className="nico-comment-shadow-host settings-host",this.createShadowDOM(t),this.addStyles(M.getCommonStyles());const e=this.buildSettingsHtml();return this.setHTML(e),this.applySettingsToUI(),this.addStyles(St),this.setupEventListeners(),t}buildSettingsHtml(){const t=r=>typeof r=="number"?r.toLocaleString():"-",e=r=>{if(!r)return "-";try{return new Date(r).toLocaleDateString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"})}catch{return r}},n=this.currentVideoInfo;return `
