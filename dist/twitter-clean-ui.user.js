...
`;class L{settingsManager;controller;overlay=null;styleElement=null;constructor(e,t){this.settingsManager=e,this.controller=t,this.injectStyles();}injectStyles(){this.styleElement||(this.styleElement=document.createElement("style"),this.styleElement.textContent=k,document.head.appendChild(this.styleElement));}show(){this.overlay||(this.overlay=this.createOverlay(),document.body.appendChild(this.overlay),setTimeout(()=>{this.showTab("visibility");},0));}hide(){this.overlay&&(this.overlay.remove(),this.overlay=null);}createOverlay(){const e=document.createElement("div");e.className="twitter-clean-ui-overlay",e.addEventListener("click",i=>{i.target===e&&this.hide();});const t=this.createModal();return e.appendChild(t),e}createModal(){const e=document.createElement("div");e.className="twitter-clean-ui-modal";const t=this.createHeader();e.appendChild(t);const i=this.createTabs();e.appendChild(i);const r=document.createElement("div");r.className="twitter-clean-ui-body",r.id="twitter-clean-ui-body",e.appendChild(r);const n=this.createFooter();return e.appendChild(n),e}createHeader(){const e=document.createElement("div");e.className="twitter-clean-ui-header";const t=document.createElement("h2");t.className="twitter-clean-ui-title",t.textContent=c("appName"),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-close",i.textContent="×",i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}createTabs(){const e=document.createElement("div");return e.className="twitter-clean-ui-tabs",[{id:"visibility",label:c("leftSidebarSettings")},{id:"rightSidebar",label:c("rightSidebarSettings")},{id:"layout",label:c("layoutSettings")},{id:"profiles",label:c("profileSettings")}].forEach((i,r)=>{const n=document.createElement("button");n.className=`twitter-clean-ui-tab ${r===0?"active":""}`,n.textContent=i.label,n.addEventListener("click",()=>{e.querySelectorAll(".twitter-clean-ui-tab").forEach(s=>{s.classList.remove("active");}),n.classList.add("active"),this.showTab(i.id);}),e.appendChild(n);}),e}showTab(e){const t=document.getElementById("twitter-clean-ui-body");if(t)switch(t.innerHTML="",e){case "visibility":t.appendChild(this.createVisibilityTab());break;case "rightSidebar":t.appendChild(this.createRightSidebarTab());break;case "layout":t.appendChild(this.createLayoutTab());break;case "profiles":t.appendChild(this.createProfilesTab());break}}createVisibilityTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(c("leftSidebarSettings"));["leftSidebar_Logo","leftSidebar_HomeLink","leftSidebar_ExploreLink","leftSidebar_NotificationsLink","leftSidebar_MessagesLink","leftSidebar_BookmarksLink","leftSidebar_ListsLink","leftSidebar_ProfileLink","leftSidebar_PremiumLink","leftSidebar_MoreMenu","leftSidebar_TweetButton","leftSidebar_ProfileMenu"].forEach(d=>{const a=t.visibility,l=this.createToggleControl(c(d),a[d]??true,u=>{const g={[d]:u};this.settingsManager.updateSettings({visibility:g}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(l);}),e.appendChild(i);const n=this.createSection(c("mainContentSettings")),s=this.createToggleControl(c("mainContent_TweetComposer"),t.visibility.mainContent_TweetComposer,d=>{const a={mainContent_TweetComposer:d};this.settingsManager.updateSettings({visibility:a}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});n.appendChild(s);const p=this.createToggleControl(c("promotedTweets"),t.visibility.promotedTweets,d=>{const a={promotedTweets:d};this.settingsManager.updateSettings({visibility:a}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});return n.appendChild(p),e.appendChild(n),e}createRightSidebarTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(c("rightSidebarSettings"));return ["rightSidebar","rightSidebar_SearchBox","rightSidebar_PremiumSubscribe","rightSidebar_TrendsList","rightSidebar_WhoToFollow","rightSidebar_Footer"].forEach(n=>{const s=t.visibility,p=this.createToggleControl(c(n),s[n]??true,d=>{const a={[n]:d};this.settingsManager.updateSettings({visibility:a}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(p);}),e.appendChild(i),e}createLayoutTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(c("layoutSettings")),r=this.createSliderControl(c("leftSidebarWidth"),t.layout.leftSidebarWidth,200,400,l=>{const u={leftSidebarWidth:l};this.settingsManager.updateSettings({layout:u}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(r);const n=this.createSliderControl(c("mainContentWidth"),t.layout.mainContentWidth,500,1200,l=>{const u={mainContentWidth:l};this.settingsManager.updateSettings({layout:u}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(n);const s=this.createSliderControl(c("rightSidebarWidth"),t.layout.rightSidebarWidth,300,500,l=>{const u={rightSidebarWidth:l};this.settingsManager.updateSettings({layout:u}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(s);const p=this.createSliderControl(c("timelineRightPadding"),t.layout.timelineRightPadding,0,100,l=>{const u={timelineRightPadding:l};this.settingsManager.updateSettings({layout:u}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(p);const d=this.createSliderControl(c("gap"),t.layout.gap,0,60,l=>{const u={gap:l};this.settingsManager.updateSettings({layout:u}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(d);const a=this.createToggleControl(c("enableRealTimePreview"),t.enableRealTimePreview,l=>{this.settingsManager.updateSettings({enableRealTimePreview:l});});return i.appendChild(a),e.appendChild(i),e}createProfilesTab(){const e=document.createElement("div"),t=this.createSection(c("profileSettings")),i=this.settingsManager.getAllProfiles(),r=this.settingsManager.getCurrentProfileId(),n=document.createElement("ul");n.className="twitter-clean-ui-profile-list",i.forEach(a=>{const l=document.createElement("li");l.className=`twitter-clean-ui-profile-item ${a.id===r?"active":""}`;const u=document.createElement("span");u.className="twitter-clean-ui-profile-name",u.textContent=a.name,l.appendChild(u);const g=document.createElement("div");if(g.className="twitter-clean-ui-profile-actions",a.id!==r){const h=document.createElement("button");h.className="twitter-clean-ui-icon-button",h.textContent="✓",h.title="切り替え",h.addEventListener("click",b=>{b.stopPropagation(),this.settingsManager.switchProfile(a.id),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles");}),g.appendChild(h);}if(a.id!=="default"){const h=document.createElement("button");h.className="twitter-clean-ui-icon-button",h.textContent="🗑",h.title="削除",h.addEventListener("click",b=>{b.stopPropagation(),confirm(`${a.name} を削除しますか？`)&&(this.settingsManager.deleteProfile(a.id),this.showTab("profiles"));}),g.appendChild(h);}l.appendChild(g),n.appendChild(l);}),t.appendChild(n);const s=document.createElement("button");s.className="twitter-clean-ui-button twitter-clean-ui-button-primary",s.textContent=c("createNewProfile"),s.style.marginTop="12px",s.addEventListener("click",()=>{const a=prompt("プロファイル名を入力してください:");a&&(this.settingsManager.createProfile(a),this.showTab("profiles"));}),t.appendChild(s);const p=document.createElement("button");p.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",p.textContent=c("exportSettings"),p.style.marginTop="12px",p.addEventListener("click",()=>{const a=this.settingsManager.exportSettings(),l=new Blob([a],{type:"application/json"}),u=URL.createObjectURL(l),g=document.createElement("a");g.href=u,g.download=`twitter-clean-ui-settings-${Date.now()}.json`,g.click(),URL.revokeObjectURL(u);}),t.appendChild(p);const d=document.createElement("button");return d.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",d.textContent=c("importSettings"),d.style.marginTop="12px",d.style.marginLeft="8px",d.addEventListener("click",()=>{const a=document.createElement("input");a.type="file",a.accept="application/json",a.addEventListener("change",async l=>{const u=l.target.files?.[0];if(u){const g=await u.text();this.settingsManager.importSettings(g)?(alert("設定をインポートしました"),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles")):alert("設定のインポートに失敗しました");}}),a.click();}),t.appendChild(d),e.appendChild(t),e}createSection(e){const t=document.createElement("div");t.className="twitter-clean-ui-section";const i=document.createElement("span");return i.className="twitter-clean-ui-section-title",i.textContent=e,t.appendChild(i),t}createToggleControl(e,t,i){const r=document.createElement("div");r.className="twitter-clean-ui-control";const n=document.createElement("span");n.className="twitter-clean-ui-label",n.textContent=e,r.appendChild(n);const s=document.createElement("div");s.className=`twitter-clean-ui-toggle ${t?"active":""}`;const p=document.createElement("div");return p.className="twitter-clean-ui-toggle-slider",s.appendChild(p),s.addEventListener("click",()=>{const d=s.classList.toggle("active");i(d);}),r.appendChild(s),r}createSliderControl(e,t,i,r,n){const s=document.createElement("div");s.className="twitter-clean-ui-control";const p=document.createElement("span");p.className="twitter-clean-ui-label",p.textContent=e,s.appendChild(p);const d=document.createElement("div");d.className="twitter-clean-ui-slider-container";const a=document.createElement("input");a.type="range",a.className="twitter-clean-ui-slider",a.min=String(i),a.max=String(r),a.value=String(t),d.appendChild(a);const l=document.createElement("span");return l.className="twitter-clean-ui-slider-value",l.textContent=`${t}px`,d.appendChild(l),a.addEventListener("input",()=>{const u=Number(a.value);l.textContent=`${u}px`,n(u);}),s.appendChild(d),s}createFooter(){const e=document.createElement("div");e.className="twitter-clean-ui-footer";const t=document.createElement("button");t.className="twitter-clean-ui-button twitter-clean-ui-button-danger",t.textContent=c("reset"),t.addEventListener("click",()=>{confirm("設定をリセットしますか？")&&(this.settingsManager.reset(),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("visibility"));}),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",i.textContent=c("close"),i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}destroy(){this.hide(),this.styleElement&&this.styleElement.parentNode&&this.styleElement.parentNode.removeChild(this.styleElement);}}class M{detector;controller;settingsManager;settingsUI;isInitialized=false;constructor(){this.detector=new w,this.controller=new C(this.detector),this.settingsManager=new v,this.settingsUI=new L(this.settingsManager,this.controller);}async initialize(){if(this.isInitialized){console.log("[TwitterCleanUI] Already initialized");return}console.log("[TwitterCleanUI] Initializing...");try{const e=this.settingsManager.getSettings();T(e.language||_()),this.detector.detectAll();const t=this.detector.getStatistics();console.log(`[TwitterCleanUI] Detected ${t.detected}/${t.total} elements`),this.controller.applySettings(e),console.log("[TwitterCleanUI] Settings applied"),this.detector.startObserving(),this.registerMenuCommand(),this.startPromotedTweetsWatcher(),this.isInitialized=!0,console.log("[TwitterCleanUI] Initialized successfully");}catch(e){console.error("[TwitterCleanUI] Initialization failed:",e);}}registerMenuCommand(){typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand("設定を開く",()=>{this.settingsUI.show();}):(document.addEventListener("keydown",e=>{e.ctrlKey&&e.shiftKey&&e.key==="X"&&(e.preventDefault(),this.settingsUI.show());}),console.log("[TwitterCleanUI] Keyboard shortcut registered: Ctrl + Shift + X"));}startPromotedTweetsWatcher(){this.settingsManager.getSettings().visibility.promotedTweets||setInterval(()=>{this.controller.hideAllPromotedTweets();},2e3);}destroy(){this.detector.stopObserving(),this.controller.destroy(),this.settingsUI.destroy(),this.isInitialized=false,console.log("[TwitterCleanUI] Destroyed");}}function N(){return new Promise(o=>{document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>o()):o();})}function R(o=1e4){return new Promise((e,t)=>{const i=Date.now(),r=()=>{if(document.getElementById("react-root")){e();return}if(Date.now()-i>o){t(new Error("Timeout waiting for react-root"));return}setTimeout(r,100);};r();})}(async()=>{try{console.log("[TwitterCleanUI] Starting..."),await N(),console.log("[TwitterCleanUI] Page loaded"),await R(),console.log("[TwitterCleanUI] React root found");const o=new M;await o.initialize(),window.twitterCleanUI=o;}catch(o){console.error("[TwitterCleanUI] Fatal error:",o);}})();
