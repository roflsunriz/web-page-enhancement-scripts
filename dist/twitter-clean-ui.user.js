...
`;class R{settingsManager;controller;overlay=null;styleElement=null;constructor(e,t){this.settingsManager=e,this.controller=t,this.injectStyles();}injectStyles(){this.styleElement||(this.styleElement=document.createElement("style"),this.styleElement.textContent=I,document.head.appendChild(this.styleElement));}show(){this.overlay||(this.overlay=this.createOverlay(),document.body.appendChild(this.overlay),setTimeout(()=>{this.showTab("visibility");},0));}hide(){this.overlay&&(this.overlay.remove(),this.overlay=null);}createOverlay(){const e=document.createElement("div");e.className="twitter-clean-ui-overlay",e.addEventListener("click",i=>{i.target===e&&this.hide();});const t=this.createModal();return e.appendChild(t),e}createModal(){const e=document.createElement("div");e.className="twitter-clean-ui-modal";const t=this.createHeader();e.appendChild(t);const i=this.createTabs();e.appendChild(i);const r=document.createElement("div");r.className="twitter-clean-ui-body",r.id="twitter-clean-ui-body",e.appendChild(r);const n=this.createFooter();return e.appendChild(n),e}createHeader(){const e=document.createElement("div");e.className="twitter-clean-ui-header";const t=document.createElement("h2");t.className="twitter-clean-ui-title",t.textContent=d("appName"),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-close",i.textContent="×",i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}createTabs(){const e=document.createElement("div");return e.className="twitter-clean-ui-tabs",[{id:"visibility",label:d("leftSidebarSettings")},{id:"rightSidebar",label:d("rightSidebarSettings")},{id:"layout",label:d("layoutSettings")},{id:"profiles",label:d("profileSettings")}].forEach((i,r)=>{const n=document.createElement("button");n.className=`twitter-clean-ui-tab ${r===0?"active":""}`,n.textContent=i.label,n.addEventListener("click",()=>{e.querySelectorAll(".twitter-clean-ui-tab").forEach(s=>{s.classList.remove("active");}),n.classList.add("active"),this.showTab(i.id);}),e.appendChild(n);}),e}showTab(e){const t=document.getElementById("twitter-clean-ui-body");if(t)switch(t.innerHTML="",e){case "visibility":t.appendChild(this.createVisibilityTab());break;case "rightSidebar":t.appendChild(this.createRightSidebarTab());break;case "layout":t.appendChild(this.createLayoutTab());break;case "profiles":t.appendChild(this.createProfilesTab());break}}createVisibilityTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(d("leftSidebarSettings"));return ["leftSidebar_Logo","leftSidebar_HomeLink","leftSidebar_ExploreLink","leftSidebar_NotificationsLink","leftSidebar_ConnectLink","leftSidebar_MessagesLink","leftSidebar_GrokLink","leftSidebar_BookmarksLink","leftSidebar_ListsLink","leftSidebar_CommunitiesLink","leftSidebar_ProfileLink","leftSidebar_PremiumLink","leftSidebar_BusinessLink","leftSidebar_CreatorStudioLink","leftSidebar_MoreMenu","leftSidebar_TweetButton","leftSidebar_ProfileMenu"].forEach(n=>{const s=t.visibility,o=this.createToggleControl(d(n),s[n]??true,c=>{const l={[n]:c};this.settingsManager.updateSettings({visibility:l}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(o);}),e.appendChild(i),e}createRightSidebarTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(d("rightSidebarSettings"));return ["rightSidebar","rightSidebar_SearchBox","rightSidebar_PremiumSubscribe","rightSidebar_TrendsList","rightSidebar_WhoToFollow","rightSidebar_TodayNews","rightSidebar_RelatedAccounts","rightSidebar_Footer"].forEach(n=>{const s=t.visibility,o=this.createToggleControl(d(n),s[n]??true,c=>{const l={[n]:c};this.settingsManager.updateSettings({visibility:l}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(o);}),e.appendChild(i),e}createLayoutTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(d("layoutSettings")),r=this.createSliderControl(d("mainContentWidth"),t.layout.mainContentWidth,500,1200,o=>{const c={mainContentWidth:o};this.settingsManager.updateSettings({layout:c}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(r);const n=this.createSliderControl(d("timelineRightPadding"),t.layout.timelineRightPadding,0,100,o=>{const c={timelineRightPadding:o};this.settingsManager.updateSettings({layout:c}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(n);const s=this.createToggleControl(d("enableRealTimePreview"),t.enableRealTimePreview,o=>{this.settingsManager.updateSettings({enableRealTimePreview:o});});return i.appendChild(s),e.appendChild(i),e}createProfilesTab(){const e=document.createElement("div"),t=this.createSection(d("profileSettings")),i=this.settingsManager.getAllProfiles(),r=this.settingsManager.getCurrentProfileId(),n=document.createElement("ul");n.className="twitter-clean-ui-profile-list",i.forEach(l=>{const u=document.createElement("li");u.className=`twitter-clean-ui-profile-item ${l.id===r?"active":""}`;const h=document.createElement("span");h.className="twitter-clean-ui-profile-name",h.textContent=l.name,u.appendChild(h);const g=document.createElement("div");if(g.className="twitter-clean-ui-profile-actions",l.id!==r){const p=document.createElement("button");p.className="twitter-clean-ui-icon-button",p.textContent="✓",p.title="切り替え",p.addEventListener("click",C=>{C.stopPropagation(),this.settingsManager.switchProfile(l.id),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles");}),g.appendChild(p);}if(l.id!=="default"){const p=document.createElement("button");p.className="twitter-clean-ui-icon-button",p.textContent="🗑",p.title="削除",p.addEventListener("click",C=>{C.stopPropagation(),confirm(`${l.name} を削除しますか？`)&&(this.settingsManager.deleteProfile(l.id),this.showTab("profiles"));}),g.appendChild(p);}u.appendChild(g),n.appendChild(u);}),t.appendChild(n);const s=document.createElement("button");s.className="twitter-clean-ui-button twitter-clean-ui-button-primary",s.textContent=d("createNewProfile"),s.style.marginTop="12px",s.addEventListener("click",()=>{const l=prompt("プロファイル名を入力してください:");l&&(this.settingsManager.createProfile(l),this.showTab("profiles"));}),t.appendChild(s);const o=document.createElement("button");o.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",o.textContent=d("exportSettings"),o.style.marginTop="12px",o.addEventListener("click",()=>{const l=this.settingsManager.exportSettings(),u=new Blob([l],{type:"application/json"}),h=URL.createObjectURL(u),g=document.createElement("a");g.href=h,g.download=`twitter-clean-ui-settings-${Date.now()}.json`,g.click(),URL.revokeObjectURL(h);}),t.appendChild(o);const c=document.createElement("button");return c.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",c.textContent=d("importSettings"),c.style.marginTop="12px",c.style.marginLeft="8px",c.addEventListener("click",()=>{const l=document.createElement("input");l.type="file",l.accept="application/json",l.addEventListener("change",async u=>{const h=u.target.files?.[0];if(h){const g=await h.text();this.settingsManager.importSettings(g)?(alert("設定をインポートしました"),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles")):alert("設定のインポートに失敗しました");}}),l.click();}),t.appendChild(c),e.appendChild(t),e}createSection(e){const t=document.createElement("div");t.className="twitter-clean-ui-section";const i=document.createElement("span");return i.className="twitter-clean-ui-section-title",i.textContent=e,t.appendChild(i),t}createToggleControl(e,t,i){const r=document.createElement("div");r.className="twitter-clean-ui-control";const n=document.createElement("span");n.className="twitter-clean-ui-label",n.textContent=e,r.appendChild(n);const s=document.createElement("div");s.className=`twitter-clean-ui-toggle ${t?"active":""}`;const o=document.createElement("div");return o.className="twitter-clean-ui-toggle-slider",s.appendChild(o),s.addEventListener("click",()=>{const c=s.classList.toggle("active");i(c);}),r.appendChild(s),r}createSliderControl(e,t,i,r,n){const s=document.createElement("div");s.className="twitter-clean-ui-control";const o=document.createElement("span");o.className="twitter-clean-ui-label",o.textContent=e,s.appendChild(o);const c=document.createElement("div");c.className="twitter-clean-ui-slider-container";const l=document.createElement("input");l.type="range",l.className="twitter-clean-ui-slider",l.min=String(i),l.max=String(r),l.value=String(t),c.appendChild(l);const u=document.createElement("span");return u.className="twitter-clean-ui-slider-value",u.textContent=`${t}px`,c.appendChild(u),l.addEventListener("input",()=>{const h=Number(l.value);u.textContent=`${h}px`,n(h);}),s.appendChild(c),s}createFooter(){const e=document.createElement("div");e.className="twitter-clean-ui-footer";const t=document.createElement("button");t.className="twitter-clean-ui-button twitter-clean-ui-button-danger",t.textContent=d("reset"),t.addEventListener("click",()=>{confirm("設定をリセットしますか？")&&(this.settingsManager.reset(),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("visibility"));}),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",i.textContent=d("close"),i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}destroy(){this.hide(),this.styleElement&&this.styleElement.parentNode&&this.styleElement.parentNode.removeChild(this.styleElement);}}try{let a=null;if(typeof GM_getValue<"u"?a=GM_getValue(w,null):a=localStorage.getItem(w),a){const e=document.createElement("style");e.id=S.STYLE_ELEMENT_ID,e.type="text/css",e.textContent=a,(document.head||document.documentElement).appendChild(e);}}catch{}const F=2e3,v=100,q=20;class G{detector;controller;settingsManager;settingsUI;isInitialized=false;settingsWatcherInterval=null;primaryMutationObserver=null;sidebarMutationObserver=null;applySettingsDebounceTimer=null;rafId=null;sidebarDebounceTimer=null;revealFailsafeTimer=null;lastUrl="";isApplyingSettings=false;constructor(){this.detector=new L,this.controller=new P(this.detector),this.settingsManager=new M,this.settingsUI=new R(this.settingsManager,this.controller),this.lastUrl=location.href;}async initialize(){if(!this.isInitialized)try{await this.settingsManager.initialize();const e=this.settingsManager.getSettings();O(e.language||B()),console.log("[CLOAK-DBG] initialize: settings.visibility.rightSidebar =",e.visibility.rightSidebar),console.log("[CLOAK-DBG] initialize: rightSidebar sub-settings =",JSON.stringify(Object.fromEntries(Object.entries(e.visibility).filter(([t])=>t.startsWith("rightSidebar"))))),this.guardedApplySettings(),this.startPrimaryMutationObserver(),this.startSidebarMutationObserver(),this.registerMenuCommand(),this.startSettingsWatcher(),this.setupNavigationInterception(),console.log("[CLOAK-DBG] initialize: about to revealSidebar"),this.logCloakState("initialize:beforeReveal"),this.revealSidebar(),this.isInitialized=!0;}catch(e){console.error("[TwitterCleanUI] Initialization failed:",e),this.revealSidebar();}}guardedApplySettings(){this.isApplyingSettings=true;try{this.detector.detectAll();const e=this.settingsManager.getSettings();this.controller.applySettings(e),console.log("[CLOAK-DBG] guardedApplySettings: done, detected rightSidebar =",this.detector.isDetected("rightSidebar"));}finally{this.isApplyingSettings=false;}}cloakSidebar(){console.log("[CLOAK-DBG] cloakSidebar called",new Error().stack?.split(`
`).slice(1,3).join(" <- "));let e=document.getElementById(b);e||(e=document.createElement("style"),e.id=b,(document.head||document.documentElement).appendChild(e),console.log("[CLOAK-DBG] cloakSidebar: created new cloak element")),e.textContent=k,this.clearRevealFailsafe(),this.revealFailsafeTimer=setTimeout(()=>{console.log("[CLOAK-DBG] revealSidebar via FAILSAFE (2000ms timeout)"),this.revealSidebar();},F);}revealSidebar(){console.log("[CLOAK-DBG] revealSidebar called",new Error().stack?.split(`
`).slice(1,3).join(" <- ")),this.clearRevealFailsafe(),requestAnimationFrame(()=>{requestAnimationFrame(()=>{const e=document.getElementById(b);e?(console.log("[CLOAK-DBG] revealSidebar rAF: clearing cloak textContent (was:",e.textContent?"ACTIVE":"EMPTY",")"),e.textContent=""):console.log("[CLOAK-DBG] revealSidebar rAF: cloak element NOT FOUND"),this.logCloakState("revealSidebar:afterClear");});});}clearRevealFailsafe(){this.revealFailsafeTimer!==null&&(clearTimeout(this.revealFailsafeTimer),this.revealFailsafeTimer=null);}logCloakState(e){const t=document.getElementById(b),i=document.querySelector('[data-testid="sidebarColumn"]');console.log(`[CLOAK-DBG] STATE@${e}:`,"cloakExists=",!!t,"cloakContent=",t?.textContent?"ACTIVE":"EMPTY","sidebarExists=",!!i,"sidebarChildren=",i?.children.length??0,"sidebarVisibility=",i?getComputedStyle(i).visibility:"N/A","sidebarDisplay=",i?getComputedStyle(i).display:"N/A","url=",location.pathname);}setupNavigationInterception(){const e=r=>{const n=location.href;if(n===this.lastUrl){console.log("[CLOAK-DBG] onNavigate: SKIPPED (same URL)",r,n);return}console.log("[CLOAK-DBG] onNavigate: TRIGGERED",r,this.lastUrl,"->",n),this.lastUrl=n,this.handleNavigation();},t=history.pushState.bind(history);history.pushState=(...r)=>{t(...r),e("pushState");};const i=history.replaceState.bind(history);history.replaceState=(...r)=>{i(...r),e("replaceState");},window.addEventListener("popstate",()=>{e("popstate");});}handleNavigation(){console.log("[CLOAK-DBG] handleNavigation: START",location.pathname),this.cloakSidebar();let e=0;const t=()=>{this.guardedApplySettings();const i=document.querySelector('[data-testid="sidebarColumn"]'),r=i!==null&&i.children.length>0;if(console.log("[CLOAK-DBG] handleNavigation:settle retry=",e,"hasSidebarContent=",r,"sidebarChildCount=",i?.children.length??0),r||e>=q){console.log("[CLOAK-DBG] handleNavigation: SETTLED, about to reveal"),this.logCloakState("handleNavigation:beforeReveal"),this.revealSidebar(),this.reattachSidebarObserver();return}e++,setTimeout(t,v);};setTimeout(t,v);}startPrimaryMutationObserver(){this.primaryMutationObserver=new MutationObserver(()=>{this.rafId!==null&&cancelAnimationFrame(this.rafId),this.rafId=requestAnimationFrame(()=>{this.applySettingsDebounceTimer&&clearTimeout(this.applySettingsDebounceTimer),this.applySettingsDebounceTimer=setTimeout(()=>{this.guardedApplySettings(),this.rafId=null;},500);});});const e=document.querySelector('[data-testid="primaryColumn"]');e?this.primaryMutationObserver.observe(e,{childList:true,subtree:true}):this.primaryMutationObserver.observe(document.body,{childList:true,subtree:true});}startSidebarMutationObserver(){const e=document.querySelector('[data-testid="sidebarColumn"]');if(!e){console.log("[CLOAK-DBG] startSidebarMO: sidebar NOT FOUND, skipping");return}console.log("[CLOAK-DBG] startSidebarMO: attaching observer"),this.sidebarMutationObserver=new MutationObserver(t=>{this.isApplyingSettings||!t.some(r=>r.type==="childList"&&(r.addedNodes.length>0||r.removedNodes.length>0))||(console.log("[CLOAK-DBG] sidebarMO: significant change detected, mutations=",t.length),this.sidebarDebounceTimer&&clearTimeout(this.sidebarDebounceTimer),this.sidebarDebounceTimer=setTimeout(()=>{console.log("[CLOAK-DBG] sidebarMO debounce: running guardedApplySettings"),this.logCloakState("sidebarMO:beforeApply"),this.guardedApplySettings(),this.logCloakState("sidebarMO:afterApply");},200));}),this.sidebarMutationObserver.observe(e,{childList:true,subtree:true});}reattachSidebarObserver(){console.log("[CLOAK-DBG] reattachSidebarObserver called"),this.sidebarMutationObserver&&(this.sidebarMutationObserver.disconnect(),this.sidebarMutationObserver=null),this.startSidebarMutationObserver();}startSettingsWatcher(){let e=0;const t=setInterval(()=>{this.guardedApplySettings(),e++,e>=10&&clearInterval(t);},500);this.settingsWatcherInterval=setInterval(()=>{this.guardedApplySettings();},5e3);}registerMenuCommand(){typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand("設定を開く",()=>{this.settingsUI.show();}):document.addEventListener("keydown",e=>{e.ctrlKey&&e.shiftKey&&e.key==="X"&&(e.preventDefault(),this.settingsUI.show());});}destroy(){this.settingsWatcherInterval&&clearInterval(this.settingsWatcherInterval),this.applySettingsDebounceTimer&&clearTimeout(this.applySettingsDebounceTimer),this.sidebarDebounceTimer&&clearTimeout(this.sidebarDebounceTimer),this.clearRevealFailsafe(),this.rafId!==null&&cancelAnimationFrame(this.rafId),this.primaryMutationObserver&&this.primaryMutationObserver.disconnect(),this.sidebarMutationObserver&&this.sidebarMutationObserver.disconnect(),this.detector.stopObserving(),this.controller.destroy(),this.settingsUI.destroy(),this.isInitialized=false;}}function K(){return new Promise(a=>{document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>a()):a();})}function j(a=1e4){return new Promise((e,t)=>{const i=Date.now(),r=()=>{if(document.getElementById("react-root")){e();return}if(Date.now()-i>a){t(new Error("Timeout waiting for react-root"));return}setTimeout(r,100);};r();})}(async()=>{try{console.log("[CLOAK-DBG] Phase2: waiting for page load..."),await K(),console.log("[CLOAK-DBG] Phase2: page loaded, waiting for react-root..."),await j(),console.log("[CLOAK-DBG] Phase2: react-root found, initializing app...");const a=new G;await a.initialize(),console.log("[CLOAK-DBG] Phase2: app initialized"),window.twitterCleanUI=a,setTimeout(()=>{const e=document.getElementById(b),t=document.querySelector('[data-testid="sidebarColumn"]');console.log("[CLOAK-DBG] Phase2 +3s check:","cloakContent=",e?.textContent?"ACTIVE":"EMPTY","sidebarVisibility=",t?getComputedStyle(t).visibility:"N/A","sidebarDisplay=",t?getComputedStyle(t).display:"N/A","sidebarInlineDisplay=",t?JSON.stringify(t.style.display):"N/A","url=",location.pathname);},3e3);}catch(a){console.error("[TwitterCleanUI] Fatal error:",a);const e=document.getElementById(b);e&&(e.textContent="");}})();
