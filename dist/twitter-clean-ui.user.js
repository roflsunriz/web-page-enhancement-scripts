...
`;class D{settingsManager;controller;overlay=null;styleElement=null;constructor(e,t){this.settingsManager=e,this.controller=t,this.injectStyles();}injectStyles(){this.styleElement||(this.styleElement=document.createElement("style"),this.styleElement.textContent=I,document.head.appendChild(this.styleElement));}show(){this.overlay||(this.overlay=this.createOverlay(),document.body.appendChild(this.overlay),setTimeout(()=>{this.showTab("visibility");},0));}hide(){this.overlay&&(this.overlay.remove(),this.overlay=null);}createOverlay(){const e=document.createElement("div");e.className="twitter-clean-ui-overlay",e.addEventListener("click",i=>{i.target===e&&this.hide();});const t=this.createModal();return e.appendChild(t),e}createModal(){const e=document.createElement("div");e.className="twitter-clean-ui-modal";const t=this.createHeader();e.appendChild(t);const i=this.createTabs();e.appendChild(i);const r=document.createElement("div");r.className="twitter-clean-ui-body",r.id="twitter-clean-ui-body",e.appendChild(r);const n=this.createFooter();return e.appendChild(n),e}createHeader(){const e=document.createElement("div");e.className="twitter-clean-ui-header";const t=document.createElement("h2");t.className="twitter-clean-ui-title",t.textContent=d("appName"),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-close",i.textContent="×",i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}createTabs(){const e=document.createElement("div");return e.className="twitter-clean-ui-tabs",[{id:"visibility",label:d("leftSidebarSettings")},{id:"rightSidebar",label:d("rightSidebarSettings")},{id:"layout",label:d("layoutSettings")},{id:"profiles",label:d("profileSettings")}].forEach((i,r)=>{const n=document.createElement("button");n.className=`twitter-clean-ui-tab ${r===0?"active":""}`,n.textContent=i.label,n.addEventListener("click",()=>{e.querySelectorAll(".twitter-clean-ui-tab").forEach(o=>{o.classList.remove("active");}),n.classList.add("active"),this.showTab(i.id);}),e.appendChild(n);}),e}showTab(e){const t=document.getElementById("twitter-clean-ui-body");if(t)switch(t.innerHTML="",e){case "visibility":t.appendChild(this.createVisibilityTab());break;case "rightSidebar":t.appendChild(this.createRightSidebarTab());break;case "layout":t.appendChild(this.createLayoutTab());break;case "profiles":t.appendChild(this.createProfilesTab());break}}createVisibilityTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(d("leftSidebarSettings"));return ["leftSidebar_Logo","leftSidebar_HomeLink","leftSidebar_ExploreLink","leftSidebar_NotificationsLink","leftSidebar_ConnectLink","leftSidebar_MessagesLink","leftSidebar_GrokLink","leftSidebar_BookmarksLink","leftSidebar_ListsLink","leftSidebar_CommunitiesLink","leftSidebar_ProfileLink","leftSidebar_PremiumLink","leftSidebar_BusinessLink","leftSidebar_CreatorStudioLink","leftSidebar_MoreMenu","leftSidebar_TweetButton","leftSidebar_ProfileMenu"].forEach(n=>{const o=t.visibility,s=this.createToggleControl(d(n),o[n]??true,c=>{const l={[n]:c};this.settingsManager.updateSettings({visibility:l}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(s);}),e.appendChild(i),e}createRightSidebarTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(d("rightSidebarSettings"));return ["rightSidebar","rightSidebar_SearchBox","rightSidebar_PremiumSubscribe","rightSidebar_TrendsList","rightSidebar_WhoToFollow","rightSidebar_TodayNews","rightSidebar_RelatedAccounts","rightSidebar_Footer"].forEach(n=>{const o=t.visibility,s=this.createToggleControl(d(n),o[n]??true,c=>{const l={[n]:c};this.settingsManager.updateSettings({visibility:l}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(s);}),e.appendChild(i),e}createLayoutTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(d("layoutSettings")),r=this.createSliderControl(d("mainContentWidth"),t.layout.mainContentWidth,500,1200,s=>{const c={mainContentWidth:s};this.settingsManager.updateSettings({layout:c}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(r);const n=this.createSliderControl(d("timelineRightPadding"),t.layout.timelineRightPadding,0,100,s=>{const c={timelineRightPadding:s};this.settingsManager.updateSettings({layout:c}),this.settingsManager.getSettings().enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(n);const o=this.createToggleControl(d("enableRealTimePreview"),t.enableRealTimePreview,s=>{this.settingsManager.updateSettings({enableRealTimePreview:s});});return i.appendChild(o),e.appendChild(i),e}createProfilesTab(){const e=document.createElement("div"),t=this.createSection(d("profileSettings")),i=this.settingsManager.getAllProfiles(),r=this.settingsManager.getCurrentProfileId(),n=document.createElement("ul");n.className="twitter-clean-ui-profile-list",i.forEach(l=>{const u=document.createElement("li");u.className=`twitter-clean-ui-profile-item ${l.id===r?"active":""}`;const h=document.createElement("span");h.className="twitter-clean-ui-profile-name",h.textContent=l.name,u.appendChild(h);const m=document.createElement("div");if(m.className="twitter-clean-ui-profile-actions",l.id!==r){const p=document.createElement("button");p.className="twitter-clean-ui-icon-button",p.textContent="✓",p.title="切り替え",p.addEventListener("click",w=>{w.stopPropagation(),this.settingsManager.switchProfile(l.id),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles");}),m.appendChild(p);}if(l.id!=="default"){const p=document.createElement("button");p.className="twitter-clean-ui-icon-button",p.textContent="🗑",p.title="削除",p.addEventListener("click",w=>{w.stopPropagation(),confirm(`${l.name} を削除しますか？`)&&(this.settingsManager.deleteProfile(l.id),this.showTab("profiles"));}),m.appendChild(p);}u.appendChild(m),n.appendChild(u);}),t.appendChild(n);const o=document.createElement("button");o.className="twitter-clean-ui-button twitter-clean-ui-button-primary",o.textContent=d("createNewProfile"),o.style.marginTop="12px",o.addEventListener("click",()=>{const l=prompt("プロファイル名を入力してください:");l&&(this.settingsManager.createProfile(l),this.showTab("profiles"));}),t.appendChild(o);const s=document.createElement("button");s.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",s.textContent=d("exportSettings"),s.style.marginTop="12px",s.addEventListener("click",()=>{const l=this.settingsManager.exportSettings(),u=new Blob([l],{type:"application/json"}),h=URL.createObjectURL(u),m=document.createElement("a");m.href=h,m.download=`twitter-clean-ui-settings-${Date.now()}.json`,m.click(),URL.revokeObjectURL(h);}),t.appendChild(s);const c=document.createElement("button");return c.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",c.textContent=d("importSettings"),c.style.marginTop="12px",c.style.marginLeft="8px",c.addEventListener("click",()=>{const l=document.createElement("input");l.type="file",l.accept="application/json",l.addEventListener("change",async u=>{const h=u.target.files?.[0];if(h){const m=await h.text();this.settingsManager.importSettings(m)?(alert("設定をインポートしました"),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles")):alert("設定のインポートに失敗しました");}}),l.click();}),t.appendChild(c),e.appendChild(t),e}createSection(e){const t=document.createElement("div");t.className="twitter-clean-ui-section";const i=document.createElement("span");return i.className="twitter-clean-ui-section-title",i.textContent=e,t.appendChild(i),t}createToggleControl(e,t,i){const r=document.createElement("div");r.className="twitter-clean-ui-control";const n=document.createElement("span");n.className="twitter-clean-ui-label",n.textContent=e,r.appendChild(n);const o=document.createElement("div");o.className=`twitter-clean-ui-toggle ${t?"active":""}`;const s=document.createElement("div");return s.className="twitter-clean-ui-toggle-slider",o.appendChild(s),o.addEventListener("click",()=>{const c=o.classList.toggle("active");i(c);}),r.appendChild(o),r}createSliderControl(e,t,i,r,n){const o=document.createElement("div");o.className="twitter-clean-ui-control";const s=document.createElement("span");s.className="twitter-clean-ui-label",s.textContent=e,o.appendChild(s);const c=document.createElement("div");c.className="twitter-clean-ui-slider-container";const l=document.createElement("input");l.type="range",l.className="twitter-clean-ui-slider",l.min=String(i),l.max=String(r),l.value=String(t),c.appendChild(l);const u=document.createElement("span");return u.className="twitter-clean-ui-slider-value",u.textContent=`${t}px`,c.appendChild(u),l.addEventListener("input",()=>{const h=Number(l.value);u.textContent=`${h}px`,n(h);}),o.appendChild(c),o}createFooter(){const e=document.createElement("div");e.className="twitter-clean-ui-footer";const t=document.createElement("button");t.className="twitter-clean-ui-button twitter-clean-ui-button-danger",t.textContent=d("reset"),t.addEventListener("click",()=>{confirm("設定をリセットしますか？")&&(this.settingsManager.reset(),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("visibility"));}),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",i.textContent=d("close"),i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}destroy(){this.hide(),this.styleElement&&this.styleElement.parentNode&&this.styleElement.parentNode.removeChild(this.styleElement);}}class A{detector;controller;settingsManager;settingsUI;isInitialized=false;settingsWatcherInterval=null;mutationObserver=null;applySettingsDebounceTimer=null;rafId=null;constructor(){this.detector=new v,this.controller=new k(this.detector),this.settingsManager=new L,this.settingsUI=new D(this.settingsManager,this.controller);}async initialize(){if(!this.isInitialized)try{await this.settingsManager.initialize();const e=this.settingsManager.getSettings();N(e.language||R()),this.detector.detectAll(),this.controller.applySettings(e),this.startMutationObserver(),this.registerMenuCommand(),this.startSettingsWatcher(),this.isInitialized=!0;}catch(e){console.error("[TwitterCleanUI] Initialization failed:",e);}}startMutationObserver(){this.mutationObserver=new MutationObserver(()=>{this.rafId!==null&&cancelAnimationFrame(this.rafId),this.rafId=requestAnimationFrame(()=>{this.applySettingsDebounceTimer&&clearTimeout(this.applySettingsDebounceTimer),this.applySettingsDebounceTimer=setTimeout(()=>{this.detector.detectAll(),this.controller.applySettings(this.settingsManager.getSettings()),this.rafId=null;},500);});});const e=document.querySelector('[data-testid="primaryColumn"]');e?this.mutationObserver.observe(e,{childList:true,subtree:true}):this.mutationObserver.observe(document.body,{childList:true,subtree:true});}startSettingsWatcher(){let e=0;const t=setInterval(()=>{this.detector.detectAll(),this.controller.applySettings(this.settingsManager.getSettings()),e++,e>=10&&clearInterval(t);},500);this.settingsWatcherInterval=setInterval(()=>{this.detector.detectAll(),this.controller.applySettings(this.settingsManager.getSettings());},5e3);}registerMenuCommand(){typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand("設定を開く",()=>{this.settingsUI.show();}):document.addEventListener("keydown",e=>{e.ctrlKey&&e.shiftKey&&e.key==="X"&&(e.preventDefault(),this.settingsUI.show());});}destroy(){this.settingsWatcherInterval&&clearInterval(this.settingsWatcherInterval),this.applySettingsDebounceTimer&&clearTimeout(this.applySettingsDebounceTimer),this.rafId!==null&&cancelAnimationFrame(this.rafId),this.mutationObserver&&this.mutationObserver.disconnect(),this.detector.stopObserving(),this.controller.destroy(),this.settingsUI.destroy(),this.isInitialized=false;}}function B(){return new Promise(a=>{document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>a()):a();})}function q(a=1e4){return new Promise((e,t)=>{const i=Date.now(),r=()=>{if(document.getElementById("react-root")){e();return}if(Date.now()-i>a){t(new Error("Timeout waiting for react-root"));return}setTimeout(r,100);};r();})}function O(){try{let a=null;if(typeof GM_getValue<"u"?a=GM_getValue(y,null):a=localStorage.getItem(y),!a)return;const e=document.createElement("style");e.id=b.STYLE_ELEMENT_ID,e.type="text/css",e.textContent=a,(document.head||document.documentElement).appendChild(e);}catch{}}O();(async()=>{try{await B(),await q();const a=new A;await a.initialize(),window.twitterCleanUI=a;}catch(a){console.error("[TwitterCleanUI] Fatal error:",a);}})();
