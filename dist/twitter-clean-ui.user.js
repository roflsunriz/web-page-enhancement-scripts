...
`;class M{settingsManager;controller;overlay=null;styleElement=null;constructor(e,t){this.settingsManager=e,this.controller=t,this.injectStyles();}injectStyles(){this.styleElement||(this.styleElement=document.createElement("style"),this.styleElement.textContent=L,document.head.appendChild(this.styleElement));}show(){this.overlay||(this.overlay=this.createOverlay(),document.body.appendChild(this.overlay),setTimeout(()=>{this.showTab("visibility");},0));}hide(){this.overlay&&(this.overlay.remove(),this.overlay=null);}createOverlay(){const e=document.createElement("div");e.className="twitter-clean-ui-overlay",e.addEventListener("click",i=>{i.target===e&&this.hide();});const t=this.createModal();return e.appendChild(t),e}createModal(){const e=document.createElement("div");e.className="twitter-clean-ui-modal";const t=this.createHeader();e.appendChild(t);const i=this.createTabs();e.appendChild(i);const r=document.createElement("div");r.className="twitter-clean-ui-body",r.id="twitter-clean-ui-body",e.appendChild(r);const n=this.createFooter();return e.appendChild(n),e}createHeader(){const e=document.createElement("div");e.className="twitter-clean-ui-header";const t=document.createElement("h2");t.className="twitter-clean-ui-title",t.textContent=c("appName"),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-close",i.textContent="×",i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}createTabs(){const e=document.createElement("div");return e.className="twitter-clean-ui-tabs",[{id:"visibility",label:c("leftSidebarSettings")},{id:"rightSidebar",label:c("rightSidebarSettings")},{id:"layout",label:c("layoutSettings")},{id:"profiles",label:c("profileSettings")}].forEach((i,r)=>{const n=document.createElement("button");n.className=`twitter-clean-ui-tab ${r===0?"active":""}`,n.textContent=i.label,n.addEventListener("click",()=>{e.querySelectorAll(".twitter-clean-ui-tab").forEach(l=>{l.classList.remove("active");}),n.classList.add("active"),this.showTab(i.id);}),e.appendChild(n);}),e}showTab(e){const t=document.getElementById("twitter-clean-ui-body");if(t)switch(t.innerHTML="",e){case "visibility":t.appendChild(this.createVisibilityTab());break;case "rightSidebar":t.appendChild(this.createRightSidebarTab());break;case "layout":t.appendChild(this.createLayoutTab());break;case "profiles":t.appendChild(this.createProfilesTab());break}}createVisibilityTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(c("leftSidebarSettings"));["leftSidebar_Logo","leftSidebar_HomeLink","leftSidebar_ExploreLink","leftSidebar_NotificationsLink","leftSidebar_MessagesLink","leftSidebar_GrokLink","leftSidebar_BookmarksLink","leftSidebar_ListsLink","leftSidebar_CommunitiesLink","leftSidebar_ProfileLink","leftSidebar_PremiumLink","leftSidebar_MoreMenu","leftSidebar_TweetButton","leftSidebar_ProfileMenu"].forEach(s=>{const a=t.visibility,u=this.createToggleControl(c(s),a[s]??true,p=>{const h={[s]:p};this.settingsManager.updateSettings({visibility:h}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(u);}),e.appendChild(i);const n=this.createSection(c("mainContentSettings")),l=this.createToggleControl(c("mainContent_TweetComposer"),t.visibility.mainContent_TweetComposer,s=>{const a={mainContent_TweetComposer:s};this.settingsManager.updateSettings({visibility:a}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});n.appendChild(l);const d=this.createToggleControl(c("promotedTweets"),t.visibility.promotedTweets,s=>{const a={promotedTweets:s};this.settingsManager.updateSettings({visibility:a}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});return n.appendChild(d),e.appendChild(n),e}createRightSidebarTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(c("rightSidebarSettings"));return ["rightSidebar","rightSidebar_SearchBox","rightSidebar_PremiumSubscribe","rightSidebar_TrendsList","rightSidebar_WhoToFollow","rightSidebar_Footer"].forEach(n=>{const l=t.visibility,d=this.createToggleControl(c(n),l[n]??true,s=>{const a={[n]:s};this.settingsManager.updateSettings({visibility:a}),t.enableRealTimePreview&&this.controller.applySettings(this.settingsManager.getSettings());});i.appendChild(d);}),e.appendChild(i),e}createLayoutTab(){const e=document.createElement("div"),t=this.settingsManager.getSettings(),i=this.createSection(c("layoutSettings")),r=this.createSliderControl(c("leftSidebarWidth"),t.layout.leftSidebarWidth,200,400,s=>{const a={leftSidebarWidth:s};this.settingsManager.updateSettings({layout:a}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(r);const n=this.createSliderControl(c("mainContentWidth"),t.layout.mainContentWidth,500,1200,s=>{const a={mainContentWidth:s};this.settingsManager.updateSettings({layout:a}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(n);const l=this.createSliderControl(c("timelineRightPadding"),t.layout.timelineRightPadding,0,100,s=>{const a={timelineRightPadding:s};this.settingsManager.updateSettings({layout:a}),t.enableRealTimePreview&&this.controller.applyLayout(this.settingsManager.getSettings());});i.appendChild(l);const d=this.createToggleControl(c("enableRealTimePreview"),t.enableRealTimePreview,s=>{this.settingsManager.updateSettings({enableRealTimePreview:s});});return i.appendChild(d),e.appendChild(i),e}createProfilesTab(){const e=document.createElement("div"),t=this.createSection(c("profileSettings")),i=this.settingsManager.getAllProfiles(),r=this.settingsManager.getCurrentProfileId(),n=document.createElement("ul");n.className="twitter-clean-ui-profile-list",i.forEach(a=>{const u=document.createElement("li");u.className=`twitter-clean-ui-profile-item ${a.id===r?"active":""}`;const p=document.createElement("span");p.className="twitter-clean-ui-profile-name",p.textContent=a.name,u.appendChild(p);const h=document.createElement("div");if(h.className="twitter-clean-ui-profile-actions",a.id!==r){const m=document.createElement("button");m.className="twitter-clean-ui-icon-button",m.textContent="✓",m.title="切り替え",m.addEventListener("click",b=>{b.stopPropagation(),this.settingsManager.switchProfile(a.id),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles");}),h.appendChild(m);}if(a.id!=="default"){const m=document.createElement("button");m.className="twitter-clean-ui-icon-button",m.textContent="🗑",m.title="削除",m.addEventListener("click",b=>{b.stopPropagation(),confirm(`${a.name} を削除しますか？`)&&(this.settingsManager.deleteProfile(a.id),this.showTab("profiles"));}),h.appendChild(m);}u.appendChild(h),n.appendChild(u);}),t.appendChild(n);const l=document.createElement("button");l.className="twitter-clean-ui-button twitter-clean-ui-button-primary",l.textContent=c("createNewProfile"),l.style.marginTop="12px",l.addEventListener("click",()=>{const a=prompt("プロファイル名を入力してください:");a&&(this.settingsManager.createProfile(a),this.showTab("profiles"));}),t.appendChild(l);const d=document.createElement("button");d.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",d.textContent=c("exportSettings"),d.style.marginTop="12px",d.addEventListener("click",()=>{const a=this.settingsManager.exportSettings(),u=new Blob([a],{type:"application/json"}),p=URL.createObjectURL(u),h=document.createElement("a");h.href=p,h.download=`twitter-clean-ui-settings-${Date.now()}.json`,h.click(),URL.revokeObjectURL(p);}),t.appendChild(d);const s=document.createElement("button");return s.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",s.textContent=c("importSettings"),s.style.marginTop="12px",s.style.marginLeft="8px",s.addEventListener("click",()=>{const a=document.createElement("input");a.type="file",a.accept="application/json",a.addEventListener("change",async u=>{const p=u.target.files?.[0];if(p){const h=await p.text();this.settingsManager.importSettings(h)?(alert("設定をインポートしました"),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("profiles")):alert("設定のインポートに失敗しました");}}),a.click();}),t.appendChild(s),e.appendChild(t),e}createSection(e){const t=document.createElement("div");t.className="twitter-clean-ui-section";const i=document.createElement("span");return i.className="twitter-clean-ui-section-title",i.textContent=e,t.appendChild(i),t}createToggleControl(e,t,i){const r=document.createElement("div");r.className="twitter-clean-ui-control";const n=document.createElement("span");n.className="twitter-clean-ui-label",n.textContent=e,r.appendChild(n);const l=document.createElement("div");l.className=`twitter-clean-ui-toggle ${t?"active":""}`;const d=document.createElement("div");return d.className="twitter-clean-ui-toggle-slider",l.appendChild(d),l.addEventListener("click",()=>{const s=l.classList.toggle("active");i(s);}),r.appendChild(l),r}createSliderControl(e,t,i,r,n){const l=document.createElement("div");l.className="twitter-clean-ui-control";const d=document.createElement("span");d.className="twitter-clean-ui-label",d.textContent=e,l.appendChild(d);const s=document.createElement("div");s.className="twitter-clean-ui-slider-container";const a=document.createElement("input");a.type="range",a.className="twitter-clean-ui-slider",a.min=String(i),a.max=String(r),a.value=String(t),s.appendChild(a);const u=document.createElement("span");return u.className="twitter-clean-ui-slider-value",u.textContent=`${t}px`,s.appendChild(u),a.addEventListener("input",()=>{const p=Number(a.value);u.textContent=`${p}px`,n(p);}),l.appendChild(s),l}createFooter(){const e=document.createElement("div");e.className="twitter-clean-ui-footer";const t=document.createElement("button");t.className="twitter-clean-ui-button twitter-clean-ui-button-danger",t.textContent=c("reset"),t.addEventListener("click",()=>{confirm("設定をリセットしますか？")&&(this.settingsManager.reset(),this.controller.applySettings(this.settingsManager.getSettings()),this.showTab("visibility"));}),e.appendChild(t);const i=document.createElement("button");return i.className="twitter-clean-ui-button twitter-clean-ui-button-secondary",i.textContent=c("close"),i.addEventListener("click",()=>this.hide()),e.appendChild(i),e}destroy(){this.hide(),this.styleElement&&this.styleElement.parentNode&&this.styleElement.parentNode.removeChild(this.styleElement);}}class N{detector;controller;settingsManager;settingsUI;isInitialized=false;settingsWatcherInterval=null;mutationObserver=null;applySettingsDebounceTimer=null;constructor(){this.detector=new C,this.controller=new v(this.detector),this.settingsManager=new E,this.settingsUI=new M(this.settingsManager,this.controller);}async initialize(){if(this.isInitialized){console.log("[TwitterCleanUI] Already initialized");return}console.log("[TwitterCleanUI] Initializing...");try{const e=this.settingsManager.getSettings();T(e.language||P()),this.detector.detectAll();const t=this.detector.getStatistics();console.log(`[TwitterCleanUI] Detected ${t.detected}/${t.total} elements`),this.controller.applySettings(e),console.log("[TwitterCleanUI] Settings applied"),this.startMutationObserver(),this.registerMenuCommand(),this.startPromotedTweetsWatcher(),this.startSettingsWatcher(),this.isInitialized=!0,console.log("[TwitterCleanUI] Initialized successfully");}catch(e){console.error("[TwitterCleanUI] Initialization failed:",e);}}startMutationObserver(){this.mutationObserver=new MutationObserver(()=>{this.applySettingsDebounceTimer&&clearTimeout(this.applySettingsDebounceTimer),this.applySettingsDebounceTimer=setTimeout(()=>{this.detector.detectAll(),this.controller.applySettings(this.settingsManager.getSettings());},100);}),this.mutationObserver.observe(document.body,{childList:true,subtree:true});}startSettingsWatcher(){let e=0;const t=setInterval(()=>{this.detector.detectAll(),this.controller.applySettings(this.settingsManager.getSettings()),e++,e>=10&&clearInterval(t);},500);this.settingsWatcherInterval=setInterval(()=>{this.detector.detectAll(),this.controller.applySettings(this.settingsManager.getSettings());},3e3);}registerMenuCommand(){typeof GM_registerMenuCommand<"u"?GM_registerMenuCommand("設定を開く",()=>{this.settingsUI.show();}):(document.addEventListener("keydown",e=>{e.ctrlKey&&e.shiftKey&&e.key==="X"&&(e.preventDefault(),this.settingsUI.show());}),console.log("[TwitterCleanUI] Keyboard shortcut registered: Ctrl + Shift + X"));}startPromotedTweetsWatcher(){this.settingsManager.getSettings().visibility.promotedTweets||setInterval(()=>{this.controller.hideAllPromotedTweets();},2e3);}destroy(){this.settingsWatcherInterval&&clearInterval(this.settingsWatcherInterval),this.applySettingsDebounceTimer&&clearTimeout(this.applySettingsDebounceTimer),this.mutationObserver&&this.mutationObserver.disconnect(),this.detector.stopObserving(),this.controller.destroy(),this.settingsUI.destroy(),this.isInitialized=false,console.log("[TwitterCleanUI] Destroyed");}}function R(){return new Promise(o=>{document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>o()):o();})}function I(o=1e4){return new Promise((e,t)=>{const i=Date.now(),r=()=>{if(document.getElementById("react-root")){e();return}if(Date.now()-i>o){t(new Error("Timeout waiting for react-root"));return}setTimeout(r,100);};r();})}(async()=>{try{console.log("[TwitterCleanUI] Starting..."),await R(),console.log("[TwitterCleanUI] Page loaded"),await I(),console.log("[TwitterCleanUI] React root found");const o=new N;await o.initialize(),window.twitterCleanUI=o;}catch(o){console.error("[TwitterCleanUI] Fatal error:",o);}})();
