 ...
 */
export async function expandDescriptionIfNeeded(
  timeoutMs = 4000,
): Promise<void> {
  ...
  const clickExpandButton = (): boolean => {
    const inlineExpander =
      descriptionEl.querySelector<HTMLElement>(
        YOUTUBE_SELECTORS.inlineExpander,
      ) ||
    ...
    // フォールバック: 既存の方法
    const matchRegex =
      /(もっと見る|もっと表示|もっと読む|Show more|SHOW MORE|More|一部を表示|…|...もっと見る)/i;
    const candidates: HTMLElement[] = [];

    if (inlineExpander) {
      candidates.push(
        ...Array.from(
          inlineExpander.querySelectorAll<HTMLElement>(
            YOUTUBE_SELECTORS.interactiveElements,
          ),
        ),
      );
    }
    ...
    for (const el of candidates) {
      try {
        ...
        if (
          aria === "false" ||
          matchRegex.test(txt) ||
          (el.id && /expand|more|collapse/i.test(el.id))
        ) {
          if (typeof el.click === "function") {
            logger.debug(`Clicking element: ${el.tagName}#${el.id} "${txt}"`);
            el.click();
            logger.info(
              "概要欄の「もっと見る」ボタンをクリックしました（フォールバック）。",
            );
            return true;
          }
        }
  ...
  const isExpanded = (): boolean => {
    ...
    const expandedDiv = (
      inlineExpander || descriptionEl
    ).querySelector<HTMLElement>("#expanded");
    if (expandedDiv) {
      const expandedText = (
        expandedDiv.textContent ||
        expandedDiv.innerText ||
        ""
      ).trim();
      // 展開前は空（0文字）、展開後は数百文字以上になる
      if (expandedText && expandedText.length > 200) {
        logger.debug(
          `Description is expanded (found ${expandedText.length} chars in #expanded)`,
        );
        return true;
      }
    }
  ...
  return new Promise((resolve) => {
    ...
    const checkExpansion = (source: string) => {
      ...
      const expandedLength = expandedDiv
        ? (expandedDiv.textContent || "").trim().length
        : 0;
      const expandedNow = isExpanded();
      const elapsed = Date.now() - startTime;

      logger.debug(
        `Check ${checkCount} (${source}): expanded=${expandedNow}, #expanded length=${expandedLength}, elapsed=${elapsed}ms`,
      );

      if (expandedNow) {
        cleanup();
        logger.info(
          `Description expanded successfully (#expanded: ${expandedLength} chars) after ${elapsed}ms`,
        );
        // DOMの完全な更新を待つ
      ...
      if (elapsed > timeoutMs) {
        cleanup();
        logger.warn(
          `Description expansion timed out after ${elapsed}ms (${checkCount} checks)`,
        );
        resolve();
        return true;
      }
