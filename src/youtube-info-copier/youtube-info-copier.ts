...
export class YouTubeInfoCopier {
  ...
  private init(): void {
    ...
    this.logger.info(
      `YouTubeInfoCopier initialized (style: ${this.launchStyle})`,
    );
  }
  ...
  private setupEventListeners(): void {
    ...
    const buttons =
      this.shadowRoot.querySelectorAll<HTMLButtonElement>(".panel-button");
  ...
  private async handleButtonClick(action: string): Promise<void> {
    try {
      ...
      if (action === "copy" && !this.descriptionExpanded) {
        this.logger.info(
          "Description not ready yet, please wait for animation...",
        );
  ...
  private async getVideoInfo(): Promise<YouTubeVideoInfo> {
    ...
    if (description === "概要取得に失敗しました") {
      ...
      if (descriptionElement) {
        // #expanded 要素を優先的に探す
        const expandedDiv =
          descriptionElement.querySelector<HTMLElement>("#expanded");
        if (expandedDiv) {
          const expandedText = (
  ...
  private showPopup(description: string): void {
    ...
    const popupContent =
      this.shadowRoot.querySelector<HTMLDivElement>(".popup-content");
    if (popupContent) {
