...
const REVEAL_BOXES: ReadonlyArray<RevealBoxConfig> = [
  ...
  },
  {
    ...
    shouldShow: (settings) => settings.hideEndScreen && document.querySelector('#movie_player.ended-mode') !== null,
  },
];
...
export class RevealBoxManager {
  ...
  public apply(settings: YoutubeUiModifierSettings, onSettingChange: YoutubeUiModifierSettingsChangeHandler): void {
    ...
    REVEAL_BOXES.forEach((config) => {
      if (!settings[config.enabledSetting] || !config.shouldShow(settings) || this.closedBoxIds.has(config.boxId)) {
  ...
  private ensureBox(config: RevealBoxConfig, onSettingChange: YoutubeUiModifierSettingsChangeHandler): void {
  ...
  private removeAll(): void {
    REVEAL_BOXES.forEach((config) => document.getElementById(config.boxId)?.remove());
  }
}
