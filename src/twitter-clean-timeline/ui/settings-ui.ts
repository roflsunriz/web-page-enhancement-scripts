...
function createModal(): HTMLElement {
  ...
  replaceAddBtn?.addEventListener("click", () => {
    settings.replaceFilter.replacements.push({
      from: "",
      to: "",
      isRegex: false,
    });
  ...
  const replaceTableContainer = modalContent.querySelector(
    "#ctl-replace-table-container",
  );
...
function saveSettingsFromModal(modal: HTMLElement): void {
  const getValue = (id: string): boolean => {
    return (
      (modal.querySelector(`#${id}`) as HTMLInputElement)?.checked ?? false
    );
  };
