...
- ファイル名の命名規則はケバブケースです。
- 編集完了後に品質確認を行ってください。具体的には、npm run lint, npm run type-check, npm run buildをこの順番で実行してください。
...
- src/twitter-media-filterはtwitter-media-filter.user.jsを編集するためのプロジェクトです。
- src/twitter-mute-filterはtwitter-mute-filter.user.jsを編集するためのプロジェクトです。
- src/twitter-mute-retweetsはtwitter-mute-retweets.user.jsを編集するためのプロジェクトです。
- src/twitter-thread-copierはtwitter-thread-copier.user.jsを編集するためのプロジェクトです。
- src/twitter-wide-layout-fixはtwitter-wide-layout-fix.user.jsを編集するためのプロジェクトです。
