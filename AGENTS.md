...
- ユーザースクリプトを変更しない管理設定だけのコミットでは、ユーザーの2026-09-23指定に従って `package.json` のリポジトリ版だけを上げる。個別スクリプトの版は実体に変更があるときだけ上げる。`CHANGELOG.md` には対象を取り違えないよう、リポジトリ運用の節で理由と検証を記録する。
- ファイル名の命名規則はケバブケース。
